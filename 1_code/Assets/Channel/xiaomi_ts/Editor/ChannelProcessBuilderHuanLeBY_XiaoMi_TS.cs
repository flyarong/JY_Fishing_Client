...
class PreProcessBuilderHuanLeBY_XiaoMi_TS : IPreprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
		string channelName = AppDefine.CurQuDao;
		if (string.Compare (channelName, "xiaomi_ts", true) != 0)
			return;

		UnityEngine.Debug.Log ("OnPreprocessBuild for xiaomi_ts channel!");

		PlayerSettings.applicationIdentifier = "com.changleyou.tthlby.mi";
...
class PostProcessBuilderHuanLeBY_XiaoMi_TS : IPostprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPostprocessBuild(BuildTarget buildTarget, string path) {
		...
		UnityEngine.Debug.Log ("OnPostprocessBuild for xiaomi_ts channel!");
	...
	private void PostprocessAndroid(string path) {
		...
		string[] deleteFiles = new string[] {
			"libs/android-gif-drawable-1.2.6.aar", "libs/open_ad_sdk.aar", "libs/volley.jar",
			"libs/OpenInstall_v2.3.2.jar","libs/openinstall-unity.jar"
		};
