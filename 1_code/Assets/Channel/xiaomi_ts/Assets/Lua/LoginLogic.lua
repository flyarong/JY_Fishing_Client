...
local function xiaomiTokenToLogin()
    local function callback(json_data)
        local lua_tbl = json2lua(json_data)
        if not lua_tbl then
            print("[LOGIN] xiaomiTokenToLogin exception: json_data invalid")
            return
        end

        dump(lua_tbl, "[LOGIN] xiaomiTokenToLogin")
        ...
        if lua_tbl.result == 0 then
            ...
            UnityEngine.PlayerPrefs.SetString("_APPID_", lua_tbl.uid)
            Network.SendRequest("login", loginData)
            Event.Brocast("bsds_send_power",{key = "login_xiaomi"})
        else
            FullSceneJH.RemoveAll()
            status = nil
...
local function login()
    ...
    elseif curChannel == "robot" then
        ...
        Network.SendRequest("login", loginData)
    elseif curChannel == "xiaomi" then
        local uid = UnityEngine.PlayerPrefs.GetString("_APPID_", "")
        dump({curLoginId = curLoginId,uid = uid,session = curToken},"<color=white>请求登录</color>")
		
		--自动登录，会有账号问题
		uid = ""

        if curLoginId and uid ~= "" then
            local tbl = {}
            tbl.uid = uid
            tbl.session = curToken
            local loginData = {
                channel_type = "xiaomi",
                login_id = curLoginId,
    ...
    elseif curChannel == "phone" then
        ...
        if curLoginId and curLoginId ~= "" and curToken and curToken ~= "" then
            ...
            local loginData = {
                ...
                market_channel = gameMgr:getMarketChannel(),
                platform = gameMgr:getMarketPlatform(),
                youke_id = loginModel.loginData.youke
            }
...
function LoginLogic.OnLoginResult(result)
    ...
    elseif result == 2153 or result == 2155 or result == 2156 or (result == 2150 and curChannel == "xiaomi") then
        if wechatErrorStatus == 0 then
            LoginModel.ClearChannelData("xiaomi")
            wechatErrorStatus = 1
...
function LoginLogic.AutoLogin()
    if loginModel.loginData.lastLoginChannel == "xiaomi" then
        LoginLogic.xiaomiLogin()
...
--微信登陆
function LoginLogic.xiaomiLogin()
    if LoginLogic.checkServerStatus then
        if not LoginLogic.CheckServerStatus(true) then
            return
        end
