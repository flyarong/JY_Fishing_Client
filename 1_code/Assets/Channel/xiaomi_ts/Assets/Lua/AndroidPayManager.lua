...
function AndroidPayManager.PayCallback(json_data)
	...
	-- 	end)
		...
		Network.SendRequest("xiaomi_wqp_pay_info", lua_tbl, function(_ret)
...
--[[function AndroidPayManager.DoOrder(order)
	...
	Network.SendRequest("xiaomi_wqp_pay_info", order, function(_data)
		dump(_data, "[PAY] tracepay: xiaomi_wqp_pay_info")
...
function AndroidPayManager.ConvertXiaoMiPay(order)
...
function AndroidPayManager.DealOrder(order, callback)
	local needPostPay, order_tbl = AndroidPayManager.ConvertXiaoMiPay(order)
	if not order_tbl then
		dump(order, "[PAY] ConvertXiaoMiPay failed")
		print("[PAY] ConvertXiaoMiPay failed, order_tbl is null")
		return
	end
	...
	Network.SendRequest("xiaomi_pay_info", order_tbl, function(_data)
		dump(_data, "[PAY] tracepay: xiaomi_pay_info")
