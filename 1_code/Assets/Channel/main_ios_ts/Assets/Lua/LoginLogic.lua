...
local function setSendLoginOverTimeCBK()
    ...
    local function cbk()
        ...
        if status == 1 or status == 2 then
			...
			--elseif curChannel == "apple" then
			--	LoginLogic.clearIOSData()
            end
...
local function iosTokenToLogin()
	local function callback(json_data)
        local lua_tbl = json2lua(json_data)
        if not lua_tbl then
            print("[LOGIN] iosTokenToLogin exception: json_data invalid")
            return
        end

        dump(lua_tbl, "[LOGIN] iosTokenToLogin")
        ...
        if lua_tbl.result == 0 then
            ...
            MainModel.LoginInfo = loginData
            UnityEngine.PlayerPrefs.SetString("_APPID_", lua_tbl.user or "")
            Network.SendRequest("login", loginData)
        else
            FullSceneJH.RemoveAll()
            status = nil
            if lua_tbl.result == -5 then
                --sdk error
                local channel = MainModel.LoginInfo.channel_type or ""
				if lua_tbl.errno == 10001 then
					HintPanel.Create(1, "请更新IOS系统至13.0以上才能进行Apple登录")
				else
	                HintPanel.Create(1, "登陆IOS失败")
				end
            elseif lua_tbl.result == -4 then
                HintPanel.ErrorMsg(3032)
            elseif lua_tbl.result == -2 then
	...
	local lt = {}
	lt.platform = "ios"
    sdkMgr:Login(lua2json(lt), callback)
end
...
local function login()
	...
	elseif curChannel == "apple" then
        local appid = UnityEngine.PlayerPrefs.GetString("_APPID_", "")
        dump({curLoginId = curLoginId,appid = appid,curToken = curToken},"<color=white>请求登录</color>")
        if curLoginId and appid ~= "" and curToken ~= "" then
            local tbl = {}
            tbl.user = appid
            tbl.refresh_token = curToken
            local loginData = {
...
function LoginLogic.OnLoginResult(result)
    ...
    elseif result == 2153 or result == 2155 or result == 2160 or result == 2161 or result == 2156 or (result == 2150 and (curChannel == "wechat" or curChannel == "apple")) then
		if curChannel == "wechat" then
		...
		elseif curChannel == "apple" then
			if wechatErrorStatus == 0 then
				cancelLogin()
				LoginModel.ClearChannelData("apple")
				wechatErrorStatus = 1
...
function LoginLogic.AutoLogin()
	...
	elseif loginModel.loginData.lastLoginChannel == "apple" then
		LoginLogic.IOSLogin()
    else
...
function LoginLogic.WechatLogin()
    ...
    readLogin("wechat", loginId, refresh_token)
end
...
function LoginLogic.IOSLogin()
    ...
    local loginId = loginModel.loginData.apple
    local refresh_token = loginModel.loginData.refresh_token
    dump({loginId = loginId, refresh_token = refresh_token}, "<color=white></color>")
    readLogin("apple", loginId, refresh_token)
end
...
function LoginLogic.clearIOSData()
	loginModel.ClearChannelData("apple")
end
