...
function LoginPanel:Start()
	...
	self.behaviour:AddClick(self.login_ios_btn.gameObject, LoginPanel.OnLoginIOSClick, self)
...
function LoginPanel:OnLoginIOSClick(go)
	DSM.PushAct({button = "ios_btn"})
	if self.privacy == true and self.service == true then
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		LoginLogic.IOSLogin()
	else
...
function LoginPanel:OnLoginIOSCloseClick(go)
