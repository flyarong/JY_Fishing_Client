...
local Button_DataS = {
	...
	{name = "赢金挑战",goto_ui = "vipyjtz",CheakFunc = "CheakViP_YJTZ_CanShow"},
...
function C:SetDefaultButton(cfg)
	dump(cfg,"<color=yellow><size=15>++++++++++cfg++++++++++</size></color>")
	local button_data
...
function C.CheakViP_YJTZ_CanShow()
	-- local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key= "actp_own_task_p_vip_game_award_task", is_on_hint = true}, "CheckCondition")
	-- if a and b then
	-- 	return true
	-- end
	-- return false
...
function C:CheakFHFL_CanShow()
	local data = GameTaskModel.GetTaskDataByID(21341)
	if not data then
		return false
	end
