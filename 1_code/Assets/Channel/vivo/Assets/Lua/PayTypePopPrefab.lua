...
function PayTypePopPrefab.Create(goodsid, desc, createcall,convert)
	...
	local call = function ()
        Network.SendRequest("vivo_create_pay_order", request, "", function(_data)
            dump(_data, "<color=green>返回订单号</color>")
            --vivo支付回调地址
           
            if _data.result == 0 then
                local luaData = {
                    productId = request.goods_id,
                    productName = goods_data.pay_title,
                    amount = goods_data.price,
                    priceType = 0,
                    developerPayload = _data.order_id,
                    productDesc =  goods_data.pay_title,
                    notifyUrl = notifyUrl,
                }
                if convert then
                    luaData.productName = luaData.productName .. "-->金币"
                end
    ...
    Network.SendRequest("vivo_query_notify_url", nil, "", function (data)
