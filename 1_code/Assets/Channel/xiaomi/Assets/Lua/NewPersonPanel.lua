...
--------------------------------------------------
   ...
   local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="drt_block_binding_phone_num", is_on_hint = true}, "CheckCondition")
    if not MainModel.IsLowPlayer() and (not a or (a and not b)) and GameGlobalOnOff.BindingPhone and parm and parm.open_award then
...
-----------------------------------------------------------------👆
    ...
    if MainModel.IsLowPlayer() then
        self.Certified.gameObject:SetActive(false)
