...
function ZPGLogic.change_panel(panelName, pram)
    ...
    if not cur_panel then
        if panelName == ZPGLogic.panelNameMap.hall then
            GameManager.GotoSceneName("game_MiniGame", ZPGModel.baseData.game_type,function()
...
function ZPGLogic.Init(pram)
    ...
    if not pram then
        SendRequestAllInfo()
    -- else
    --     Network.SendRequest("guess_apple_enter_room",nil,"",function (data)
    --         dump(data,"<color=red>guess_apple_enter_room</color>")
    ...
    -- if pram and pram.all_info then
    --     ZPGLogic.change_panel(ZPGLogic.panelNameMap.game, pram)
    --     ZPGModel.on_fg_all_info("guess_apple_all_info_response",pram.all_info)
    -- else
    --     Network.SendRequest("guess_apple_enter_room",nil,"",function (data)
    --         dump(data,"<color=red>guess_apple_enter_room</color>")
    --         if data.result == 0 then
...
function ZPGLogic.quit_game(call, quit_msg_call)
    Network.SendRequest("guess_apple_quit_room", nil, "请求退出", function (data)
