...
---------------------------------2021.8.24游戏小优化----------------------  --]]     
    ...
    SpineManager.AddDDZPlayerSpine(self.spine_dz_nan.transform:GetComponent("SkeletonAnimation"),1)
    SpineManager.AddDDZPlayerSpine(self.spine_nm_nan.transform:GetComponent("SkeletonAnimation"),3)
    ...
    local btn_map = {}
    btn_map["left_top"] = {self.btnnode,self.btnnode_2,self.btnnode_3}
    ...
    elseif ZPGModel.data.game_status == "game" then
        ...
        ZPGAnimManager.PlayPlantEnd(self.layer2,function()
            ZPGAnimManager.PlayAppleAnim(self.AreaObject,self.layer2,function() 
                ...
                if ZPGModel.data and ZPGModel.data.winner then
...
function C:CheckCanBetByMoney()
    ...
    local item_num = GameItemModel.GetItemCount("prop_guess_apple_bet_" .. ZPGModel.data.current_bet_index)
    if bet_money > self.player_gold and item_num <= 0 then
...
function StartCountDown(timeout,targetTxt,callback,HighLightObj)
    countdown = timeout
...
function CaculateItemIndexByTotal(total)
...
function CaculateItemIndexForShow(total)
    --线性减少显示的数量
...
function CalculateAppleShowByCount(count)
    count = count or 0
