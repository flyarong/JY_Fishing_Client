...
-- guess_apple_bet @ {
--   request {
--     bet_index $ : integer    # 下注的投入索引，如果有替代资产，就不会扣金币
...
function M.on_fg_all_info(_,data)
    ...
    if data.result ~= 0 then
        ...
        if s then
            ...
            m_data.winner = CaculateWinner(m_data.apple_data.left_apple,m_data.apple_data.right_apple)
        end
...
function CaculateWinner(left,right)
