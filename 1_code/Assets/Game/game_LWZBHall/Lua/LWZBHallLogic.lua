-- 创建时间:2020-08-26
local cur_path = "Game.game_LWZBHall.Lua."
ext_require(cur_path .. "LWZBHallModel")
ext_require(cur_path .. "LWZBHallGamePanel")
ext_require(cur_path .. "LWZBHelpPanel")


LWZBHallLogic = {}
local L = LWZBHallLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    ...
    this = L
    dump(parm, "<color=red>LWZBHallLogic Init parm</color>")
    --初始化model
    local model = LWZBHallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  LWZBHallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = LWZBHallGamePanel.Create()}
        end
    end
end

return LWZBHallLogic