...
DdzActionUiManger = basefunc.class()


function DdzActionUiManger.Create(gamePanel,playerOperateUI,dizhuCardUI)
	local instance=DdzActionUiManger.New()
...
function DdzActionUiManger:Refresh()
	--全部隐藏
	...
	if DdzMatchModel.data then
		...
		local hash={}
		if act_list and #act_list>0 and DdzMatchModel.data.s2cSeatNum then
			local len= #act_list
			local len_ = len
				...
				if i>0 then
					...
					if not hash[act.p] then
						if not m_data.cur_p or  (act.p~=m_data.cur_p and m_data.cur_p~=6) or m_data.status==nor_ddz_base_lib.get_action_status(act) then
							hash[act.p]=true
							self:RefreshAction(DdzMatchModel.data.s2cSeatNum[act.p],act)
						end
					end
				else
					break
...
--根据当前状态调整动作的UI的显示
function DdzActionUiManger:changeActionUIShowByStatus()
	--如果上一个已经是不同类型
	local m_data=DdzMatchModel.data
	if m_data then
		if m_data.status==DdzMatchModel.Status.cp then
			local cur_p=m_data.cur_p
			if cur_p and DdzMatchModel.data.s2cSeatNum then 
			 	local other_p={}
			 	other_p[1]=DdzMatchModel.data.s2cSeatNum[cur_p]
			 	for i=2,DdzMatchModel.maxPlayerNumber do
			 		other_p[i]=other_p[i-1]-1
			 		if other_p[i]==0 then
			 			other_p[i]=DdzMatchModel.maxPlayerNumber
			 		end
			 	end
...
function DdzActionUiManger:RefreshAction(seatNum,act)
	--清空之前残留
	...
	if act.type>99 or act.type==0 then
		...
		DdzMatchModel.ddz_algorithm:get_cpInfo_by_action(act)
		...
		if DdzMatchModel.data then
			if DdzMatchModel.data.seatNum[seatNum] == DdzMatchModel.data.dizhu then
...
--带动画和音效刷新
function DdzActionUiManger:RefreshActionWithAni(seatNum,act)
	...
	if act.type>99 or act.type==0 then
		...
		elseif act.type==101 then
			if act.rate==2 then
				...
				--绕过了服务器的情况
				if seatNum==1 and not act.p then
		...
		elseif act.type==102 then
			if act.rate==0 then-- 不抢地主
				...
				sound="sod_game_rob"
				DdzMatchModel.data.er_qiang_dizhu_count = DdzMatchModel.data.er_qiang_dizhu_count + 1
				local qtext = self.playerOperateUI[1].qiang_hint_txt
				if IsEquals(qtext) then
					qtext.text = "抢" .. DdzMatchModel.data.er_qiang_dizhu_count .. "次"
				end
			end
		end
		sound=sound ..  AudioBySex(DdzMatchModel, DdzMatchModel.data.seatNum[seatNum])
			...
			if idx==#self.showCardList[seatNum] then
				if act.type == 13 or act.type == 14 then
					...
					if  DdzMatchModel.data.seatNum[seatNum] == DdzMatchModel.data.dizhu then
						...
						for i=1,DdzMatchModel.maxPlayerNumber do
							if i ~= seatNum then
								if DdzMatchModel.data.seatNum[i] == DdzMatchModel.data.dizhu then
									SpineManager.BeiZha(i)
									ParticleManager.DDZBeiZha(i)
								else
...
function DdzActionUiManger:PlayBombBGM()
	--播放炸弹不切换背景音
	...
	if not self.bombbgmKey then
		...
		self.bombbgmKey = ExtendSoundManager.PlaySound(audio_config.ddz.bgm_gameboom.audio_name, 1, function ()
			self.bombbgmKey = nil
			self:RefreshBGM(true)
		end)
	end
end
function DdzActionUiManger:ClearCardList()
end

function DdzActionUiManger:MyExit()
...
--刷新背景音
function DdzActionUiManger:RefreshBGM(isCoerce)
    if not self.bombbgmKey and DdzMatchModel.data and DdzMatchModel.data.total_round and DdzMatchModel.data.round_info and DdzMatchModel.data.round_info.round then
        if DdzMatchModel.data.total_round == DdzMatchModel.data.round_info.round then
            --决赛背景音
...
function DdzActionUiManger:PlayBSKS()
