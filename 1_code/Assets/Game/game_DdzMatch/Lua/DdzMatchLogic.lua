...
function DdzMatchLogic.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == panelNameMap.wait then
            cur_panel = {name = panelName, instance = MatchComWaitStartPanel.Create({model = DdzMatchModel,logic = DdzMatchLogic,ani = DDZAnimation})}
        elseif panelName == panelNameMap.wait_rematch then
            cur_panel = {name = panelName, instance = MatchComWaitRematchPanel.Create({model = DdzMatchModel,logic = DdzMatchLogic,ani = DDZAnimation})}
        ...
        elseif panelName == panelNameMap.rank then
            ...
            cur_panel = {name = panelName, instance = MatchComRankPanel.Create(parm, function (game_id)
                DdzMatchModel.ClearMatchData(game_id)
            end)}
        end
    end
