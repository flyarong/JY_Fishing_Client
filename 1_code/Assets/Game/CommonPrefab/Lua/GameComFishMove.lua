...
GameComFishMove = basefunc.class()
local C = GameComFishMove
...
function C:ctor(parm)
	...
	self.jd = 100
    self.map = parm.map or {xMin=-800, xMax=800, yMin=-460, yMax=460}
    self.init_speed = parm.init_speed or 1

	self.speed_list = {}
	self.speed_list[#self.speed_list + 1] = VehicleSpeed_YS.Create(self, {speed=self.init_speed})
...
function C:FrameUpdate(time_elapsed)
	...
	if self.cur_time > 0 then
		...
		if self.entity and IsEquals(self.entity) then
			self.entity:UpdateTransform(self.cur_pos, Vec2DAngle(self.cur_heading))
		end
	end
end
...
function C:Start()
	...
	self.cur_speed = self.cur_speed or (1 * self.init_speed)
end
function C:Stop()
	self.isStart = false
end
...
function C:JiaS()
	...
	parm.speed1 = self.cur_speed + (3 * self.init_speed)
	parm.time = 1
	parm.curve = 1
