...
function C.Init()
	GameButtonManager.Exit()
	print("<color=red>初始化全新的活动系统</color>")
    this = GameButtonManager
    MakeLister()
    AddLister()
end
function C.Exit()
    ...
    if result == 0 then
        ...
        if MainModel.IsLowPlayer() then
            xpcall(function ()
                ...
                game_module_config = HotUpdateConfig("Game." .. platform .. ".Lua.game_module_config_v1")
            end, function (error)
...
function C.ExtLoadLua(key, lua_name)
    return ext_require("Game.Activity." .. key .. ".Lua." .. lua_name)
end
-- 加载Lua
function C.LoadLua()
...
function C.InitConfig()
    ...
    -- 特殊管理器
    this.Config.module_map["sys_qx"] = {lua="SYSQXManager",key="sys_qx"}
...
-- 加载的模块配置
function C.GetLoadModuleByKey(key)
    return this.Config.module_map[key]
end

-- 活动对应的跳转
function C.GotoUI(parm)
    local v = this.Config.module_map[parm.gotoui]
    if v then
...
function C.RunFunExt(gotoui, fun_name, call, ...)
    local v = this.Config.module_map[gotoui]
    if v then
        if _G[v.lua] and _G[v.lua][fun_name] then
            if type (_G[v.lua][fun_name]) == "function" then
                return true, _G[v.lua][fun_name](...)
...
function C.GetGameEnterCondiKeyByKey(Key)
    for k,v in pairs(this.Config.all_enter_btn_list) do
        if v.parm and v.parm[1] and v.parm[1] == Key then
            return v.condi_key
        end
    end
end