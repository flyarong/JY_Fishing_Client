...
RedHintManager.RedHintKey = 
{
	...
	RHK_BBSC_Task = "RHK_BBSC_Task",-- 新人红包任务
	...
	RHK_Daily_Task = "RHK_Daily_Task", -- 每日活跃任务
	RHK_Daily_Task_GET = "RHK_Daily_Task_GET", -- 每日活跃任务领取提示
...
RedHintManager.IsKeyRed = function (key)
	...
	elseif key == RedHintManager.RedHintKey.RHK_Daily_Task then
		return ActiveDailyTaskManager.IsRedDailyTaskGet()
	elseif Key == RedHintManager.RedHintKey.RHK_Daily_Task_GET then
		return ActiveDailyTaskManager.IsRedDailyTaskGet()
	else
		return false
	end
...
RedHintManager.IsRedEmailNew = function ()
	...
	if ss == ACTIVITY_HINT_STATUS_ENUM.AT_Red then
		return true
	else
		return false
	end
...
RedHintManager.IsMoneyCenter = function ()
	if not _G["GameMoneyCenterModel"] then
		return false
	end
...
RedHintManager.IsRedHallBag = function ()
	if GameItemModel.IsBagRadByTag(1) then
		return true
	end
	local a,b = GameButtonManager.RunFunExt("sys_by_bag", "IsHaveRad")
	if a and b then
		return true
	else
		return false
...
RedHintManager.IsRedHallActivity = function ()
	if not _G["SYSACTBASEManager"] then
		return false
	  end
	
	if SYSACTBASEManager.GetHintState({goto_type="normal"}) == ACTIVITY_HINT_STATUS_ENUM.AT_Red then
		return true
	else
		return false
	end
...
RedHintManager.IsRedHallActivityGet = function ()
	...
	if SYSACTBASEManager.GetHintState({goto_type="normal"}) == ACTIVITY_HINT_STATUS_ENUM.AT_Get then
		return true
	else
		return false
	end
...
RedHintManager.IsRedHallTaskAccurate = function ()
	if SYSXYJLManager and SYSXYJLManager.CheckTaskAccurateCanGet() == true then
        return true
    else
        return false
    end
...
RedHintManager.IsRedBBSCHallTask = function ()
	-- dump(ActivitySevenDayModel.CanGetStatus, "<color=yellow>新人红包可领取</color>")
...
RedHintManager.IsRedVIPGiftHallTask = function ()
	if VIPGiftModel and VIPGiftModel.CanGetStatus.vip_gift == true	then
        return true
    else
        return false
    end
...
RedHintManager.IsRedWeeklyCardHallTask = function ()
	if not _G["ActivityShop20Panel"] then
		return false
	end
	...
	if has_award54 then
		if HallPanel.GetQYSZhouKaRemain() == 0 then return true end
	end
...
RedHintManager.IsRedFuliGet = function ()
	if JYFLManager and JYFLManager.GetHintState() == ACTIVITY_HINT_STATUS_ENUM.AT_Get then 
		return true
	else
		return false
	end
