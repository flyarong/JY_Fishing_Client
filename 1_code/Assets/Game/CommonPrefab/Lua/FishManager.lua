...
function C.InstantiateFish(data)
    ...
    if MainModel.myLocation == "game_Fishing3D" then
        if data.fish_style == "fish_team" then
            ...
            if use_fish_cfg.fish_id == 38 then
                fish = Fish3DJLong.Create(FishGroupNodeTran, data)
            elseif use_fish_cfg.fish_id == 36 then
                local panel = FishingLogic.GetPanel()
                local node = panel:GetTSPathNode(data.path, use_fish_cfg.fish_id)
                fish = Fish3DHYLong.Create(node, data)
            elseif use_fish_cfg.fish_id == 50 then
                fish = Fish3DBOX.Create(tran, data)
            elseif use_fish_cfg.fish_id == 49 then
                local panel = FishingLogic.GetPanel()
                if panel.ts_fish_root and panel.ts_fish_root[data.path] then
                    fish = Fish3DZYCS.Create(panel.ts_fish_root[data.path], data)
                else
                    dump(panel.ts_fish_root, "<color=red>DDDDDDDDDDDDDDDDDDDDDD</color>")
                end            
            else
...
function C.CreateFish(data)
    ...
    if fish_cstime <= cur_time then
        ...
        -- 创建领航员
        pilot = VehicleManager.Create(FishNodeTran, {ID=data.fish_id, m_vPos=m_vPos, m_vHeading=m_vHeading, m_vSide=m_vSide}, fish_cfg.speed_type)
        ...
        if FishingModel.IsRecoverRet then
            if FishingModel.data.frozen_time_data and next(FishingModel.data.frozen_time_data) then
                for k,v in ipairs(FishingModel.data.frozen_time_data) do
                    local a = v
                    local b = v + FishingModel.Defines.frozenIndate
                    if b > cur_time then
                        b = cur_time
                    end
...
--鱼的死亡消息(注意: 鱼可以死多次)
function C.S2CFishDead(fish_id, ZZ, parm)
...
function C.lightning_harm_sx(rateList1, rateList2, all_rate, max_rate, max_num)
...
-- 范围闪电
function C.CalcLightningFishHarmAndPos(fish_id, max_rate, seat_num, max_num, pos, r)
    r = r or 4
    local max_num = max_num or 10
    local pj_rate = max_rate / max_num
    local vec1 = {x=pos.x, y=pos.y}
    ...
    for k,v in pairs(FishTable) do
        ...
        if (not fish_id or k ~= fish_id) and (not v.data.seat_num or v.data.seat_num == seat_num) and
            (not v.data.ori_life or v.data.ori_life <= 0) and v:CheckIsInPool_Whole() and Vec2DLength(Vec2DSub(vec1, vec2)) <= r then
    ...
    local cur_list = C.lightning_harm_sx(rateList1, rateList2, all_rate, max_rate, max_num)
    dump(cur_list, "<color=red>计算范围闪电</color>")
    return cur_list
end
...
function C.CalcBulletHarm(fish_id, pos, seat_num)
    ...
    for k,v in pairs(FishTable) do
        if k ~= fish_id and (not v.data.seat_num or v.data.seat_num == seat_num) and v:CheckIsInPool() and fish_id then
            if IsEquals(v.transform) then
                local vec2 = {x=v.transform.position.x, y=v.transform.position.y}
                if Vec2DLength(Vec2DSub(vec1, vec2)) <= r then
                    list[#list + 1] = v.data.fish_id
                end
            else
                dump(v, "<color=red>EEE nil transform</color>")
            end
        end
    end
    cur_list = C.calc_fish_hit_list(list, sx_cfg)
