...
function C:ctor(parent, data)
    ...
    if net_type then
        self.prefab = CachePrefabManager.Take("FishNetPrefab_" .. net_type, nil, 10)
    else
        if gun_config.net_prefab then
            self.prefab = CachePrefabManager.Take(gun_config.net_prefab, nil, 10)
            ...
            self.prefab = CachePrefabManager.Take("FishNetPrefab_1", nil, 10)
        end
    end
