...
function M:Init()
    ...
    self:FirstSetGunLevel()
    self:FirstFreeSkill()
end

function M:FirstSetGunLevel()
    local s_d, s_t
...
function M:FirstFreeSkill()
    local s_d, s_t
    if self:CheckPlayerCurState("skill") then
        local cur_skill = self:GetPlayerCurState("skill")
        s_d, s_t = cur_skill.s_d, cur_skill.s_t
    else
        s_d, s_t = M.GetStatusData(self.config.player_speed.freed_skill,self.config.player_status.freed_skill,self.freed_skill_time,self.cur_time)
        if not s_d or not s_d.key then return end
...
function M.ChangeStatus(p,p_instance,fish_list,cfg)
    ...
    if sw_cfg.gold then
        for i,v in ipairs(sw_cfg.gold) do
            if v.num and v.num <= M.GetCurGold(p) then
                return M.ChangeCurStatus(cfg,v.status)
            end
        end
    end
...
function M.CheckFreeSkill(s_d,p,fish_list)
    ...
    elseif t_id == 4 then
        local power = FishingModel.GetGunSkinPower(p.base.seat_num, p.index)
    ...
    elseif t_id == 4 then
        ...
        if f and IsEquals(f.transform) then
            ...
            data.vec = Vector3.New(math.random(WDU.xMin,WDU.xMax),
                                    math.random(WDU.yMin,WDU.yMax), 0)
        end
    elseif t_id == 5 then
        --炮触发技能
        msg_type = "gun_skill"
