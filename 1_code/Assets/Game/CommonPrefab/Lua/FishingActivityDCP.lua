...
function M:ChangeGame()
    dump(self.data, "<color=red>OOOOOOOOOOOOOOOOOOOO ChangeGame</color>")
    ...
    if self.data.num and self.data.num > 0 and self.data.status == 1 then
        ...
        if n_g.gunprefab ~= "GunPrefab14" then
            self.org_gun_cfg = {}
            self.org_gun_cfg.gunprefab = n_g.gunprefab
            n_g.gunprefab = "GunPrefab14"
...
function M:PlayActivityNode()
    ...
    if not self.data.speed then
        ...
        seq:OnKill(function ()
            FishingAnimManager.PlayMoveAndHideFX(self.ui.transform, "Fishing3D_dianchixie", beginPos, endPos, 2, 1, function ()
