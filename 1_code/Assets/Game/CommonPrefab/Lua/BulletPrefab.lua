...
function C:ctor(parent, data)
    ...
    if data.type then
        if data.type == 2 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_cjhl", nil, 10)
        elseif data.type == 1 then            
            local rate = index > 5 and 2 or 1
            rate = 2-- rate or 1
            if rate == 1 then
                self.prefab = CachePrefabManager.Take("BulletPrefab_free1", nil, 10)
            else
                self.prefab = CachePrefabManager.Take("BulletPrefab_free2", nil, 10)
            end
        elseif data.type == 3 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_sbsk", nil, 10)
        elseif data.type == 4 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_f_cs", nil, 10)
        elseif data.type == 6 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_gd", nil, 10)
        elseif data.type == 8 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_3d_11", nil, 10)
        elseif data.type == 10 then
            self.prefab = CachePrefabManager.Take("BulletPrefab_3d_12", nil, 10)
        else
            if gun_config.bullet_prefab then
                self.prefab = CachePrefabManager.Take(gun_config.bullet_prefab, nil, 10)
            else
                self.prefab = CachePrefabManager.Take("BulletPrefab_" .. index, nil, 10)
            end
        end
    else
        if gun_config.bullet_prefab then
