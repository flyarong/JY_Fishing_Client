...
local ad_error ={
    [20001] = "恭喜您获得免广告特权免除该次广告，充值成为VIP1及以上可永久免广告哦！",
    [40007] = "广告获取失败，请重试。",
    [40020] = "恭喜您获得免广告特权免除该次广告，充值成为VIP1及以上可永久免广告哦！",
...
function M.csj_ad(key, call, cg_call)
    if not isOnOff then
        if call then
    ...
    DSM.ADShow(key)
    RewardADMgr.PrepareAD(ad_id, "金币", 3, MainModel.UserInfo.user_id, "extra", width, height, function(id, preRet)
...
function M.ylh_ad(key, call, cg_call)
    if AppDefine.IsEDITOR() then
        ad_key_map[key] = {play_time=os.time()}
        if call then
            call({result=0, isVerify=true})
    ...
    YLHRewardADMgr.YLHLoadRewardVideoAd(key)
end

-- 随机播放广告
function M.RandPlay(key, call, cg_call)
    local channel_type = gameMgr:getMarketPlatform()
    if advertising_config.platform[channel_type] then
        local func_name = advertising_config.platform[channel_type].."_ad"
        if M[func_name] then
            M[func_name](key, call, cg_call)
        end
    end
end
...
function M.IsCloseAD()
    local channel_type = gameMgr:getMarketPlatform()
    -- IOS VIVO 小米 没有广告 
    -- normal 因穿山甲广告政策调整，欢乐天天捕鱼暂时无法接入广告，需要对其中的广告系统进行关闭，解决方案年后来落实。 (2021/2/8)
    if gameRuntimePlatform == "Ios" or channel_type == "vivo" or channel_type == "xiaomi" then
        return true
    else
        return false
    end
