...
function PayPanel.Create(goodsType, isOpenType,finishCallback,hide_type)
    if GameGlobalOnOff.InternalTest then
        if not instance then
            if isOpenType and isOpenType == "normal" then
                instance = HintPanel.Create(1, "测试期间，暂不开放", function ()
                    instance = nil
                end)
            else
                instance = HintPanel.Create(1, "金币不足请联系客服", function ()
                    instance = nil
                end)
            end
        end
...
function PayPanel:ctor(goodsType, isOpenType, finishCallback, hide_type)
    ...
    Event.Brocast("global_game_panel_open_msg", {ui="PayPanel"})
end
...
function PayPanel:MyExit()
    ...
    Event.Brocast("global_game_panel_close_msg", {ui="PayPanel"})
end
...
function PayPanel:InitTge(type)
    ...
    GetTextureExtend(ui_table.icon_img, config.icon_image_unselect, config.is_local_icon)
...
function PayPanel:SwitchGroup(goodsType)
    -- 选中的按钮再次点击不执行刷新
    if self.goodsType and self.goodsType == goodsType then
        return
    end
    dump(goodsType, "<color=red>||| SwitchGroup</color>")
    ...
    if self.ext_pre then
        self.ext_pre:MyExit()
        self.ext_pre = nil
    end
    -- 扩展：炮台
    if goodsType == GOODS_TYPE.paotai then
        self.ext_pre = GameManager.GotoUI({gotoui="sys_by_bag", goto_scene_parm="panel_buy", parent=self.ext_node.transform})
    end
    -- 扩展：XXX
...
function PayPanel:CreateGoodsItemsToContent(goodsType, content)
    ...
    local tab_shop = MainModel.GetShopingConfig(GOODS_TYPE[goodsType])
    if tab_shop then
        for k, v in pairs(tab_shop) do
...
function PayPanel:CreateGoodsItem(goodsData, goodsType, parent)
    ...
    -- todo:商品加赠修改
    if goodsData.jz_title then
        goTable.give_img.gameObject:SetActive(true)
        goTable.givedesc_txt.text = goodsData.jz_title
    else
    ...
    -- doc = string.gsub(doc, "￥", "<size=50>￥</size>")
    ...
    if goodsData.tips then
        PointerEventListener.Get(goTable.pay_item_goods_btn.gameObject).onDown = function ()
            local pos = UnityEngine.Input.mousePosition
            GameTipsPrefab.ShowDesc(goodsData.tips, pos, GameTipsPrefab.TipsShowStyle.TSS_34)
        end
        PointerEventListener.Get(goTable.pay_item_goods_btn.gameObject).onUp = function ()
            GameTipsPrefab.Hide()
        end
    end
    ...
    goTable.pay_item_goods_btn.onClick:AddListener(
        function()
            ...
            elseif goodsType == GOODS_TYPE.jing_bi then
                ...
                --         HintPanel.Create(2, "是否确定消耗50.00福利券来兑换\n51万金币？", function ()
                        ...
                        if goodsData.goods_id == 1 or goodsData.goods_id == 101 then
                            ...
                            if a then
                                if b then
                                    ...
                                    LittleTips.Create("V0才可以进行1福卡兑换金币")
                                end
                            else
    ...
    if gift_status == 1 then
        ...
        if gift_data then
            if gift_data.random then
                ...
                goTable.doc_txt.text = "随机获得金币" 
            else
                -- todo:商品加赠修改
                if gift_data.jz_title then                    
                    goTable.givedesc_txt.text = gift_data.jz_title
                else
...
function PayPanel:SetZSConvertJB(goodsType)
    ...
    if IsEquals(go) then
        ...
        local d = goTable.diamIcon_img
        d.sprite = GetTexture("ty_icon_zs_6y")
        d.gameObject:SetActive(true)
    end
end
...
function PayPanel:ShopExpressionHintXYCJ()
    if GameGlobalOnOff.ShopExpressionHintXYCJ then
        HintPanel.Create(
            2,
            "该功能已下架，游戏主界面“幸运抽奖”功能可以产生大量红包，立即前往试试吧！",
            function()
                Network.SendRequest(
...
function PayPanel:CreateActGoodsItem(goodsData, goodsType, parent)
    ...
    if goodsData.id == 84 or goodsData.id == 10425 or goodsData.id == 10426 or goodsData.id == 10427 then
        goTable.act_tag_img.sprite = GetTexture("sc_icon_tjbm")
    end
    goTable.price_txt.text = "<size=45>￥</size>" .. goodsData.price / 100
    GetTextureExtend(goTable.icon_img, "ty_icon_jb_198y", 1)
    goTable.icon_img:SetNativeSize()
    goTable.pay_item_goods_btn.onClick:AddListener(
        function()
            ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function PayPanel:SetYHQHint(go, goods_data)
    ...
    -- dump("<color=white>1111111111233131321</color>")
    if gift_status == 1 then
        return
    end
