...
local config = HotUpdateConfig("Game.CommonPrefab.Lua.fish_activity_config")
ext_require ("Game.CommonPrefab.Lua.FishingActivityBullet")
ext_require ("Game.CommonPrefab.Lua.FishingActivityDCP")

if MainModel.myLocation == "game_Fishing3D" then
    ext_require("Game.game_Fishing3D.Lua.Fishing3DActivityBK")
    ext_require("Game.game_Fishing3D.Lua.FishingActivityZT3D")
    ext_require("Game.game_Fishing3D.Lua.FishingActivityXY")
else
    ext_require ("Game.normal_fishing_common.Lua.FishingActivityTime")
    ext_require("Game.normal_fishing_common.Lua.FishingActivityZT")
    ext_require("Game.normal_fishing_common.Lua.FishingActivityBK")
end
...
FISHING_ACTIVITY_ENUM = {
    ...
    power2_bullet = "power2_bullet",
    power3_bullet = "power3_bullet",
    crit2_bullet = "crit2_bullet",
}
...
function M.GetStatusAndInstance(data)
    ...
    if data and data.msg_type then
        ...
        elseif data.msg_type == FISHING_ACTIVITY_ENUM.free_bullet or
               ...
               data.msg_type == FISHING_ACTIVITY_ENUM.pierce_bullet or
               data.msg_type == FISHING_ACTIVITY_ENUM.power_bullet or
               data.msg_type == FISHING_ACTIVITY_ENUM.power2_bullet then
            a_status = FishingActivityBullet.CheckActivityStatus(data)
            a_instance = FishingActivityBullet
        elseif data.msg_type == FISHING_ACTIVITY_ENUM.crit2_bullet then
            a_status = FishingActivityXY.CheckActivityStatus(data)
            a_instance = FishingActivityXY
        elseif data.msg_type == FISHING_ACTIVITY_ENUM.shell_lottery then
            if #data.data == 4 then
                ...
                a_status = Fishing3DActivityBK.CheckActivityStatus(data)
                a_instance = Fishing3DActivityBK
            end
...
function M.CheckIsOnePlayerActiviey(msg_type)
    if msg_type == FISHING_ACTIVITY_ENUM.crit
        ...
        or msg_type == FISHING_ACTIVITY_ENUM.power2_bullet
        or msg_type == FISHING_ACTIVITY_ENUM.crit2_bullet
        then
        return true
    end
    return false
