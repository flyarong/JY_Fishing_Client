...
C.name = "FishingMatchQYSWaitPanel"
...
function C:MakeLister()
    self.lister = {}
    self.lister["fsqmg_cancel_signup_response"] = basefunc.handler(self, self.on_fsmg_cancel_signup)
    self.lister["model_status_msg"] = basefunc.handler(self, self.on_status_msg)
    self.lister["fsqmg_req_cur_player_num_response"] = basefunc.handler(self, self.on_signup_num_msg)
...
function C:UpdateQuerySignup()
	Network.SendRequest("fsqmg_req_cur_player_num")
end
...
function C:OnBackClick()
	HintPanel.Create(2, "比赛即将开始，现在离开可能错过比赛，请记得提前来参赛哦！", function ()
		Network.SendRequest("fsqmg_cancel_signup", nil, "取消报名")
	end)
end
