...
HintUnlockPanel = basefunc.class()
...
function HintUnlockPanel.Create(type,msg,confirmCbk,cancelCbk,parent)
    return HintUnlockPanel.New(type,msg,confirmCbk,cancelCbk,parent)
end

--[[错误提示板
    直接提供错误编号即可
]]
function HintUnlockPanel.ErrorMsg(errorID,callback,parent)
    local msg
    if errorID then
        if errorID == 0 then
            return
    ...
    return HintUnlockPanel.New(1,msg,callback,nil,parent)
end


function HintUnlockPanel:ctor(type,msg,confirmCbk,cancelCbk,parent)
    log("<color=yellow>--->>>Create HintUnlockPanel. type:" .. type .. ", msg:" .. (msg or "") .. "</color>")
    self.type = type
    self.msg = msg
...
function HintUnlockPanel:SetButtonText(btn1)
    if btn1 then
...
-- 参考大厅提示效果
function HintUnlockPanel:InitUIHall()
    self.UIEntity = newObject("HintUnlockPanel", self.parent.transform)
    LuaHelper.GeneratingVar(self.UIEntity.transform, self)

    self.text = self.UIEntity.transform:Find("hint_info/hint_info_txt"):GetComponent("Text")
...
function HintUnlockPanel:ShowGou()
    self.is_gou = true
    self.gourect.gameObject:SetActive(true)
    self:SetGou()
end

function HintUnlockPanel:SetGou()
    if self.is_gou then
...
function HintUnlockPanel:SetDesc(desc)
