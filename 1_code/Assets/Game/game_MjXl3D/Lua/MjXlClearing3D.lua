...
MjXlClearing = basefunc.class()

MjXlClearing.name = "MjXlClearing3D"

local instance
function MjXlClearing.Create(parent)
    SysInteractivePlayerManager.Close()
    SysInteractiveChatManager.Hide()

	if not instance then
		instance = MjXlClearing.New(parent)
	else
		instance:MyRefresh()
	end
	return instance
...
function MjXlClearing.isGameOverStatus()
	return MjXlModel.data.model_status == MjXlModel.Model_Status.gameover
end

-- 关闭
function MjXlClearing:Close()
...
function MjXlClearing:AddMsgListener(lister)
    for proto_name,func in pairs(lister) do
        Event.AddListener(proto_name, func)
    end
end

function MjXlClearing:MakeLister()
    self.lister = {}
...
function MjXlClearing:RemoveListener()
    for proto_name,func in pairs(self.lister) do
        Event.RemoveListener(proto_name, func)
    end
end
function MjXlClearing:MyRefresh()
	self:SetBackAndConfirmBtn()
	self:ShowMyself()
end

function MjXlClearing:ctor(parent)
	self.gameExitTime = os.time()

	local parent = parent or GameObject.Find("Canvas/LayerLv2").transform
	self:MakeLister()
	self:AddMsgListener(self.lister)
	local obj = newObject(MjXlClearing.name, parent)
	local tran = obj.transform
	self.transform = tran
	LuaHelper.GeneratingVar(self.transform, self)

    self.GameExitTimeText = tran:Find("mask/GameExitTimeText"):GetComponent("Text")
    self.GameExitTimeText.text = os.date("%Y.%m.%d %H:%M:%S", self.gameExitTime)
	self.fengding = tran:Find("mask/fengding")
	self.mask = tran:Find("mask"):GetComponent("Image")
	...
	self.BackButton.onClick:AddListener(function ()
		...
		--self:OnBackClick()
		OperatorActivityLogic.CanLeaveGameBeforeEnd(true, basefunc.handler(self, self.OnBackClick), true)
	end)
...
function MjXlClearing:ShowMyself()
	SysInteractivePlayerManager.Close()
    SysInteractiveChatManager.Hide()
	if self.delayShow then
		self.delayShow:Stop()
...
function MjXlClearing:InitRect()
	if MjXlModel.data.glory_score_count then
		local v1 = MjXlModel.data.glory_score_count
		local v2 = MjXlModel.data.glory_score_change
		--GameHonorModel.UpdateHonorValue(v1)
	end
	self:SetBackAndConfirmBtn()
	local room_rent = MjXlModel.baseData.room_rent
	if room_rent then
		self.RoomRentTxt.text = "服务费：" .. room_rent.asset_count .. AwardManager.GetAwardName(room_rent.asset_type)
	end
	
	if MjXlModel.data.score_change_list then
		dump(MjXlModel.data.score_change_list, "<color=yellow>==============================================>>> MjXlModel.data.score_change_list:</color>")
		self:GetMyScoreChange(MjXlModel.data.score_change_list)
	else
		log("<color=yellow>---------------------------------------->>>> No score change list!</color>")
	end

	local clearData = MjXlModel.data.settlement_info
	...
	local cfg = GameFreeModel.GetGameIDToConfig(MjXlModel.baseData.game_id)
...
function MjXlClearing:InitOthers(i, data)
	...
	local base = MjXlModel.data.game_players_info[seatno]
	...
	--- 飘的多少倍
	if MjXlModel.daPiao then
		local piaoNum = MjXlModel.data.playerInfo[seatno].piaoNum
		if piaoNum and (piaoNum == 0 or piaoNum == 1 or piaoNum == 3 or piaoNum == 5 ) then
			
			if piaoNum > 0 then
				rect.piaoIcon.gameObject:SetActive(true)
				--rect.piaoIcon.sprite = GetTexture( MjXlModel.piaoIconVec[piaoNum] )
			else
		...
		--- 总共有几个人飘
		--[[local piaoPlayerNum = MjXlModel.GetDaPiaoPlayerNum()
		...
		if piaoPlayerNum == 1 then
			rect.daPiaoStakeText.text = string.format("%d人飘底分%d",piaoPlayerNum,2*MjXlModel.data.init_stake)
		elseif piaoPlayerNum >= 2 then
			rect.daPiaoStakeText.text = string.format("%d人飘底分%d",piaoPlayerNum,4*MjXlModel.data.init_stake)
		end]]
	else
		rect.piaoIcon.gameObject:SetActive(false)
		--rect.daPiaoStakeText.gameObject:SetActive(false)
	...
	--[[if data.settle_data.settle_type == "hu" then
		...
		if data.settle_data.hu_pai then
			MjCard.Create(rect.HPRect, MjXlModel.PaiType.hu, data.settle_data.hu_pai)
	...
	--[[if data.shou_pai then
		...
		for i,v in ipairs(data.shou_pai) do
			MjCard.Create(rect.SPRect, MjXlModel.PaiType.sp, v)
		end
	end]]
	--[[if data.pg_pai then
...
function MjXlClearing:CalcMyResult(data)
	local seatno = data.seat_num
	if MjXlModel.GetRealPlayerSeat () == seatno then
		self.huindex = 1
		...
		if data.settle_data.settle_type == "hu" then
		    local ui_config = GameFreeModel.GetGameIDToConfig(MjXlModel.baseData.game_id)
		...
		self.my_score = score
		if not OperatorActivityLogic.IsBigUI() then
			self:OnShareClick()
		end
		...
		ExtendSoundManager.PlaySound(isWin and audio_config.ddz.sod_game_win.audio_name or audio_config.ddz.sod_game_lose.audio_name)
		...
		if isWin and MjXlModel.data.ls_count > 1 and OperatorActivityModel.IsActivated(MjXlModel.baseData.game_id, ActivityType.Consecutive_Win) and ActivityConsecutiveWin.CanBeAwarded() then
			self.WinNode:SetActive(false)
			self.count_txt.text = MjXlModel.data.ls_count
...
function MjXlClearing:CreateDetailItem(col1, col2, col3, col4, isBroke)
...
function MjXlClearing.GetGangName(op)
	return (op == "wg" and "刮风" or "下雨")
end

function MjXlClearing.GetGangPlusCount(op)
	return (op == "wg" and 1 or 2)
end

function MjXlClearing.GetAffectedPlayerPos(mySeat, affected)
	local p = {}
	if #MjXlModel.data.settlement_info == 2 then
		p[1] = "对家"
	else
...
function MjXlClearing:ShowDetailsAndPlayers(isShow)
	...
	self.mask.transform.localPosition = self.mask.transform.localPosition + Vector3.New(0, 130, 0) * (isShow and -1 or 1)
	self.mask.enabled = not isShow
	self.ReadyButton.transform.localPosition = self.ReadyButton.transform.localPosition + Vector3.New(0, 170, 0) * (isShow and -1 or 1)
end

function MjXlClearing:GetPlayerScore(seatno)
	local myScore = 0
	for _, info in ipairs(MjXlModel.data.game_players_info) do
...
function MjXlClearing:InitBPF(data, bpf)
	dump(data, "<color=yellow>--->>>MjXlClearing:InitBPF</color>")
	local yingfengding = MjXlModel.data.yingfengding
	local sData = data.settle_data
	local msg = ""
	local show = false
	local showMsg = false
	local iconImg = "ddz_settlement_icon_bp"
	local cfg = GameFreeModel.GetGameIDToConfig(MjXlModel.baseData.game_id)
	dump(cfg, "<color=yellow>game id:" .. MjXlModel.baseData.game_id .. ", config</color>")
	...
	if MjXlModel.data.game_bankrupt then
...
function MjXlClearing:IsCanShare()
	if GameGlobalOnOff.ShowOff and self.my_score > 0 and (self.pai_type or self.hu_type) then
		return true
	end
	return false
end
function MjXlClearing:OnOff()
end

function MjXlClearing:GetFanShu(data)
	local fan = 0
...
function MjXlClearing:GetHuPaiType(settle_data,pg_pai)
	local huPaiType = ""
	local hu_type_str = ""
	local dai_geng_str = ""
	local gang_str = ""
...
function MjXlClearing:OnChangedeskClick()
    MjXlModel.HZCheck()
end

-- 准备
function MjXlClearing:OnReadyClick()
	MjXlModel.ZBCheck()
end

-- 返回
function MjXlClearing:OnBackClick()
...
function MjXlClearing:OnShareClick()
	if not self:IsCanShare() then return end
	MJSharePrefab.Create(MjXlModel.baseData.game_id, {time=self.gameExitTime, my_score = self.my_score, multi = self.multi,
		paiType=self.pai_type, huType=self.hu_type, pai=self.my_shou_pai, geng=self.geng, huindex=self.huindex})
end
function MjXlClearing:on_fg_ready_response_code(result)
	if result == 0 then
		self:Close()
	end
end
function MjXlClearing:on_fg_huanzhuo_response_code(result)
	if result == 0 then
		self:Close()
	end
end

function MjXlClearing:on_model_fg_gameover_msg()
...
function MjXlClearing:SetBackAndConfirmBtn()
	local is_game_over = MjXlClearing.isGameOverStatus()
    if is_game_over then
		--红包
		GameManager.GotoUI({gotoui = "sys_total_red",goto_scene_parm = "panel",transform = self.transform,game_id = MjXlModel.baseData.game_id})
    end
...
function MjXlClearing:TryFixStuck()
...
function MjXlClearing:StopFixStuck()
...
function MjXlClearing:OnSeeCardBtnClicked()
	self.ShowCard = not self.ShowCard
	self:ShowDetailsAndPlayers(not self.ShowCard)
end

function MjXlClearing:OnMissionBtnClicked()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function MjXlClearing:CheckShow1YuanGift(call)
	if GameGlobalOnOff.Shop_10_gift_bag ~= nil and GameGlobalOnOff.Shop_10_gift_bag == false then
		if call then call() end
		return
	end

	log("<color=blue>-------------------------------------->>> My seat:" .. MjXlModel.data.seat_num .. "</color>")
	local brokeUp = false
	local myScore = self:GetPlayerScore(MjXlModel.data.seat_num)
...
function MjXlClearing:GetHuTypeShort(settleData, pg_pai)
...
function MjXlClearing:GetMyScoreChange(dataList)
	local index = 1
	local mySeat = MjXlModel.data.seat_num
	self.scoreDetails = {}
...
function MjXlClearing:InitDetailList(data)
	dump(self.scoreDetails, "<color=green>---------------------------------->>>> score details:</color>")
	dump(data, "<color=green>===============================================>>> settle data:</color>")
	dump(MjXlModel.data.settlement_info, "<color=green>===============================================>>> settle info:</color>")

	if self.scoreDetails and #self.scoreDetails > 0 then
		local gameCfg = GameFreeModel.GetGameIDToConfig(MjXlModel.baseData.game_id)
		local seatno = data.seat_num
		local settleInfo = MjXlModel.data.settlement_info
		local details = self.scoreDetails
		local mySeat = MjXlModel.data.seat_num
		local piaoNum = 0

		if MjXlModel.data.playerInfo then
			for i = 1, #MjXlModel.data.playerInfo do
				if MjXlModel.data.playerInfo[i].piaoNum and MjXlModel.data.playerInfo[i].piaoNum > 0 then
					piaoNum = piaoNum + MjXlModel.data.playerInfo[i].piaoNum
				end
			end
		...
		for n = #details, 1, -1 do
			...
			if opType ~= "settle" then
				...
				elseif opType == "gang" then
					content[1] = (details[n].score < 0 and "被" or "" ) .. MjXlClearing.GetGangName(op)
					content[2] = MjXlClearing.GetGangPlusCount(op)
						...
						for _, seatId in ipairs(details[n].affected) do
							...
							for _, info in ipairs(settleInfo) do
								if info.seat_num == seatId then
									...
									self:CreateDetailItem(content[1], content[2], (details[n].score < 0 and "-" or "+") .. StringHelper.ToCash(math.abs(details[n].getScore)), MjXlClearing.GetAffectedPlayerPos(seatno, {seatId}), isBroke)
									break
								end
							end
						end
				...
				if opType == "gang" or (opType == "hu" and details[n].score > 0) or op == "zhuan_yu" then
					self:CreateDetailItem(content[1], content[2], (details[n].score < 0 and "-" or "+") .. StringHelper.ToCash(math.abs(details[n].getScore)), MjXlClearing.GetAffectedPlayerPos(seatno, details[n].affected), details[n].score ~= details[n].getScore)
				end
			else
				if op == "cj" then
				...
				for _, d in ipairs(details[n].data) do
					...
					if (details[n].score > 0 and rs < 0) or (details[n].score < 0 and rs > 0) then
						...
						self:CreateDetailItem(content[1], content[2], (rs > 0 and "-" or "+") .. StringHelper.ToCash(math.abs(d.score)), MjXlClearing.GetAffectedPlayerPos(seatno, {d.cur_p}), d.lose_surplus ~= 0)
					end
				end
			end
		end
...
function MjXlClearing:FormatNum(num)
...
function MjXlClearing:game_share()
	self:OnShareClick()
end