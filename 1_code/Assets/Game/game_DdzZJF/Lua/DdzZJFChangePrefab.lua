...
DdzZJFChangePrefab = basefunc.class()
local C = DdzZJFChangePrefab
C.name = "DdzZJFChangePrefab"
local df_tb = {}
local jrtj_tb = {}
local fz_pay = 200
local aa_pay = 50
...
function C:SetUI()
	local beishu = {"feng_ding_32b","feng_ding_64b","feng_ding_128b","feng_ding_256b"}
	self.tx_tge.isOn = DdzFKModel.get_ori_game_cfg_byOption("yingfengding") == 1
	self.jrtj_slider.value = self:getJrtj_ui_value()
	self.df_slider.value = self:getDf_ui_value()
	for i = 1,4 do 
		if DdzFKModel.get_ori_game_cfg_byOption(beishu[i]) == 1 then 
			self["bs"..i.."_tge"].isOn = true
		end
	end
	self.fwq1_tge.isOn = DdzFKModel.IsFZPaY()
end


function C:getJrtj_ui_value()
	local  v = DdzFKModel.get_ori_game_cfg_byOption("enter_limit")
	for i = 1,#jrtj_tb do 
		if jrtj_tb[i] == v then  
			return i
		end
...
function C:getDf_ui_value()
	local v = DdzFKModel.get_ori_game_cfg_byOption("init_stake")
	for i = 1,#df_tb do 
