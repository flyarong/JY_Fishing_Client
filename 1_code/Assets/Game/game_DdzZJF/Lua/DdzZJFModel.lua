...
function DdzFKModel.on_nor_ddz_nor_action_msg(proto_name, data)
    ...
    if m_data.base_rate and act_type == 100 and data.action.rate > m_data.base_rate then
        ...
        m_data.my_rate = data.action.rate*m_data.init_rate
    end
...
function DdzFKModel.on_friendgame_all_info(proto_name, data)
    ...
    if data.status_no==-1 then
        ...
        --初始化算法库
        m_data.ddz_algorithm=nor_ddz_algorithm_lib.New(m_data.translate_config.kaiguan, "nor_ddz_nor")
...
function DdzFKModel.on_friendgame_quit_msg(proto_name, data)
    ...
    if data.seat_num == m_data.seat_num then
        ...
        -- dump(m_data.playerInfo,"<color=red>222222222222222222222</color>")
        -- dump(DdzFKModel.data.player_ready,"<color=red>333333333333333333333</color>")
        -- dump(m_data.playerInfo[seatno].base,"444444444444444444444444444444")
...
function DdzFKModel.zijianfang_begin_vote_alter_rule_msg(_,data)
    ...
    if not DdzFKModel.IsFZ() then
        DdzZJFRuleChangeNoticePrefab.Create(data,function ()
            Network.SendRequest("zijianfang_player_vote_alter_rule",{opt = 1},"",function(data)
        ...
        function ()
            Network.SendRequest("zijianfang_player_vote_alter_rule",{opt = 0},"",function ()
                Network.SendRequest(
