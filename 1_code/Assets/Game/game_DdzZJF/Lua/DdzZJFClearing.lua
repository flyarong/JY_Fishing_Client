...
DdzFKClearing.name = "DdzZJFClearing"
...
function DdzFKClearing:ctor(parent)
	...
	self.TopBackButton = tran:Find("TopBackButton"):GetComponent("Button")
	self.TopBackButton.onClick:AddListener(function ()
		self:OnBackClick()
	end)
	self.ChangeButton = tran:Find("ChangeButton"):GetComponent("Button")
	self.ChangeButton.onClick:AddListener(function ()
		self:OnChangeClick()
	end)
...
function DdzFKClearing:InitRect()
	...
	for i=1,3 do
		...
		self.playerInfo[i].CurScore.text = sData.award[pSeat] >= 0 and "+"..sData.award[pSeat] or sData.award[pSeat]
		...
		self.playerInfo[i].AllScore.text = pData[pSeat].base.score >= 0 and pData[pSeat].base.score or pData[pSeat].base.score
	end
	self:UpdateGameOver()
...
function DdzFKClearing:OnConfirmClick()
	if DdzFKModel.data.vote_result and DdzFKModel.data.vote_result == 0 then
		DdzFKClearing.Close()
		DdzFKModel.on_js_exit()
	else
		DdzFKClearing.Close()
		DdzFKModel.on_js_xyj()
	end
end
function DdzFKClearing:UpdateGameOver()
	if DdzFKModel.data.status == DdzFKModel.Status.settlement then
		self.ConfirmButton.gameObject:SetActive(false)
		self.TopBackButton.gameObject:SetActive(false)
		...
		self.TopBackButton.gameObject:SetActive(true)
		if DdzFKModel.data.vote_result and DdzFKModel.data.vote_result == 0 then
			self.ChangeButton.gameObject:SetActive(false)
		else
...
function DdzFKClearing:OnBackClick()
	if DdzFKModel.data.vote_result and DdzFKModel.data.vote_result == 0 then
		DdzFKClearing.Close()
		DdzFKModel.on_js_exit()
	else
		Network.SendRequest("zijianfang_exit_room", nil, "", function (data)
...
function DdzFKClearing:OnChangeClick()
	DdzZJFChangePrefab.Create()
end
