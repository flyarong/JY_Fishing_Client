...
DdzZJFGamePanel = basefunc.class()

DdzZJFGamePanel.name = "DdzZJFGamePanel"
...
local instance
function DdzZJFGamePanel.Create()
    instance=DdzZJFGamePanel.New()
    ...
    return createPanel(instance,DdzZJFGamePanel.name)
end
function DdzZJFGamePanel.Bind()
    local _in=instance
    instance=nil
    return _in
end

function DdzZJFGamePanel:Awake()
            ...
            if self:IsJSFun() then
                ...
                if m_data and m_data.model_status == DdzFKModel.Model_Status.gaming then
                    local hint =
                        ...
                        function()
                            --发起申请解散房间
                            Network.SendRequest("zijianfang_begin_vote_cancel_room", nil, function (data)
...
function DdzZJFGamePanel:OnEasyClick(obj)
    local uipos = tonumber(string.sub(obj.name,-1,-1))
    local data = DdzFKModel.GetPosToPlayer(uipos)
    if data and data.base then
        SysInteractivePlayerManager.Create(data.base, uipos)
    else
        dump(data, "<color=red>玩家没有入座</color>")
    end
end
function DdzZJFGamePanel:OnExitClick()
    if DdzFKModel.GetReadyStatusByID(MainModel.UserInfo.user_id) then 
        HintPanel.Create(1,"取消准备后才能退出哦!")
        return
    end
    Network.SendRequest(
        ...
        function(data)
            if data.result == 0 then
                ...
                MainLogic.ExitGame()
                GameManager.GotoSceneName("game_ZJF")
            else
                HintPanel.ErrorMsg(data.result)
            end
        end
...
function DdzZJFGamePanel:OnBeginClick()
    -- ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function DdzZJFGamePanel:OnCopyClick()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function DdzZJFGamePanel:OnShareClick()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
    -- local title
    -- local description
    -- local strOff = "false"
    ...
    -- 分享链接
    local shareLink = string.format(share_link_config.share_link[5].link[1], 
        DdzFKModel.data.friendgame_room_no,
        self.RuleText.text,
        MainLogic.GetPTDeeplinkKeyword(),
        DdzFKModel.data.friendgame_room_no,
        MainModel.UserInfo.user_id)
    -- local shareLink =
    --     '{"type": 3, "title": "' ..
    --     title ..
    --         '", "description": "' .. description .. '", "url": "' .. url .. '", "isCircleOfFriends": ' .. strOff .. "}"
    ShareLogic.ShareGM(
        shareLink,
        function(str)
...
function DdzZJFGamePanel:OnClickGPSButton()
    DdzFKModel.RefreshGPS(true)
end
function DdzZJFGamePanel:OnVoiceDown()
    self.begPos = UnityEngine.Input.mousePosition
    GameVoicePanel.RecordVoice()
end
function DdzZJFGamePanel:OnVoiceUp()
...
function DdzZJFGamePanel:SetHideMenu()
    self.MenuBG.gameObject:SetActive(false)
    self.TopButtonImage.gameObject:SetActive(false)
end
function DdzZJFGamePanel:update_callback()
    local dt=1
...
function DdzZJFGamePanel:RefreshClock()
    local flag=nil
...
function DdzZJFGamePanel:Start()
    -- self:MyRefresh()
end

function DdzZJFGamePanel:MyInit()
    ...
    self.behaviour:AddClick(self.back_btn.gameObject, DdzZJFGamePanel.OnClickCloseSignup, self)
    self.behaviour:AddClick(self.select_laizi_son.select_laizi_btn_bg.gameObject, DdzZJFGamePanel.OnClickCloseSelectLaizi, self);
...
function DdzZJFGamePanel:RefreshBackBtn()
    local m_data = DdzFKModel.data
...
function DdzZJFGamePanel:OnClickCloseSignup()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
    Network.SendRequest("nor_ddz_nor_cancel_signup",{})
end

function DdzZJFGamePanel:MyRefresh()
    if DdzFKModel.data then
        ...
        self.ready_txt = self.transform:Find("ready_btn/ready_txt"):GetComponent("Text")
        self.ready_btn.onClick:RemoveAllListeners()
        if  m_data.model_status == DdzFKModel.Model_Status.wait_join or m_data.model_status == DdzFKModel.Model_Status.wait_begin  then
            self.ready_btn.gameObject:SetActive(true)
            if MainModel.UserInfo.user_id == m_data.room_owner  then 
                self.ready_txt.text = "开始比赛"
                self.ready_btn.onClick:AddListener(
                    function ()
                        if DdzFKModel.IsAllReady() then 
                            Network.SendRequest("zijianfang_begin_game")
                        else
                            HintPanel.Create(1,"请等待所有玩家准备完成")
                        end 
                    end
                )
            else
                ...
                self.ready_btn.onClick:AddListener(
                    function ()
                        local data = DdzFKModel.GetReadyStatusByID(MainModel.UserInfo.user_id)
                        ...
                        Network.SendRequest("zijianfang_ready",{opt = DdzFKModel.GetReadyStatusByID(MainModel.UserInfo.user_id) and 0 or 1},"")
                    end
                )
            end
        else
...
-- 是否是房主或者游戏中
function DdzZJFGamePanel:IsJSFun()
    local m_data = DdzFKModel.data
    if
...
function DdzZJFGamePanel:RefreshMenu()
    local wanfa = {
        nor_ddz_nor  = "三带一对、四带两对、三张必带、叫加倍",
        nor_ddz_lz  = "三带一对、四带两对、三张必带、叫加倍",
        nor_ddz_boom   = "三带一对、四带两对、三张必带、叫加倍",
        nor_ddz_er   = "三带一对、四带两对、三张必带、叫加倍",
    }
    if DdzFKModel.GetCurrBeiShu() then 
        self.RuleText.text = "封顶倍数:"..DdzFKModel.GetCurrBeiShu().."  "..wanfa[DdzFKModel.data.game_type]
    end 
end
-- 刷新中间区域(方位)
function DdzZJFGamePanel:RefreshCenter()
    local data = DdzFKModel.data
    if data then
        if (data.model_status == DdzFKModel.Model_Status.wait_join or data.model_status == DdzFKModel.Model_Status.wait_begin)
            and DdzFKModel.IsFZ(data.seat_num) then
            if GameGlobalOnOff.InviteFriends then
                if  DdzFKModel.data.password == 1 then 
...
function DdzZJFGamePanel:SetRate()
...
function DdzZJFGamePanel:RefreshDdzJiPaiQi()
    --自动显示记牌器
...
function DdzZJFGamePanel:AutoShowDdzJiPaiQiCB()
...
--记牌器
function DdzZJFGamePanel:DdzJiPaiQiCB()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function DdzZJFGamePanel:DdzPayJiPaiQiCB()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
-- 刷新游戏结算界面
--todo
function DdzZJFGamePanel:RefreshClearing()
...
function DdzZJFGamePanel:RefreshRate()
...
function  DdzZJFGamePanel:RefreshScore(p_seat)
    if DdzFKModel.data then
    	local m_data = DdzFKModel.data
        if p_seat then
...
function DdzZJFGamePanel:RefreshPlayerReadyStatus()
    local call = function (seat)
        local cSeatNum = DdzFKModel.data.s2cSeatNum[seat]
        if self.playerInfoUI then
            if DdzFKModel.data.model_status == DdzFKModel.Model_Status.wait_join
                or DdzFKModel.data.model_status == DdzFKModel.Model_Status.wait_begin
                or DdzFKModel.data.model_status == DdzFKModel.Model_Status.gameover
            then
                self.playerInfoUI[cSeatNum].HandImage.gameObject:SetActive(DdzFKModel.GetReadyStatusBySeatNum(seat)) 
            else
                self.playerInfoUI[cSeatNum].HandImage.gameObject:SetActive(false) 
            end
        end
    end
    for i = 1,3 do 
        local cSeatNum = DdzFKModel.data.s2cSeatNum[i]
        call(cSeatNum)
    end
...
function DdzZJFGamePanel:ShowOrHideWarningView(status)
    for i=1,3 do
        --刷新warning
...
function DdzZJFGamePanel:ShowOrHideDdzView(status)
...
function DdzZJFGamePanel:ShowOrHidePermitUI(status,people)   
    if people==2 then
...
function DdzZJFGamePanel:ShowOrHideActionUI(status,people)    
    if people==2 then
...
function DdzZJFGamePanel:MyExit()
    ...
    GPSPanel.Close()
    --closePanel(DdzZJFGamePanel.name)
    self.dzCardObj = nil
    self.cardObj = nil
    self.lzSelectCardObj = nil
end

function DdzZJFGamePanel:MyClose()
    self:MyExit()
    closePanel(DdzZJFGamePanel.name)
end

function DdzZJFGamePanel:RefreshAutoStatus(pSeatNum)
    --根据状态显示和隐藏面板
    if DdzFKModel.data then
        ...
        if auto and DdzFKModel.data.seatNum then
            ...
            if not pSeatNum then
                for i=1,3 do
                    if  auto[DdzFKModel.data.seatNum[i]]==1 and DdzFKModel.data.model_status ~= DdzFKModel.Model_Status.gameover  then
                         --显示
                        if i == 1 then
                            self.DdzFKPlayersActionManger:ChangeClickStatus(1)
                        end
            ...
            --刷新单个人
                ...
                if auto[DdzFKModel.data.seatNum[pSeatNum]]==1 and DdzFKModel.data.model_status ~= DdzFKModel.Model_Status.gameover then
                  --显示
                    if pSeatNum == 1 then
                        self.DdzFKPlayersActionManger:ChangeClickStatus(1)
                    end
...
--刷新剩余的牌 和warning 
function DdzZJFGamePanel:RefreshRemainPaiWarningStatus(pSeatNum)
    --根据状态显示和隐藏面板
    if DdzFKModel.data then
        ...
        if remain_pai_amount and DdzFKModel.data.seatNum then
           ...
           for i = 1, DdzFKModel.maxPlayerNumber do
                if self.warningUI[i] then
                    self.warningUI[i]:SetActive(false)
                end
                if self.cardsRemainUI[i] then
...
function DdzZJFGamePanel:RefreshRemainPaiWarningStatusWithAni(pSeatNum,act_type,pai_count)
...
function DdzZJFGamePanel:CanelAutoBtnCB()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
-- 刷新简易交互UI
function DdzZJFGamePanel:RefreshEasyChat()
...
function DdzZJFGamePanel:RefreshPlayerInfo(pSeatNum)
    dump(pSeatNum,"<color=red>333</color>")
    dump( DdzFKModel.data,"<color=red>333</color>")
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        ...
        local RefreshPlayerAllInfo=function (pSeatNum)
            dump(DdzFKModel.data,"<color=red>信息-------</color>")
            ...
            for i=1,3 do
                ...
                if DdzFKModel.data.seatNum[i] == dizhu then
                    if not SpineManager.GetSpine(i) then
                        spine = newObject("@spine_dz_nan",self.playerInfoUI[i].spine_node.transform):GetComponent("SkeletonAnimation")
                    ...
                    if not SpineManager.GetSpine(i) then
                        spine = newObject("@spine_nm_nan",self.playerInfoUI[i].spine_node.transform):GetComponent("SkeletonAnimation")
                        SpineManager.AddDDZPlayerSpine(spine,i)
                        if i == 1 then
                            SetSortingOrder(i,-1)
                        end  
...
function DdzZJFGamePanel:SetFZ(FZImage, seatno)
...
-- 设置离线状态
function DdzZJFGamePanel:SetDX(DXImage, HeadIcon, net_quality)
    -- 离线状态
...
function DdzZJFGamePanel:SetDXSpine(DXImage, HeadIcon, net_quality)
...
-- 设置分数
function DdzZJFGamePanel:SetScore(ScoreText, score)
    if score >= 0 then
...
function DdzZJFGamePanel:RefreshPermitStatus()
    --隐藏所有权限
...
function DdzZJFGamePanel:RefreshClockPos(parent)
...
--刷新结算
function DdzZJFGamePanel:RefreshSettlement()
    if DdzFKModel.data and DdzFKModel.data.seat_num  then
        local data = DdzFKModel.data
...
--刷新地主UI显示
function DdzZJFGamePanel:RefreshDiZhuAndMultipleStatus()
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        local data = DdzFKModel.data
...
function DdzZJFGamePanel:GetLaizi()
	local data = DdzFKModel.data
	if not data or not data.laizi then return 0 end
	return data.laizi
end

function DdzZJFGamePanel:GetDizhu()
	local data = DdzFKModel.data
...
--刷新赖子牌显示
function DdzZJFGamePanel:RefreshLaiziStatus(playAnim)
...
function DdzZJFGamePanel:RefreshLaiziDepend(laizi)
    --刷新牌
	self.DdzFKPlayersActionManger:Refresh()
...
function DdzZJFGamePanel:LaiziInDizhu()
	local laizi = self:GetLaizi()
...
function DdzZJFGamePanel:FillSelectLaizi(btn_ident, btn_table, pai_list, container, callback)
...
function DdzZJFGamePanel:ShowSelectLaiziType(pai_list_table, callback)
...
function DdzZJFGamePanel:ResetSelectLaiziType(btn_table)
...
function DdzZJFGamePanel:OnClickCloseSelectLaizi()
	self:ResetSelectLaiziType(self.selectLaiziBtnTable)
	self:RefreshPermitStatus()
end


function DdzZJFGamePanel:ShowOrHideCustHeadIcon(status,seatNum)
    -- if not seatNum then
    --     for i=1,3 do
...
function DdzZJFGamePanel:ShowOrHideHeadInfo(status,seatNum)
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        if not seatNum then
            for i=1,3 do
                self.playerInfoUI[i].cust_head_icon_img.gameObject:SetActive(status)
...
function DdzZJFGamePanel:ShowOrHidePlayerInfo(status,seatNum)
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        if not seatNum then
            for i=1,3 do
...
-- 刷新游戏总结算界面
function DdzZJFGamePanel:RefreshGameOver()
    ...
    DdzFKModel.data.status==DdzFKModel.Status.gameover then
        RoomCardGameOver.Create(
            ...
            true,
            MainModel.UserInfo.user_id == DdzFKModel.data.room_owner
        )
    else
        RoomCardGameOver.Close()
    end
...
function DdzZJFGamePanel:MakeLister()
    lister={}
    ...
    lister["model_xyj_msg"] = basefunc.handler(self, self.MyRefresh)
end

-- 发起投票
function DdzZJFGamePanel:model_begin_vote_cancel_room_response()
    -- RoomCardDissolve.Create(parm)
end

-- 玩家投票返回
function DdzZJFGamePanel:model_player_vote_cancel_room_response()
end

-- 投票开始
function DdzZJFGamePanel:model_friendgame_begin_vote_cancel_room_msg()
    if DdzFKModel.data and DdzFKModel.data.vote_parm then
        RoomCardDissolve.Create(DdzFKModel.data.cur_race, DdzFKModel.data.room_rent, DdzFKModel.data.vote_data.begin_player_id, DdzFKModel.data.vote_parm, true)
    end
end

-- 投票结束
function DdzZJFGamePanel:model_friendgame_over_vote_cancel_room_msg()
...
--gps
function DdzZJFGamePanel:model_query_gps_info_msg(isTrustDistance)
    self.GPSHintImg.gameObject:SetActive(isTrustDistance)
end

function DdzZJFGamePanel:RefreshVoteStatus()
    --根据状态显示和隐藏面板
    RoomCardDissolve.MyExit()
    if DdzFKModel.data and DdzFKModel.data.vote_data and DdzFKModel.data.vote_parm then
        local parm = DdzFKModel.data.vote_parm
        RoomCardDissolve.Create(DdzFKModel.data.cur_race, DdzFKModel.data.room_rent, DdzFKModel.data.vote_data.begin_player_id, parm,true)
    end
end

--刷新房间是否退出
function DdzZJFGamePanel:RefreshRoomDissolveStatus()
...
-- 离线状态变化
function DdzZJFGamePanel:model_friendgame_net_quality(pSeatNum)
    local uiPos = DdzFKModel.data.s2cSeatNum[pSeatNum]
    self:RefreshPlayerInfo(uiPos)
end
--开始游戏
function DdzZJFGamePanel:model_nor_ddz_nor_begin_msg()
    self:MyRefresh()
end
--准备
function DdzZJFGamePanel:model_nor_ddz_nor_ready_msg(pSeatNum)
    print("<color=red>准备 pSeatNum " .. pSeatNum .. "</color>")
    local uiPos = DdzFKModel.data.s2cSeatNum[pSeatNum]
    self:RefreshPlayerInfo(uiPos)
end
function DdzZJFGamePanel:model_nor_ddz_nor_settlement_msg()
    self:RefreshSettlement()
...
function DdzZJFGamePanel:on_nor_ddz_nor_enter_room_msg()
    print("[DDZ LaiZi] GamePanel on_nor_ddz_nor_enter_room_msg") 
    self:MyRefresh()
end

-- 分数改变
function DdzZJFGamePanel:on_nor_ddz_nor_score_change_msg()
end
-- 玩家进入
function DdzZJFGamePanel:model_friendgame_join_msg(seatno)
    print("<color=red>玩家进入 model_friendgame_join_msg</color>")
    self:RefreshPlayerInfo(DdzFKModel.data.s2cSeatNum[seatno])
end
-- 玩家退出
function DdzZJFGamePanel:model_friendgame_quit_msg(seatno)
    dump(seatno,"<color=red>2222</color>")
    dump(DdzFKModel.data.s2cSeatNum,"44444444444444444444444444")
    self:RefreshPlayerInfo(DdzFKModel.data.s2cSeatNum[seatno])
end
-- 总结算
function DdzZJFGamePanel:friendgame_gameover_msg(pSeatNum)
    DdzFKClearing.SetGameOver()
    DdzFKModel.InitReadyStatus()
    --self:RefreshGameOver()
end

function DdzZJFGamePanel:friendgame_gameover_msg_com()
    DdzFKClearing.SetGameOver()
end

function DdzZJFGamePanel:on_nor_ddz_nor_pai_msg()
...
function DdzZJFGamePanel:on_nor_ddz_nor_action_msg()
...
function DdzZJFGamePanel:on_nor_ddz_nor_jiabeifinshani_msg()
...
function DdzZJFGamePanel:on_nor_ddz_nor_permit_msg()
...
function DdzZJFGamePanel:on_nor_ddz_nor_dizhu_msg()
    if DdzFKModel.data then
        local data = DdzFKModel.data
...
function DdzZJFGamePanel:on_nor_ddz_nor_laizi_msg()
    self:RefreshLaiziStatus(true)
end

function DdzZJFGamePanel:on_nor_ddz_nor_auto_msg(player)
...
function DdzZJFGamePanel:on_nor_ddz_nor_new_game_msg()
    self:MyRefresh()
    --新的局数
    if DdzFKModel.data then
...
function DdzZJFGamePanel:on_nor_ddz_nor_start_again_msg()
    self:MyRefresh()
    DDZAnimation.StartAgainCard(self.start_again_cards_pos)
end

function DdzZJFGamePanel:on_nor_ddz_nor_game_clearing_msg()
    self:RefreshSettlement()
...
function DdzZJFGamePanel:model_zjf_player_ready_info_change(player_id)
    print("<color=red>准备刷新UI ---------</color>")
    dump(DdzFKModel.data.s2cSeatNum)
    self:RefreshPlayerReadyStatus()
...
function DdzZJFGamePanel:AotoExitGame()
...
function DdzZJFGamePanel:StopAotoExitGame()
...
function DdzZJFGamePanel:RefreshOutTime()
    --刷新自己
    if DdzFKModel.data.model_status == DdzFKModel.Model_Status.wait_join
        or DdzFKModel.data.model_status == DdzFKModel.Model_Status.wait_begin then
            if DdzFKModel.GetReadyStatusByID(MainModel.UserInfo.user_id) or DdzFKModel.IsFZ() then 
                self:StopAotoExitGame()
            else
                self:AotoExitGame()
            end
