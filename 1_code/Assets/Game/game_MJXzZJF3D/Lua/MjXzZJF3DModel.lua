...
function MjXzFKModel.on_friendgame_quit_msg(proto_name, data)
    ...
    if data.seat_num == m_data.seat_num then
        ...
        MainLogic.ExitGame()
        MjXzFKLogic.change_panel(DdzFKLogic.panelNameMap.hall)
    end
...
function MjXzFKModel.on_friendgame_all_info(proto_name, data)
    ...
    if data.status_no==-1 then
        ...
        --初始化算法库
        mj_algorithm=nor_mj_base_lib.New(m_data.translate_config.kaiguan,m_data.translate_config.multi , m_data.game_type)
...
function MjXzFKModel.GetPlayerSeat ()
    if not m_data.seat_num then
        error("test")
    end
    --print("<color=yellow>----------------- GetPlayerSeat:</color>",m_data.seat_num)
    return m_data.seat_num
end
-- 返回自己的UI位置
...
function MjXzFKModel.GetSeatnoToPos (seatno)
    if seatno then
        ...
        return (seatno - seftSeatno + MjXzFKModel.data.player_count) % MjXzFKModel.data.player_count + 1
    else
        return m_data.seat_num
    end
end
...
function MjXzFKModel.GetPosToSeatno (uiPos)
    local seftSeatno = MjXzFKModel.GetPlayerSeat()
    return (uiPos - 1 + seftSeatno - 1) % MjXzFKModel.data.player_count + 1
end
