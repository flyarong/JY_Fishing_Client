...
local panelNameMap = {
    ...
    wait = "MatchComWaitStartPanel",
    wait_rematch = "MatchComWaitRematchPanel",
    game = "DdzPDKMatchGamePanel",
    rank = "MatchComRankPanel"
}
local cur_loc
local cur_panel
local delayShowRank
...
function DdzPDKMatchLogic.change_panel(panelName,enterSceneCall)
    ...
    if not cur_panel then
        if panelName == panelNameMap.hall then
            ...
            local match_type_id = GameMatchModel.GetCurMatchType()
            local parm = {match_type_id = match_type_id}
            GameManager.GotoSceneID(GameConfigToSceneCfg.game_Match.ID, parm,nil,enterSceneCall)
        elseif panelName == panelNameMap.wait then
            cur_panel = {name = panelName, instance = MatchComWaitStartPanel.Create({model = DdzPDKMatchModel,logic = DdzPDKMatchLogic,ani = DDZAnimation})}
        elseif panelName == panelNameMap.wait_rematch then
            cur_panel = {name = panelName, instance = MatchComWaitRematchPanel.Create({model = DdzPDKMatchModel,logic = DdzPDKMatchLogic,ani = DDZAnimation})}
        ...
        elseif panelName == panelNameMap.rank then
            ...
            cur_panel = {name = panelName, instance = MatchComRankPanel.Create(parm, function (game_id)
...
function DdzPDKMatchLogic.on_nor_mg_match_discard_msg(data)
    local cur_config = GameMatchModel.GetGameIDToConfig()
...
function DdzPDKMatchLogic.on_nor_mg_all_info()
    ...
    elseif DdzPDKMatchModel.data.model_status == DdzPDKMatchModel.Model_Status.wait_begin then
        --等待开始界面
        if GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_DdzPDKMatch then
            go_to = panelNameMap.wait
        elseif GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_DdzMatchNaming then
...
function DdzPDKMatchLogic.Init(isNotSendAllInfo)
    ...
    if GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_DdzMatch then
