...
DdzPDKMatchActionUiManger = basefunc.class()


function DdzPDKMatchActionUiManger.Create(gamePanel,playerOperateUI,dizhuCardUI)
	local instance=DdzPDKMatchActionUiManger.New()
...
function DdzPDKMatchActionUiManger:Refresh()
	--全部隐藏
	...
	if DdzPDKMatchModel.data then
		...
		local hash={}
		if act_list and #act_list>0 and DdzPDKMatchModel.data.s2cSeatNum then
			local len= #act_list
			local len_ = len
			len = len -1
			for i=len_,len,-1 do
				if i>0 then
					...
					if not hash[act.p] then
						if not m_data.cur_p or (act.p~=m_data.cur_p and m_data.cur_p~=6) or m_data.status==nor_pdk_base_lib.get_action_status(act) then
							hash[act.p]=true
							self:RefreshAction(DdzPDKMatchModel.data.s2cSeatNum[act.p],act)
						end
					end
				else
					break
...
--根据当前状态调整动作的UI的显示
function DdzPDKMatchActionUiManger:changeActionUIShowByStatus()
	--如果上一个已经是不同类型
	local m_data=DdzPDKMatchModel.data
	if m_data then
		if m_data.status==DdzPDKMatchModel.Status.cp then
			local cur_p=m_data.cur_p
			if cur_p and DdzPDKMatchModel.data.s2cSeatNum then 
			 	local other_p={}
			 	other_p[1]=DdzPDKMatchModel.data.s2cSeatNum[cur_p]
			 	for i=2,DdzPDKMatchModel.maxPlayerNumber do
			 		other_p[i]=other_p[i-1]-1
			 		if other_p[i]==0 then
			 			other_p[i]=DdzPDKMatchModel.maxPlayerNumber
			 		end
			 	end
...
function DdzPDKMatchActionUiManger:RefreshAction(seatNum,act)
	--清空之前残留
	...
	if act.type>99 or act.type==0 then
		...
		DdzPDKMatchModel.ddz_algorithm:get_cpInfo_by_action(act)
		...
		if DdzPDKMatchModel.data then
			if DdzPDKMatchModel.data.seatNum[seatNum] == DdzPDKMatchModel.data.dizhu then
...
--带动画和音效刷新
function DdzPDKMatchActionUiManger:RefreshActionWithAni(seatNum,act)
	...
	if act.type>99 or act.type==0 then
		...
		elseif act.type==102 then
			if act.rate==0 then-- 不抢地主
				...
				sound="sod_game_rob"
				DdzPDKMatchModel.data.er_qiang_dizhu_count = DdzPDKMatchModel.data.er_qiang_dizhu_count + 1
				local qtext = self.playerOperateUI[1].qiang_hint_txt
				if IsEquals(qtext) then
					qtext.text = "抢" .. DdzPDKMatchModel.data.er_qiang_dizhu_count .. "次"
				end
			end
		elseif act.type==110 then
			--闷抓
			sound="sod_game_menzhua"
			local my_rate=  act.my_rate or DdzPDKMatchModel.data.my_rate
		...
		sound=sound..AudioBySex(DdzPDKMatchModel, DdzPDKMatchModel.data.seatNum[seatNum])
		ExtendSoundManager.PlaySound(audio_config.ddz[sound].audio_name)
	else
		local sound=nor_pdk_base_lib.get_paiType_sound(act.type,act.pai)
		sound=sound..AudioBySex(DdzPDKMatchModel, DdzPDKMatchModel.data.seatNum[seatNum])
			...
			if idx==#self.showCardList[seatNum] then
					...
					if  DdzPDKMatchModel.data.seatNum[seatNum] == DdzPDKMatchModel.data.dizhu then
						for i=1,DdzPDKMatchModel.maxPlayerNumber do
							if i ~= seatNum then
								...
								ParticleManager.DDZBeiZha(i)
							end
						end
					else
						for i=1,DdzPDKMatchModel.maxPlayerNumber do
							if i ~= seatNum then
								if DdzPDKMatchModel.data.seatNum[i] == DdzPDKMatchModel.data.dizhu then
									SpineManager.BeiZha(i)
									ParticleManager.DDZBeiZha(i)
								else
				...
				DDZAnimation.ShowChupaiCard(card,function ()
					ExtendSoundManager.PlaySound(audio_config.ddz[sound].audio_name)
						...
						--播放动画
						if act.type==6 then
							ParticleManager.DDZShunZi(self.playerOperateUI[seatNum].cards_out_ani_pos)
						elseif act.type==7 then
							ParticleManager.DDZLianDui(self.playerOperateUI[seatNum].cards_out_ani_pos)
						elseif act.type==11 or act.type==10 or act.type==12 or act.type== 32 then
							DDZAnimation.plain_animation()
							ParticleManager.DDZFeiJi(self.playerOperateUI[seatNum].cards_out_ani_pos)
...
function DdzPDKMatchActionUiManger:PlayBombBGM()
...
function DdzPDKMatchActionUiManger:ClearCardList()
end

function DdzPDKMatchActionUiManger:MyExit()
...
--刷新背景音
function DdzPDKMatchActionUiManger:RefreshBGM(isCoerce)
    if not self.bombbgmKey and DdzPDKMatchModel.data and DdzPDKMatchModel.data.total_round and DdzPDKMatchModel.data.round_info and DdzPDKMatchModel.data.round_info.round then
        if DdzPDKMatchModel.data.total_round == DdzPDKMatchModel.data.round_info.round then
            --决赛背景音
            ExtendSoundManager.PlaySceneBGM(audio_config.game.bgm_bisai_juesai.audio_name,isCoerce)
        else
            ExtendSoundManager.PlaySceneBGM(audio_config.game.bgm_bisai_bisaizhong.audio_name,isCoerce)
        end
    end
end

function DdzPDKMatchActionUiManger:PlayBSKS()
	local data = DdzPDKMatchModel.data
    if not self.is_bsks and data and data.round_info and data.round_info.round and data.round_info.round == 1 then
		self.is_bsks = ExtendSoundManager.PlaySound(audio_config.game.bgm_bisai_kaishi.audio_name)
    end
end