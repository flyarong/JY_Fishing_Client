...
package.loaded["Game.game_Login.Lua.LoginWXTSManager"] = nil
require "Game.game_Login.Lua.LoginWXTSManager"
    ...
    local function callback(json_data)
        ...
        if lua_tbl.result == 0 then
            ...
            if platform ~= "normal" then
                ...
                Network.SendRequest("get_wechat_bind_list", {device_id = loginModel.loginData.device_id, channel_args=loginData.channel_args}, "", function (data)
                    dump(data, "<color=red>|||||||||||||||||||||||||||||||||</color>")
                    if data.result == 0 then
                        loginData.login_id = data.login_id
                        lua_tbl.refresh_token = data.refresh_token
                        loginData.channel_args = lua2json(lua_tbl)

                        if data.bind_datas ~= nil and #data.bind_datas > 0 then
                            FullSceneJH.RemoveAll()
                            status = 0
                            LoginWXTSPanel.Create(data.bind_datas, function (d)
                                dump(d, "select wx item")

                                FullSceneJH.Create("正在登陆...", 1)

                                loginData.old_channel_type = d.channel_type
                                loginData.old_login_id = d.login_id
                                lua_tbl.sms_vcode = d.code
                                loginData.channel_args = lua2json(lua_tbl)

                                dump(loginData, "selected loginData")
...
function LoginLogic.login_wechat()
    wechatTokenToLogin()
end
...
local function login()
    ...
    elseif curChannel == "wechat" then
        ...
        if curLoginId and appid ~= "" and curToken and curToken ~= "" then
            local tbl = {}
            tbl.appid = appid
