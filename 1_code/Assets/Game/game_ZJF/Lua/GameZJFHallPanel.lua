		...
		for i, id in ipairs(v.ids) do
			for _, g in ipairs(gameCfg) do
				if g.id == id then
					...
					if game_type and game_type == g.game_type then
						defaultIndex = k
						self.defSubIndex = i
					end
					break
						...
						if data.table_list[i].password == 0 then 
							Network.SendRequest("zijianfang_join_room",{
								...
								if _data.result == 0 then
									GameManager.GotoSceneName(GameZJFModel.game_type2scence[data.game_type] , false)
								else
...
function GameZJFHallPanel:CallEnter(game_id)
	...
	local xsyd_signup = function ()
        ...
        GameManager.GotoSceneID( GameZJFModel.data.sceneID , true , nil , function()
    ...
    check = function ()
		...
		if ss == 1 then
			if GameGlobalOnOff.Shop_10_gift_bag ~= nil and GameGlobalOnOff.Shop_10_gift_bag == false then
				local dd = GameZJFModel.GetGameIDToConfig(game_id)
