...
GameZJFRightItemPrefab = basefunc.class()

local C = GameZJFRightItemPrefab

C.name = "GameZJFRightItemPrefab"
...
function C:MyRefresh()
    ...
    self.Base_txt.font = GetFont("pp_" .. self.config.imageIndex .. "_1")
...
function C:PlayAnim(t)
    ...
    self.openseq = DoTweenSequence.Create()
        ...
        if not self.selectIndex then
            ...
            if self.selectIndex ~= selectIndex then
                if selectIndex == self.index then
                    ...
                    self.selectseq = DoTweenSequence.Create()
