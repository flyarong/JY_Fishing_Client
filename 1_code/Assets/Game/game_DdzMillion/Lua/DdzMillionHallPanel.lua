...
function DdzMillionHallPanel:OnBackClick( go )
    ...
    if match_type_id then
        local scene_name = GameConfigToSceneCfg.game_Match.SceneName
...
function DdzMillionHallPanel:OnApplyClick( go )
	...
	Network.SendRequest("dbwg_query_shared_status", nil, "", function (data)
		if data.result == 0 then
			if data.status == 0 then
				SharePanel.Create("million", "apply", function ()
					self:ShareFinishCall()
				end)
			else
				self:CallApply()
			end
...
function DdzMillionHallPanel:ShareFinishCall()
	Network.SendRequest("dbwg_shared_finish", nil, nil, function (data)
		if data.result == 0 then
...
function DdzMillionHallPanel:OnInviteFriendClick( go )
	SharePanel.Create("million", "invite")
end
