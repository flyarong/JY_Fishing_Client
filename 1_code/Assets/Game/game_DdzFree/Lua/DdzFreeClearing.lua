        ...
        function()
            ...
            if state == "Install" or state == "Update" then
                ...
                if Network.SendRequest("fg_quit_game", nil, "") then
                    MainLogic.ExitGame()
                    MainLogic.GotoScene("game_Match")
                end
            end
        end
    )
...
function DdzFreeClearing:UpdateExchange()
    ...
    if is_game_over and DdzFreeModel.data.exchange_hongbao and DdzFreeModel.data.exchange_hongbao.is_exchanged == 0 then
        ...
        self.ExChongbao.text = StringHelper.ToRedNum(DdzFreeModel.data.exchange_hongbao.hong_bao )
        self.ext_model = DdzFreeModel
        Event.Brocast("global_sysqx_uichange_msg", {key="dh", panelSelf=self})
    else
...
function DdzFreeClearing:OnExchangeClick()
    ...
    if not iss then
        local rr = StringHelper.ToRedNum(DdzFreeModel.data.exchange_hongbao.hong_bao )
...
function DdzFreeClearing:InitRect()
    ...
    if OperatorActivityModel.IsActivated(DdzFreeModel.baseData.game_id, ActivityType.Consecutive_Win) then
...
function DdzFreeClearing:OnRefreshActivityData()
    if self.isWin and DdzFreeModel.data.ls_count > 1 and OperatorActivityModel.IsActivated(DdzFreeModel.baseData.game_id, ActivityType.Consecutive_Win) and ActivityConsecutiveWin.CanBeAwarded() then
        self.shengli.gameObject:SetActive(false)
