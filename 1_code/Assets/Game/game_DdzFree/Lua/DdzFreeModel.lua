...
local FreeAwardConfig = require "Game.game_DdzFree.Lua.ddz_drivingrange_award" --奖励配置

DdzFreeModel = {}
DdzFreeModel.maxPlayerNumber = 3
DdzFreeModel.game_type = {
    nor = "nor_ddz_nor",
    lz = "nor_ddz_lz",
    er = "nor_ddz_er"
}

DdzFreeModel.jdz_type = {
    nor = "nor",
    mld = "mld"
}

DdzFreeModel.Model_Status = {
    --等待分配桌子，疯狂匹配中
...
--注册斗地主正常逻辑的消息事件
function DdzFreeModel.AddMsgListener()
...
--删除斗地主正常逻辑的消息事件
function DdzFreeModel.RemoveMsgListener()
    for proto_name, _ in pairs(lister) do
        if proto_name == "AssetChange" then
...
local function InitMatchData(gameID)
    if not DdzFreeModel.baseData then
        DdzFreeModel.baseData = {}
    end
    DdzFreeModel.data = {
        --游戏名
        name = nil,
...
local function get_my_jdz_permit_data()
    if m_data and DdzFreeModel.baseData.jdz_type == DdzFreeModel.jdz_type.mld then
        if m_data.status== DdzFreeModel.Status.jdz and m_data.cur_p==m_data.seat_num then
            ...
            if m_data.men_data[m_data.seat_num]==0 then
                ...
                --检查我是否必抓
                -- if not nor_ddz_base_lib.is_must_zhua( m_data.my_pai_list) then
                --     m_data.jdz_permit_data.buzhua=true
                -- end
                dump(m_data.is_must_mld_opt, "<color=yellow>m_data.is_must_mld_opt</color>")
...
local function get_my_jb_permit_data()
    if m_data and DdzFreeModel.baseData.jdz_type == DdzFreeModel.jdz_type.mld then
        if m_data.status== DdzFreeModel.Status.jiabei and (m_data.cur_p==m_data.seat_num or (m_data.cur_p==4 and m_data.dizhu~=m_data.seat_num )) then
            --看自己是否是地主
            if m_data.dizhu==m_data.seat_num then
                ...
                if m_data.zhua_data[m_data.seat_num]==0 then
                    ...
                    --检查我是否必倒  或者 地主是闷
                    -- if m_data.men_data[m_data.dizhu]==2 or not nor_ddz_base_lib.is_must_dao(m_data.my_pai_list) then
...
function DdzFreeModel.InitGameData()
    if not m_data then
...
function DdzFreeModel.Init()
    InitMatchData()
    this = DdzFreeModel
    this.InitUIConfig()
    MakeLister()
    this.AddMsgListener()
    update = Timer.New(DdzFreeModel.Update, updateDt, -1, true)
    update:Start()
    return this
end

function DdzFreeModel.Exit()
    if this then
        DdzFreeModel.RemoveMsgListener()
        update:Stop()
        update = nil
        this = nil
        lister = nil
        m_data = nil
        DdzFreeModel.data = nil
    end
end

function DdzFreeModel.InitUIConfig()
    this.UIConfig = {
        award = {},
    }
...
--********************response
--1.比赛报名结果
function DdzFreeModel.on_fg_signup_response(_, data)
    dump(data, "<color=yellow>on_fg_signup_response</color>")
    if data.result == 0 then
        DdzFreeModel.InitGameData()
        m_data.model_status = DdzFreeModel.Model_Status.wait_table
        m_data.status = nil
        m_data.players_info = {}

        m_data.countdown = data.cancel_signup_cd
        m_data.game_model = data.game_model
        DdzFreeModel.baseData.game_type = data.game_type
        MainLogic.EnterGame()
        Event.Brocast("model_fg_signup_response", data.result)
        Event.Brocast("activity_fg_signup_msg")
    end
end
function DdzFreeModel.on_fg_switch_game_response(proto_name, data)
    DdzFreeModel.on_fg_signup_response(proto_name, data)
end

function DdzFreeModel.on_fg_cancel_signup_response(_, data)
    dump(data, "<color=yellow>on_fg_cancel_signup_response</color>")
    if data.result == 0 then
        --清除数据
        InitMatchData()
        MainLogic.ExitGame()
...
--退出游戏
function DdzFreeModel.on_fg_quit_game_response(proto_name, data)
    dump(data, "<color=yellow>on_fg_quit_game_response</color>")
    if data.result == 0 then
        --清除数据
        local game_id = DdzFreeModel.baseData.game_id
        ...
        if ui_config then
            ...
            dump(game_id, "<color=red>DdzFreeModel config</color>")
        end
    else
        HintPanel.ErrorMsg(data.result)
    end
...
--换桌
function DdzFreeModel.on_fg_huanzhuo_response(proto_name, data)
...
-- 准备
function DdzFreeModel.on_fg_ready_response(_, data)
    dump(data, "<color=yellow>on_fg_ready_response</color>")
    Event.Brocast("fg_ready_response_code", data.result)
    if data.result == 0 then
        DdzFreeModel.InitGameData()
        m_data.model_status = DdzFreeModel.Model_Status.wait_begin
        m_data.status = nil
...
--再玩一把
function DdzFreeModel.on_fg_replay_game_response(proto_name, data)
...
--***********************DDZFree
--所有数据
function DdzFreeModel.on_fg_all_info(proto_name, data)
    ...
    if data.status_no == -1 then
        ...
        if s then
            ...
            DdzFreeModel.baseData.game_type = s.game_type
            DdzFreeModel.baseData.jdz_type = s.jdz_type
            m_data.countdown = s.countdown

            DdzFreeModel.data = DdzFreeModel.data and DdzFreeModel.data or {}
            --转化成算法需要的配置数据
            if s.jdz_type == "mld" then
                DdzFreeModel.translate_config = {kaiguan = nor_ddz_base_lib.KAIGUAN_MLD, multi = {}}
            else
                DdzFreeModel.translate_config = {kaiguan = nor_ddz_base_lib.KAIGUAN, multi = {}}
            end
            nor_ddz_base_lib.set_game_type(DdzFreeModel.baseData.game_type)
            --初始化算法库
            DdzFreeModel.ddz_algorithm =
                nor_ddz_algorithm_lib.New(DdzFreeModel.translate_config.kaiguan, DdzFreeModel.baseData.game_type)
            
            DdzFreeModel.SetMaxPlayerNum(DdzFreeModel.baseData.game_type)
        end
        ...
        if s then
            ...
            DdzFreeModel.baseData.game_id = s.game_id
        end
        ...
        nor_ddz_base_lib.transform_seat(
            ...
            m_data.s2cSeatNum,
            m_data.seat_num,
            DdzFreeModel.maxPlayerNumber
        )

        DdzFreeModel.baseData.room_rent = data.room_rent
        ...
        if data.activity_data then
            ...
            DdzFreeModel.GetLSCount(data.activity_data)
        else
    ...
    if m_data then
        Event.Brocast("activity_fg_all_info",{activity_data = m_data.activity_data,game_type = DdzFreeModel.baseData.game_type, game_id = DdzFreeModel.baseData.game_id,model_status = m_data.model_status,status = m_data.status})
        if OperatorActivityModel.IsActivated(DdzFreeModel.baseData.game_id, ActivityType.Consecutive_Win) and m_data.model_status == DdzFreeModel.Model_Status.gameover then
...
--进入房间
function DdzFreeModel.on_fg_enter_room_msg(proto_name, data)
    dump(data, "<color=yellow>on_fg_enter_room_msg</color>")
    m_data.model_status = DdzFreeModel.Model_Status.gaming
    m_data.status = DdzFreeModel.Status.wait_join
    ...
    m_data.seatNum = {}
    m_data.s2cSeatNum = {}
    nor_ddz_base_lib.transform_seat(m_data.seatNum, m_data.s2cSeatNum, m_data.seat_num, DdzFreeModel.maxPlayerNumber)
...
--其他玩家进入游戏
function DdzFreeModel.on_fg_join_msg(proto_name, data)
...
--其他玩家离开游戏
function DdzFreeModel.on_fg_leave_msg(proto_name, data)
...
--分数改变
function DdzFreeModel.on_fg_score_change_msg(proto_name, data)
...
--比赛结束
function DdzFreeModel.on_fg_gameover_msg(proto_name, data)
    dump(data, "<color=red>比赛结束</color>")
    m_data.model_status = DdzFreeModel.Model_Status.gameover
    m_data.status = DdzFreeModel.Status.gameover    
...
function DdzFreeModel.on_fg_auto_cancel_signup_msg(proto_name, data)
    InitMatchData()
    MainLogic.ExitGame()
...
---- 自动退出游戏报名
function DdzFreeModel.on_fg_auto_quit_game_msg(proto_name, data)
...
---- 准备
function DdzFreeModel.on_fg_ready_msg(proto_name, data)
...
---- 活动数据更新
function DdzFreeModel.on_fg_activity_data_msg(proto_name, data)
    dump(data, "<color=yellow>------------on_fg_activity_data_msg--------------</color>")
    if data.activity_data then
        m_data.activity_data = data.activity_data
        DdzFreeModel.GetLSCount(data.activity_data)
    else
...
--***************************nor
--状态信息
function DdzFreeModel.on_nor_ddz_nor_status_info(proto_name, data)
...
function DdzFreeModel.on_nor_ddz_nor_ready_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_nor_ready_msg</color>")
end

function DdzFreeModel.on_nor_ddz_nor_begin_msg(proto_name, data)
...
--发牌
function DdzFreeModel.on_nor_ddz_nor_pai_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_nor_pai_msg</color>")
    m_data.status = DdzFreeModel.Status.fp
...
function DdzFreeModel.on_nor_ddz_mld_kan_my_pai_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_mld_kan_my_pai_msg</color>")
    m_data.my_pai_list=data.my_pai_list
    table.sort(m_data.my_pai_list)
...
--确认地主
function DdzFreeModel.on_nor_ddz_nor_dizhu_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_nor_dizhu_msg</color>")
    m_data.status = DdzFreeModel.Status.set_dz
    ...
    if DdzFreeModel.baseData.jdz_type == DdzFreeModel.jdz_type.nor then
        ...
        if DdzFreeModel.baseData.game_type == DdzFreeModel.game_type.er then
            m_data.my_rate = m_data.init_rate + DdzFreeModel.data.er_qiang_dizhu_count
        end
        --初始化记牌器
...
--确认地主牌 闷拉倒
function DdzFreeModel.on_nor_ddz_mld_dizhu_pai_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_mld_dizhu_pai_msg</color>")
    m_data.status = DdzFreeModel.Status.set_dz
...
--赖子
function DdzFreeModel.on_nor_ddz_nor_laizi_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_nor_laizi_msg</color>")
    m_data.laizi = data.laizi
    Event.Brocast("model_nor_ddz_nor_laizi_msg", data.p)
end

--权限信息轮询
function DdzFreeModel.on_nor_ddz_nor_permit_msg(proto_name, data)
    ...
    if DdzFreeModel.baseData.jdz_type == DdzFreeModel.jdz_type.mld then
         --other : 0 不是必须 1 必须
        m_data.is_must_mld_opt = data.other
        get_my_jdz_permit_data()
        get_my_jb_permit_data()
    end
...
function DdzFreeModel.on_nor_ddz_nor_action_msg(proto_name, data)
...
--托管
function DdzFreeModel.on_nor_ddz_nor_auto_msg(proto_name, data)
...
--结算
function DdzFreeModel.on_nor_ddz_nor_settlement_msg(proto_name, data)
    dump(data, "<color=yellow>on_nor_ddz_nor_settlement_msg</color>")
    m_data.status = DdzFreeModel.Status.settlement
    ...
    -- }
    -- Event.Brocast("fg_activity_data_msg","fg_activity_data_msg",m_data)
    DdzFreeModel.grand_total_settlement()
end

--打完一局重新发牌
function DdzFreeModel.on_nor_ddz_nor_new_game_msg(proto_name, data)
...
--都没有叫地主重新开始
function DdzFreeModel.on_nor_ddz_nor_start_again_msg(proto_name, data)
    dump(data, "<color=red>on_nor_ddz_nor_start_again_msg</color>")
    --考虑是否需要清除数据
    InitMatchStatusData(data.status)
...
--资产改变
function DdzFreeModel.AssetChange(proto_name, data)
...
--*******************************方法
--判断玩家是否胜利
function DdzFreeModel.PlayerIsWin(seat_num)
...
--]]
function DdzFreeModel.getMyPermitData()
    if m_data then
        if m_data.cur_p and m_data.cur_p == m_data.seat_num then
            if m_data.status == DdzFreeModel.Status.jdz then
                return {type = DdzFreeModel.Status.jdz, jdz_min = m_data.base_rate + 1}
            elseif m_data.status == DdzFreeModel.Status.jiabei then
                return {type = DdzFreeModel.Status.jiabei, is_jiabei = m_data.jiabei}
            elseif m_data.status == DdzFreeModel.Status.q_dizhu then
                return {type = DdzFreeModel.Status.q_dizhu}
            elseif m_data.status == DdzFreeModel.Status.cp then
                --判断是否为必须出牌
                ...
                if not is_must then
                    power =
                        DdzFreeModel.ddz_algorithm:check_cp_capacity_by_pailist(
                        m_data.action_list,
                        m_data.my_pai_list,
                        m_data.laizi
                    )
                end
                return {type = DdzFreeModel.Status.cp, is_must = is_must, power = power}
            end
        end
    end
    return nil
...
function DdzFreeModel.getNextAward(winCount)
...
-- 公共倍数 就是地主叫的分数
function DdzFreeModel.GetGongGongBeishu()
    if m_data and m_data.settlement_info then
...
-- 地主倍数 地主加的倍数
function DdzFreeModel.GetDiZhuBeishu()
    if m_data and m_data.settlement_info then
...
-- 农民倍数 农民加的倍数
function DdzFreeModel.GetNongMinBeishu()
    if m_data and m_data.settlement_info then
        local d = 0
...
-- 额外倍数
function DdzFreeModel.GetEWaiBeishu()
    if m_data and m_data.settlement_info then
        local d = 0
...
-- 炸弹
function DdzFreeModel.GetZhadanBeishu()
    if m_data and m_data.settlement_info then
...
-- 春天
function DdzFreeModel.GetCTBeishu()
    if m_data and m_data.settlement_info then
...
-- 总倍数
function DdzFreeModel.GetZongBeishu()
    local beishu = {}
    local rr = 1
    local isB = false
    beishu[#beishu + 1] = DdzFreeModel.GetGongGongBeishu()
    beishu[#beishu + 1] = DdzFreeModel.GetEWaiBeishu()
    beishu[#beishu + 1] = DdzFreeModel.GetZhadanBeishu()
    beishu[#beishu + 1] = DdzFreeModel.GetCTBeishu()
...
-- 判断是否能进入
function DdzFreeModel.IsRoomEnter(id)
...
-- 返回自己的座位号
function DdzFreeModel.GetPlayerSeat()
    if m_data.seat_num then
...
-- 返回自己的UI位置
function DdzFreeModel.GetPlayerUIPos()
    return DdzFreeModel.GetSeatnoToPos(m_data.seat_num)
end

-- 根据座位号获取玩家UI位置
function DdzFreeModel.GetSeatnoToPos(seatno)
    local seftSeatno = DdzFreeModel.GetPlayerSeat()
    return (seatno - seftSeatno + DdzFreeModel.maxPlayerNumber) % DdzFreeModel.maxPlayerNumber + 1
end

-- 根据UI位置获取玩家座位号
function DdzFreeModel.GetPosToSeatno(uiPos)
    local seftSeatno = DdzFreeModel.GetPlayerSeat()
    return (uiPos - 1 + seftSeatno - 1) % DdzFreeModel.maxPlayerNumber + 1
end

-- 根据UI位置获取玩家座位号
function DdzFreeModel.GetPosToPlayer(uiPos)
    local seatno = DdzFreeModel.GetPosToSeatno(uiPos)
    return m_data.players_info[seatno]
end

-- 是否是自己 玩家自己的UI位置在1号位
function DdzFreeModel.IsPlayerSelf(uiPos)
    return uiPos == 1
end

-- 根据玩家ID返回动画显示的位置
-- 交互动画必须要这个方法
function DdzFreeModel.GetAnimChatShowPos(id)
    if m_data and m_data.players_info then
        for k, v in ipairs(m_data.players_info) do
            if v.id == id then
                local uiPos = DdzFreeModel.GetSeatnoToPos(v.seat_num)
                if DdzFreeModel.data.dizhu and DdzFreeModel.data.dizhu > 0 then
                    return uiPos, true
                else
                    return uiPos, false
                end
...
function DdzFreeModel.ClearMatchData(gameID)
    InitMatchData(gameID)
end

function DdzFreeModel.SetMaxPlayerNum(game_type)
    if game_type == DdzFreeModel.game_type.er then
        DdzFreeModel.maxPlayerNumber = 2
    else
        DdzFreeModel.maxPlayerNumber = 3
    end
end
...
-- 返回农民的座位号
function DdzFreeModel.GetSeatNM()
...
--闷拉倒---------------------------------------
--地主是否是闷抓
function DdzFreeModel.IsMenZhua()
...
--倒的人数
function DdzFreeModel.GetDaoNum()
    local num = 0
...
function DdzFreeModel.GetSettlementRateShowData()
    local base_rate=1
    DdzFreeModel.settlementRateShowData={}
    local data=DdzFreeModel.settlementRateShowData
    --[[
    zhua_pai --抓牌的倍率
    dao=
    la=
...
function DdzFreeModel.CalcBetMultipliers()
    local data = {}
    local farmerNum = 2
...
function DdzFreeModel.hz_call()
    Network.SendRequest("fg_huanzhuo", nil, "请求换桌")
end
function DdzFreeModel.zb_call()
    Network.SendRequest("fg_ready", nil, "请求准备")
end
function DdzFreeModel.hintCondition(call)
    local game_id = DdzFreeModel.baseData.game_id
    local ui_config = GameFreeModel.GetGameIDToConfig(game_id)
    PayFastFreePanel.Create(ui_config, call)
end
function DdzFreeModel.checkCondition(call)
    local game_id = DdzFreeModel.baseData.game_id
    local ss = GameFreeModel.IsAgainRoomEnter(game_id)
    if ss == 1 then
        DdzFreeModel.hintCondition(call)
        return false
    elseif ss == 2 then
...
-- 换桌检查
function DdzFreeModel.HZCheck()
    if DdzFreeModel.checkCondition(DdzFreeModel.hz_call) then
        DdzFreeModel.hz_call()
    end
end
-- 准备检查
function DdzFreeModel.ZBCheck()
    if DdzFreeModel.checkCondition(DdzFreeModel.zb_call) then
        DdzFreeModel.zb_call()
    end
end

-- 检查一个玩家是否是地主
function DdzFreeModel.CheckDZ(seat)
...
--获取结算时所有玩家数据
function DdzFreeModel.GetPlayersData()
...
function DdzFreeModel.GetLSCount(activity_data)
    local aid = 0
    local lsc = 1
...
--[[累计胜负统计]]
function DdzFreeModel.grand_total_settlement(  )
    if DdzFreeModel.IsMyWin()then
