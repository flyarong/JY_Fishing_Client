...
function M:MakeLister()
    ...
    self.lister["qhb_signup_response"] = basefunc.handler(self, self.on_qhb_signup_response)
...
function M:InitUI()
    self.hby_node.gameObject:SetActive(false)
    self.jbp.gameObject:SetActive(false)
    ...
    for i=1,#self.ui_config do
        ...
        temp_ui.bg_btn.onClick:AddListener(function ()
            ...
            Network.SendRequest("qhb_signup", {id = self.ui_config[i].game_id}, "正在报名")
        end)
...
function M:on_qhb_signup_response(_,data)
    dump(data, "<color=white>qhb_signup_response</color>")
    if data.result ~= 0 then
        HintPanel.ErrorMsg(data.result)
        return
    end
    M.Close()
    GameManager.GotoUI({gotoui = "qhb"})
end
