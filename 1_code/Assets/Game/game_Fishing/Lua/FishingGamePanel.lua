...
function C:ReadyFinish()
    ...
    if FishingModel.data.fish_map_id % 2 == 0 then
        ...
        self.by_bg.sprite = GetTexture("by_bg")
    end
...
function C:FishWaveEvent()
    ...
    if FishingModel.data.fish_map_id % 2 == 0 then
        ...
        FishingAnimManager.PlaySwitchoverMap(self.Fishing2DUI_Tran, "by_bg", "by_bg1", false, function ()
            self.by_bg.sprite = GetTexture("by_bg1")
            ExtendSoundManager.PlaySceneBGM(audio_config.by.bgm_by_yuchaogame.audio_name)
        end)
    else
        FishingAnimManager.PlayWaveHint(self.FXNode, false, "by_imgf_ycjs")
        FishingAnimManager.PlaySwitchoverMap(self.Fishing2DUI_Tran, "by_bg1", "by_bg", false, function ()
            self.by_bg.sprite = GetTexture("by_bg")
            ExtendSoundManager.PlaySceneBGM(audio_config.by.bgm_by_game.audio_name)
        end)
    end
...
function C:GetTimeSkillPos(skill_type)
    return self.nor_skill_prefab[skill_type .. "Button"].transform.position
end

function C:GetZHPos()
    return self.nor_skill_prefab.MFButton.transform.position
end
...
function C:on_ui_zongzi_fly_finish_msg(seat_num, score)
    ...
    if my_seat_num == seat_num then
        FishingAnimManager.PlayAddZongzi(seat_num, score, self.ZZAddMoneyNode, self:GetPlayerPos(my_seat_num))
    end
end
