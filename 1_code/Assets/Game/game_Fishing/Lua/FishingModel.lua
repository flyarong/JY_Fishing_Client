...
local function MakeLister()
    ...
    lister["fsg_all_info_test_response"] = this.on_fsg_all_info_test_response

    lister["fsg_join_msg"] = this.on_fsg_join_msg
    lister["fsg_leave_msg"] = this.on_fsg_leave_msg
...
function FishingModel.OnAssetChange(data)
    ...
    local isUp = false
    if data.change_type and string.len(data.change_type) > 9 then
        local kk = string.sub(data.change_type, 1, 9)
        if kk ~= "fish_game" then
            isUp = true
        end
    else
        isUp = true
...
function FishingModel.GetLockFishID(seat_num)
    if m_data.players_info[seat_num] then
        ...
        if not fish or not fish:CheckIsInPool_Whole() then
...
function FishingModel.SendAllInfo()
    ...
    Network.SendRequest("fsg_all_info_test", nil, "")
end
-- 断线重连数据 all_info
...
function FishingModel.S2CFrameMessage(data)
    ...
    local userdata = FishingModel.GetPlayerData()
    if m_data.cache_score ~= (userdata.wait_add_score + userdata.base.score + userdata.wait_dec_score) then
...
function FishingModel.GetPlayerLaserState(seat_num)
    if m_data and m_data.players_info and m_data.players_info[seat_num] and m_data.players_info[seat_num].base then
...
function FishingModel.GetGunSkinPower(seat_num, index)
    local cfg = FishingModel.GetGunCfg(index)
    return cfg.laser_max_rate
end
...
function FishingModel.GetMinGunCfg()
    if m_data and m_data.barbette_id then
...
function FishingModel.GetSkillMoney(key)
    if this.Config.skill_money[FishingModel.game_id] then
        return this.Config.skill_money[FishingModel.game_id][key]
    end
    return this.Config.skill_money[#this.Config.skill_money][key]
end
