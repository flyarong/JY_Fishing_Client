...
function ShatterGolden2EggsBetPanel:InitRect()
	...
	for i = 1, #config.extra2eggs do
		...
		b.transform:Find("Text"):GetComponent("Text").text=StringHelper.ToCash(config.extra2eggs[i].base_money).."金币"
		b.gameObject:SetActive(true)
		b.gameObject:GetComponent("Button").onClick:AddListener(
			function ()
				local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="zajindan_cs_bet_"..i}, "CheckCondition")
			    if a and not b then
			    	return
			    end
				self:OnButtonDown(i)
