...
function ShatterGoldenEggPanel:MakeLister()
    ...
    ------改版 ----
    lister["extend_status_notify_msg"] = basefunc.handler(self,self.on_extend_status_notify_msg)
end
...
function ShatterGoldenEggPanel:ctor(param)
    ...
    Event.Brocast("open_act_027_panel")
    --Event.Brocast("open_sys_act_base")

end

function ShatterGoldenEggPanel:Awake()
	ExtendSoundManager.PlaySceneBGM(audio_config.game.bgm_zajindanbeijing.audio_name)
end
...
function ShatterGoldenEggPanel:InitRect()
	...
	self.spawn_btn.onClick:AddListener(function()
		ExtendSoundManager.PlaySound(audio_config.game.bgm_huanyipi.audio_name)
	...
	for k, v in ipairs(logics) do
		...
		btnComponent.onClick:AddListener(function()
			if not self.enableAutoHitEgg and self:CanSwitchHammer(config_idx, not self.is2EggMode) then
				ExtendSoundManager.PlaySound(audio_config.game.bgm_huanchuizi.audio_name)
	...
	self.bl_txt.text = PlayerPrefs.GetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id).."/"
		..(PlayerPrefs.GetInt("new_cs_data_infor_nowtime"..MainModel.UserInfo.user_id) == 0 and 100 or PlayerPrefs.GetInt("new_cs_data_infor_nowtime"..MainModel.UserInfo.user_id))
	--self.sum_txt.text = self.now_data.."/"..self.data_time
	self.Slider.gameObject:GetComponent("Slider").value = (PlayerPrefs.GetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id) <= 100) and (PlayerPrefs.GetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id)/100) or 1
end

function ShatterGoldenEggPanel:SpawnEggs(hammer_idx)
	if not self.is2EggMode then
		local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="zajindan_lv_"..hammer_idx}, "CheckCondition")
	    if a and not b then
	    	return
	    end
	end
...
function ShatterGoldenEggPanel:OnSwitchMode(idx)
	...
	elseif self:CanAction() and self.enableAction then
		...
		--self.modebtn_vfx.gameObject:SetActive()
		self.modeIcon_img.sprite = GetTexture(self.is2EggMode and "zjd_btn_ptms3" or "zjd_icon12_1")
		...
		----改版-----
		self.root.gameObject:SetActive(not self.is2EggMode)
		self.root1.gameObject:SetActive(self.is2EggMode)
...
function ShatterGoldenEggPanel:OpenShop()
    ...
    GameButtonManager.RunFun({ gotoui="sys_jjj", data={ show_gift_panel={pay_cb = cb} } }, "CheckAndRunJJJ", function ()
...
function ShatterGoldenEggPanel:OnCloseSaleBox(isBoughtGiftBox)
	if SG_Config.OpenShopOnCloseSale and not isBoughtGiftBox then
        GameButtonManager.RunFun({ gotoui="sys_jjj", data={ show_gift_panel={isduring_xsth = true} } }, "CheckAndRunJJJ", function ()
...
function ShatterGoldenEggPanel:HitEgg(egg)
	...
	if self.is2EggMode then
		local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="zajindan_cs_bet_"..self.base2eggindex}, "CheckCondition")
	    if a and not b then
	    	return
	    end
	else
...
function ShatterGoldenEggPanel:StopTween()
	DOTweenManager.KillLayerKeyTween(ShatterGoldenEggModel.dotweenLayerKey)
end
...
function ShatterGoldenEggPanel:MixingAwards(finally_callback, callback)
	...
	local seq = DoTweenSequence.Create({dotweenLayerKey=ShatterGoldenEggModel.dotweenLayerKey})
	local isOK = false
...
function ShatterGoldenEggPanel:CanSpawnEggs(idx)
	...
	if not ShatterGoldenEggLogic.CheckReplaceMoney(idx) and not self.is2EggMode then
		--LittleTips.Create("您金币不足，请购买足够金币", nil, {bg = "com_bg_notice"})
		self:OpenShop()
...
function ShatterGoldenEggPanel:HammerHit(idx)
	-----改版--------
	if not self.is_can_zd and self.is2EggMode then 
		HintPanel.Create(1,"福气值不足，普通模式中砸蛋可增加福气值")
		return
	end
	...
	--ExtendSoundManager.PlaySound(audio_config.game.bgm_zayixia.audio_name)
	if self.is2EggMode then
		ExtendSoundManager.PlaySound(audio_config.game.bgm_zayixia1.audio_name)
	else
		ExtendSoundManager.PlaySound(audio_config.game.bgm_zayixia.audio_name)
	end
end
...
function ShatterGoldenEggPanel:ClearAll()
	...
	-- DOTweenManager.KillLayerKeyTween("new_seq_layer_cs_key")
end
...
function ShatterGoldenEggPanel:handle_sge_hit(result)
	...
	self.kaijiang = result.kaijiang or {}
	...
	if self.is2EggMode and dikou_money > 0 then
		...
		self:ShowEgg2Hint(mode_hammer, string.format("已扣除锤子为您抵扣%d金币", dikou_money))
	end
...
function ShatterGoldenEggPanel:ShowAward(config_idx, slot_idx, award, callback, forceFile, forcePeriod, forceOffsetTime, offset_callback)
	...
	self:PlayParticle(container, particleFile, egg.transform.position, particlePeriod, function()
		...
		if IsEquals(egg) then
			...
			local Icon1 = egg.transform:Find("@Icon1")
			local image = icon.transform:GetComponent("Image")
			...
			local seq = DoTweenSequence.Create({dotweenLayerKey="new_seq_layer_cs_key"})
			seq:Join(Icon1.gameObject.transform:DOLocalMoveY(50, effect.show_time))
			seq:OnKill(function ()
...
function ShatterGoldenEggPanel:PlayBrokenSound(list, normal, bgm)
	local total = self:CalculateAward(list)
	if total == 0 then
		--if normal then ExtendSoundManager.PlaySound("bgm_kongdan.mp3") end
	elseif total < 4 then
		if normal then ExtendSoundManager.PlaySound("bgm_1bei.mp3") end
	elseif total < 12 then
		if normal then ExtendSoundManager.PlaySound("bgm_4bei.mp3") end
	else
		if bgm then ExtendSoundManager.PlaySceneBGM("bgm_7bei.mp3", true) end
	end
end
...
function ShatterGoldenEggPanel:FlyMoney(list, slot_idx)
	...
	local function play_fire(egg, money, level)
		...
		local icon = egg.transform:Find("Icon")
		local icon1 = egg.transform:Find("@Icon")
		...
		-----------------改版
		if not table_is_null(self.kaijiang) then

			local kj_result = basefunc.deepcopy(self.kaijiang)
			local level = self.level
			self.seq_1 = DoTweenSequence.Create({dotweenLayerKey="new_seq_layer_cs_key"})
			self.seq_1:AppendInterval(0.8)
			self.seq_1:OnKill(function ()
				if not self.is2EggMode and kj_result and kj_result[#kj_result] and kj_result[#kj_result].award_value and kj_result[#kj_result].award_value ~= 0 then
					GameComAnimTool.PlayMoveAndHideFXBZ(egg.transform,"cs_fx_prefab",egg.transform.position,self.bl_txt.transform.position,0,1,function ()
						self.nuber_txt.gameObject:SetActive(true)
						self.nuber_txt.text = "+"..level*kj_result[#kj_result].award_value
						self.bl_txt.text = self.now_data.."/"..self.data_time
						self.sum_txt.text = self.now_data.."/"..self.data_time
						self.Slider.gameObject:GetComponent("Slider").value = (self.now_data <= 100) and (self.now_data/100) or 1
					end,nil,"new_seq_layer_cs_key")
				end
			end)
		end
	end
		...
		local callbacks = {
			{
				...
				method = function()
					ShatterGoldenRewardPanel.Create({money = jingbi, callback = function()
						self:PushTimer(hammer_idx, -1)
						ExtendSoundManager.PlaySceneBGM(audio_config.game.bgm_zajindanbeijing.audio_name, true)
...
function ShatterGoldenEggPanel:CatchAllAtOnce(focus_idx, list)
	...
	ExtendSoundManager.PlaySound(audio_config.game.bgm_jinzhangshike.audio_name)
	self:PlayCover({focus_idx}, "jindan_glow_H", 2, function()
		...
		ExtendSoundManager.PlaySound(audio_config.game.bgm_yiwangdajin.audio_name)
		self:PlayAnimation(hammer, "hit", 0, callbacks, function()
			self.locked = false
		...
		self:PlayCover({egg_idx}, effect_name, period, function()
			...
			if egg_idx == focus_idx then
				if loop_count == 0 then
					sound_key = ExtendSoundManager.PlaySound(audio_config.game.bgm_zailaiyici.audio_name, 2)
				end
				loop_count = loop_count + 1
			...
			if egg_tbl[egg_idx] and loop_count >= LOOP_NUM then
				...
				if sound_key ~= "" and sound_key ~= nil then
					...
					print("[SGE] ContinuousHit PlaySound failed:" .. audio_config.game.bgm_zailaiyici.audio_name)
				end
				...
				ExtendSoundManager.PlaySound(audio_config.game.bgm_zailaiyicidanbao.audio_name)
...
function ShatterGoldenEggPanel:FairiesSpreadingFlowers(focus_idx, list)
	...
	self:PlayCover({focus_idx}, "jindan_glow_H", 2, function()
		...
		self:PlayTitle("TNSH")
		ExtendSoundManager.PlaySound(audio_config.game.bgm_tiannvsanhua.audio_name)
		recursion(1)
	end)
	self:PushTimer(hammer_idx, 10)
end
...
function ShatterGoldenEggPanel:BigHammer(focus_idx, list)
	...
	self:PlayCover({focus_idx}, "jindan_glow_H", 1, function()
		...
		self:PlayTitle("YCDY")
		ExtendSoundManager.PlaySound(audio_config.game.bgm_yichuidingyin.audio_name)
		self:PlayAnimation(hammer, "hit", 0, callbacks, function()
			self.locked = false
...
-----改版------
function ShatterGoldenEggPanel:on_extend_status_notify_msg(_, data)
	dump(data)
	if data and data.key and data.key == "zajindan_cs_process_value" then
		self.is_can_zd = (data.status >= data.time) and true or  false
		self.now_data = data.status
		self.data_time = data.time
		
		PlayerPrefs.SetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id,data.status)
		PlayerPrefs.SetInt("new_cs_data_infor_nowtime"..MainModel.UserInfo.user_id,data.time)
	

		self.bl_txt.text = self.now_data.."/"..self.data_time
		self.sum_txt.text = self.now_data.."/"..self.data_time
		self.Slider1.gameObject:GetComponent("Slider").value = (data.status <= 100) and (data.status/100) or 1
		self.Slider.gameObject:GetComponent("Slider").value = (PlayerPrefs.GetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id) <= 100) and (PlayerPrefs.GetInt("new_cs_data_infor_nowdata"..MainModel.UserInfo.user_id)/100) or 1
	end
end

function ShatterGoldenEggPanel:StartTimer()
	self:StopTimer()
	self.main_timer = Timer.New(
        function ()
          	if self.Slider.gameObject and self.Slider.gameObject:GetComponent("Slider").value == 1 then
          		self.modebtn_vfx.gameObject:SetActive(true)
          		--self.mode_btn.enabled = true

      		else
      			self.modebtn_vfx.gameObject:SetActive(false)
      			--self.mode_btn.enabled = false
      		end 
      		self.root.gameObject:SetActive(not self.is2EggMode)
      		self.root1.gameObject:SetActive(self.is2EggMode)
      		self.total_txt.gameObject:SetActive(not self.is2EggMode)
			if self.is2EggMode and self.data_time then
      			self.sm_txt.text = "每"..self.data_time.."福气值可砸蛋"
			else
				self.sm_txt.text = "本局还可砸"
			end
        end
    ,1,-1)
     self.main_timer_1 = Timer.New(
        function ()
        	if self.nuber_txt.gameObject.gameObject.activeSelf  then
        		self.nuber_txt.gameObject.gameObject:SetActive(false)
    		end
        end
    ,3,-1)
...
function ShatterGoldenEggPanel:StopTimer()
