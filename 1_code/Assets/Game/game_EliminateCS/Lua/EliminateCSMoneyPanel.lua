...
function C:AssetChange(data)
    if table_is_null(EliminateCSModel.data) or EliminateCSModel.data.status_lottery == EliminateCSModel.status_lottery.run then return end
    if data.change_type and data.change_type ~= "xxl_caishen_game_award" and data.change_type ~= "task_award_no_show" then
        self:RefreshGoldText()
    end
end
