...
EliminateCSInfoPanel = basefunc.class()
local M = EliminateCSInfoPanel
M.name = "EliminateCSInfoPanel"
local instance_Info
function M.Create()
    instance_Info = M.New()
...
--断线重连获取数据 或者 结束的时候 重新刷新一次数据
function M:view_lottery_end_nor(data)
   self:MyRefresh(data)
end

function M:DesPrefabInto(id, num, IsReConnect)
    local num = "x" .. num
    EliminateCSDesPrefab.Create(id, num, IsReConnect, self.Content)
end

function M:AddGold(S)
    if self.gold + S > EliminateCSModel.GetAwardMoney() then
        return
    end
    self.gold = self.gold + S
    -- if  self.gold>=EliminateCSModel.GetAwardMoney() then
    -- 	self.gold=EliminateCSModel.GetAwardMoney()
    -- end
...
function M:eliminate_refresh_yazhu(data)
    ...
    EliminateCSModel.SetBet(self.BetList)
end

function M:MyRefresh(data)
    dump(self.gold, "由每次累计算出的金币")
    local sum = 0
