...
FishFarmBagFryPanel = basefunc.class()
local C = FishFarmBagFryPanel
C.name = "FishFarmBagFryPanel"
...
function C:MakeLister()
    ...
    self.lister["fishbowl_hatch_response"] = basefunc.handler(self,self.on_fishbowl_hatch_response)
end
...
function C:CreateItemPrefab()
	...
	for i=1,#self.fish_list do
		local pre = FishFarmBagItemBase.Create(self,self.Content.transform,i,self.fish_list[i],"fry")
...
function C:on_fishbowl_hatch_response()
	LittleTips.Create("投放成功!")
end