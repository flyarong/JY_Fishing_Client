...
function fish_dead(data, type, ZZ)
    ...
    if userdata and userdata.base then
        local _money_log_id = money_log_id
            ...
            if score == 0 then
                if skill_data then
                    ...
                    if skill_data.type == 9 then
                        -- skill_data.act_type = 0 是boss鱼
                        -- skill_data.act_type = 10 是话费鱼
                        if false--[[skill_data.act_type and skill_data.act_type ~= 10 and skill_data.act_type ~= 8--]] then
                ...
                buf1.score = score
                buf1.dead_rate = dead_rate
                buf1.fish_id = fish_id
                ...
                if type and (type == FishingSkillManager.FishDeadAppendType.QP_bomb
                    ...
                    or type == FishingSkillManager.FishDeadAppendType.Boom
                    or type == FishingSkillManager.FishDeadAppendType.tstx
                    or type == FishingSkillManager.FishDeadAppendType.huolong
                    or type == FishingSkillManager.FishDeadAppendType.Lightning) then
                        is_bubojiaqian = true
                end
                if skill_data and (skill_data.type == FishingSkillManager.FishDeadAppendType.lhzp
                                or skill_data.type == FishingSkillManager.FishDeadAppendType.huolong) then
                ...
                if not skill_data then
                    ...
                    if not is_bubojiaqian and buf1.cfg_fish_id ~= 38 then
                    ...
                    FishManager.S2CFishDead(fish_id, ZZ, buf1)
                else
                    if fish then
                    ...
                    if skill_data.type == FishingSkillManager.FishDeadAppendType.tstx then -- 鲨鱼 + 吞噬效果
                        FishManager.S2CFishDead(fish_id, ZZ, buf1)
                        FishingAnimManager.PlayShowAndHideFXAndCall_A(panelSelf.FlyGoldNode.transform, "Fish3D077_shuihua", beginPos, 1, nil, function ()
                            if not is_bubojiaqian then
                                FishingAnimManager.PlayDeadFX(buf1, panelSelf.FlyGoldNode.transform, beginPos, endPos, playerPos, mbPos, gunPos, name_img, delta_t)
                            end
                            Event.Brocast("model_dispose_skill_data", skill_data)
                        end, 1, 0.5)
                    elseif skill_data.type == FishingSkillManager.FishDeadAppendType.zycs_pd then -- 章鱼拍打 效果
                        FishManager.S2CFishDead(fish_id, nil, skill_data)
                        if not is_bubojiaqian then
                            FishingAnimManager.PlayDeadFX(buf1, panelSelf.FlyGoldNode.transform, beginPos, endPos, playerPos, mbPos, gunPos, name_img, delta_t)
                        end
                    elseif FishingSkillManager.IsSkillAndBD(skill_data) then
                        -- 炸弹前奏音效
                        if buf1.cfg_fish_id == 19 then
                            ExtendSoundManager.PlaySound(audio_config.by3d.bgm_by_zhadanyu_rensheng.audio_name)
                        elseif buf1.cfg_fish_id == 21 then
                            ExtendSoundManager.PlaySound(audio_config.by3d.bgm_by_zhadanyu1_rensheng.audio_name)
                        end
