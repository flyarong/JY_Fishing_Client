...
function C:ForceShoot(parm)
    ...
    if parm and parm.vec then
        ...
        local rr = self.GunAnim.transform.eulerAngles.z + 90
    ...
    if parm and parm.type then
        self:SendRunShoot(dirVec, nil, nil, parm.type, parm.data)
    else
        self:SendRunShoot(dirVec, nil, nil, 5, parm.data)
    end
end
-- 自动开枪
...
function C:SendShoot(vec, auto, FishID)
    ...
    local user = FishingModel.GetPosToPlayer(self.seat_num)
    if not user or not user.base then
        return
    end
    -- 炮台切换待解锁状态
    if self.is_on_hint then
        return
    end
    if user and user.base and FishingModel.GetPlayerSeat() == user.base.seat_num then
        if user.index > #FishingModel.data.barbette_id then
            local a,b,desc = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="fishing3d_".. FishingModel.data.game_id .. "_" .. user.index, is_on_hint=true}, "CheckCondition")
            if a and not b and desc then
                user.is_auto = false
                Event.Brocast("set_gun_auto_state", { seat_num=user.base.seat_num })
                self.is_on_hint = true
                GameManager.GotoUI({gotoui="vip", goto_scene_parm="hint", data={desc=desc, call=function ()
                    user.index = FishingModel.data.barbette_id[#FishingModel.data.barbette_id]
                    self.is_on_hint = false
...
-- 开始开枪
function C:SendRunShoot(vec, auto, FishID, act_type, parm)
    local dirVec = vec
    --先旋转炮台在开枪
    local r = Vec2DAngle(dirVec)
    if FishingModel.IsRotationPlayer() then
...
function C:Rotate(isLeft)
    ...
    if self.GunAnim.transform.eulerAngles.z > rotateRrangeHalf and self.GunAnim.transform.eulerAngles.z <(360 - rotateRrangeHalf) then
        if direct > 0 then
            self.GunAnim.transform:Rotate(Vector3.forward, -1 *(self.GunAnim.transform.eulerAngles.z - rotateRrangeHalf))
        else
            self.GunAnim.transform:Rotate(Vector3.forward,(360 - rotateRrangeHalf - self.GunAnim.transform.eulerAngles.z))
        end
    end
end
-- 设置是否自动
...
function C:Debug()
    print("<color=red><size=15>[Debug] FishingGun</size></color>")
    dump(self)
end
