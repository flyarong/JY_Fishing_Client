...
function C:GetSpeed(time_elapsed)
	return self.fish_base:GetSpeed(time_elapsed)
end
...
function C:UpdateStatus(b)
	if self.data.status and self.data.status > 0 then
		...
		elseif self.fish_cfg.id == 24 then
			local m_scale = 1
...
function C:AttractHit(pos, t, b)
	self.fish_base:AttractHit(pos, t, b)
end

function C:CloseFishID()
	self.fish_base:CloseFishID()
end