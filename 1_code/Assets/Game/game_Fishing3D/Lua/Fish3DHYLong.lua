...
Fish3DHYLong = basefunc.class()
local C = Fish3DHYLong
C.name = "Fish3DHYLong"

Fish3DHYLong.FishState = 
{
	FS_Nor="正常",
	FS_Flee="逃离",
	FS_Hit="受击",
	...
	if event then
		...
		for k,v in ipairs(e_t) do
			...
			elseif tt == "move" then
				self:Kill_sf_seq()
				local parm = {}
				parm.x = tonumber(bb[2])
				parm.y = tonumber(bb[3])
				parm.time = tonumber(bb[4])
				self.sf_seq = DoTweenSequence.Create()
				self.sf_seq:Append(self.transform:DOLocalMove(Vector3.New(self.transform.localPosition.x+parm.x, self.transform.localPosition.y+parm.y, self.transform.localPosition.z), parm.time))
...
function C:ctor(parent, data)
	...
	self.init_pos = Vector3.zero
	self.transform.localPosition = self.init_pos

	self.m_fish_state = Fish3DHYLong.FishState.FS_Nor
	self.root_scale = 1
	...
	self.lock_node = tran:Find("fish3dnode/fish3d/bs_0005 1/Bone017/Bone016")
	self.hang_node = tran:Find("hang_node")

	self.fish_tran = tran:Find("fish3dnode/fish3d"):GetComponent("Transform")

	self.fish_mesh = tran:Find("fish3dnode/fish3d/bs_0005 1/bs_0005 2"):GetComponent("SkinnedMeshRenderer")
...
function C:CheckIsInPool()
	if self.m_fish_state == Fish3DHYLong.FishState.FS_Flee or
		self.m_fish_state == Fish3DHYLong.FishState.FS_Dead or
		self.m_fish_state == Fish3DHYLong.FishState.FS_FeignDead then
		return false
	end
	return true
end
...
function C:SetFeignDead(b)
	if self.m_fish_state == Fish3DHYLong.FishState.FS_Dead or self.m_fish_state == Fish3DHYLong.FishState.FS_Flee then
		return
	end
	if b then
		self.m_fish_state = Fish3DHYLong.FishState.FS_FeignDead
		self:SetBox2D(false)
	else
		self.m_fish_state = Fish3DHYLong.FishState.FS_Nor
		self:SetBox2D(true)
...
function C:Flee()
	if self.m_fish_state ~= Fish3DHYLong.FishState.FS_Dead and self.m_fish_state ~= Fish3DHYLong.FishState.FS_Flee and
		self.m_fish_state ~= Fish3DHYLong.FishState.FS_FeignDead then

		self.m_fish_state = Fish3DHYLong.FishState.FS_Flee
		self:SetBox2D(false)
		self:Back_iceobj()
...
function C:Hit()
	if self.m_fish_state ~= Fish3DHYLong.FishState.FS_Dead and self.m_fish_state ~= Fish3DHYLong.FishState.FS_Flee then
		self.m_fish_state = Fish3DHYLong.FishState.FS_Hit
		if self.anim_time then
			self.anim_time:Stop()
		end
...
function C:ShowDead(_dead_index, ZZ, parm)
	dump(parm, "<color=red>火龙 死亡表现/color>")
	ExtendSoundManager.PlaySound(audio_config.by.bgm_by_beikejiangli.audio_name)
	self:SetBox2D(false)
	self.m_fish_state = Fish3DHYLong.FishState.FS_Dead
	if self.anim_time then
		self.anim_time:Stop()
	end
	self:Kill_sf_seq()
