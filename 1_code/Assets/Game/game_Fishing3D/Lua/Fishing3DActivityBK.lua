...
Fishing3DActivityBK = basefunc.class()

local M = Fishing3DActivityBK
M.name = "Fishing3DActivityBK"
...
function M:MakeLister()
    ...
    self.lister["uifx_move_finish_show_fish_by_id"] = basefunc.handler(self, self.uifx_move_finish_show_fish_by_id)
    self.lister["uifx_all_move_finish"] = basefunc.handler(self, self.uifx_all_move_finish)
end
...
function M:ctor(data,config)
    ...
    self.actbk_state = Fishing3DActivityBK.FABKState.FABKS_Nor
    ...
    self.bknode = {}
    self.bknode_pos = self.panelSelf:GetTSPath(uipos)
...
function M:SetTimeText()
    if self.cur_time < self.all_time then
        if self.data.seat_num == FishingModel.GetPlayerSeat() then
            ...
            if tt <= 10 and self.actbk_state == Fishing3DActivityBK.FABKState.FABKS_Nor then
                self.actbk_state = Fishing3DActivityBK.FABKState.FABKS_Zh
...
function M:uifx_all_move_finish(seat_num)
    if seat_num == self.data.seat_num and self.data.status == 0 then
        FishingModel.SendActivity(self.data)
    end
end
function M:uifx_move_finish_show_fish_by_id(seat_num, i)
    if seat_num == self.data.seat_num then
        local pos = self.bknode_pos[i]
        if self.data.data and self.data.data[i] then
            local fish = FishManager.GetFishByID(self.data.data[i])
            if fish and fish.CreateFinish then
                fish:CreateFinish()
            end
        end
