...
function M:Update()
    ...
    if self.cur_time >= self.all_time then
        ...
        if self.data.num and self.data.num > 0 then
            Event.Brocast("ui_oper_manual_shoot_msg", {seat_num = self.data.seat_num, type = 5, data={status=self.data.x} })
        end
    end
end
function M:on_ui_oper_manual_shoot_msg(data)
    if data.seat_num == self.data.seat_num then
        Event.Brocast("ui_oper_manual_shoot_msg", {seat_num = data.seat_num, type = 5, vec=data.vec, data={status=self.data.x} })
        if self.update then
            self.update:Stop()
            self.update = nil
        end
...
function M:ChangeGame()
    ...
    if self.data.num and self.data.num > 0 and self.data.status == 1 then
        ...
        if n_g.gunprefab ~= "GunPrefab15" and n_g.gunprefab ~= "GunPrefab16" then
            self.org_gun_cfg = {}
            self.org_gun_cfg.gunprefab = n_g.gunprefab
            if self.data.x == 17 then
                n_g.gunprefab = "GunPrefab15"
            else
                n_g.gunprefab = "GunPrefab16"
            end
    ...
    if self.data.num and self.data.num == 0 and self.data.status == 1 then
        ...
        local uipos = FishingModel.GetSeatnoToPos(self.data.seat_num)
        local p_pre = pan.PlayerClass[uipos]
        p_pre.gun_obj.gameObject:SetActive(false)
    end
    ...
    if self.data.num and self.data.num > 0 and self.data.status == 1 then
        ...
        -- Event.Brocast("ui_player_change_zt", {seat_num = self.data.seat_num, status=self.data.x, is_cj_or_sc = true})
    else
        self.xh_audio = ExtendSoundManager.CloseSound(self.xh_audio)
        if IsEquals(p_pre.FSZT_btn) then
            p_pre.FSZT_btn.gameObject:SetActive(false)
        ...
        -- Event.Brocast("ui_player_change_zt", {seat_num = self.data.seat_num, status=self.data.x, is_cj_or_sc = false})
    end
end
--[[
    @desc: 还原游戏
...
function M:RecoverGame()
    ...
    local uipos = FishingModel.GetSeatnoToPos(self.data.seat_num)
    local p_pre = pan.PlayerClass[uipos]
    p_pre.gun_obj.gameObject:SetActive(true)
end
...
function M:PlayActivityNode()
    ...
    if not self.data.speed then
        ...
        seq:OnKill(function ()
            FishingAnimManager.PlayMoveAndHideFX(self.ui.transform, "Fishing3D_chuanyunxie", beginPos, endPos, 2, 1, function ()
