...
function M.Create()
    instance = M.New()
    return createPanel(instance, M.name)
end
...
function M:qhb_my_hb_change_msg(get_data)
    ...
    if get_data.boom and get_data.boom == 1 then
        ...
        str = string.format( "<size=60>%s 在您的红包里抢到了 %s 金币\n但踩到了雷点，<color=red>系统奖励您 %s 金币</color></size>",name,jb,boom_count)
        LittleTips.CreateSP1(str)    
        return
    else
        str = string.format( "%s 在您的红包里抢到了 %s 金币",name,jb)
    end
    LittleTips.CreateSP(str)
end
