...
function MjXzGamePanel:Awake()
    ...
    -- ExtendSoundManager.PlaySceneBGM("majiang_bgm_game.mp3")
...
function MjXzGamePanel:MyRefresh()
    ...
    if m_data then
        ...
        if not m_data.model_status then
            ...
            if GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_MjXzMatch3D then
                MatchComWaitStartPanel.Create({model = MjXzModel,logic = MjXzLogic,ani = MjAnimation})
            elseif GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_MjMatchNaming then
                MatchComWaitRematchPanel.Create()
            end
            MatchComRankPanel.Close()
        ...
        elseif m_data.model_status == MjXzModel.Model_Status.wait_begin then
            ...
            print("<color=white>涨停》》》》》》》》》game_MjMatchNaming</color>",GameMatchModel.GetGameIDToGameType())
        ...
        elseif m_data.model_status == MjXzModel.Model_Status.wait_table then
            if GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_MjXzMatch3D then
                MatchComWaitStartPanel.CloseUI()
            elseif GameMatchModel.GetGameIDToGameType() == GameMatchModel.GameType.game_MjMatchNaming then
                MatchComWaitRematchPanel.CloseUI()
            end
            MatchComRankPanel.Close()
    ...
    if MjXzModel.data then
        ...
        if myData.model_status == MjXzModel.Model_Status.promoted then
            if myData.promoted_type then
                ...
                ExtendSoundManager.PlaySound(audio_config.game.bgm_bisai_jinji.audio_name)
                SysInteractivePlayerManager.Close()
                SysInteractiveChatManager.Hide()
...
function MjXzGamePanel:nor_mg_gameover_msg()
    --self:RefreshWait()
    local gameCfg = GameMatchModel.GetGameIDToConfig(GameMatchModel.data.game_id)
...
function MjXzGamePanel:CreateRankPanel()
    ...
    MatchComRankPanel.Create(parm, function (game_id)
        MjXzModel.ClearMatchData(game_id)
    end)
    self:HideWaitPanel()
    self:HideRevivePanel()
end
...
function MjXzGamePanel:nor_mj_xzdd_pai_msg()
    ...
    MatchComWaitPanel.ShowPromotionInfo(GameMatchModel.data.game_id,MjXzModel.data.round_info)
end
...
function MjXzGamePanel.PlayBSKS()
    ...
    if not is_bsks and data and data.round_info and data.round_info.round and data.round_info.round == 1 then
		is_bsks = ExtendSoundManager.PlaySound(audio_config.game.bgm_bisai_kaishi.audio_name)
    end
end
...
function MjXzGamePanel:ShowWaitPanel(is_ani)
	...
	data.game_cfg = GameMatchModel.GetGameIDToConfig(_data.game_id)
	data.award_cfg = GameMatchModel.GetGameIDToAward(_data.game_id)
    ...
    MatchComWaitPanel.Create(data)
end

function MjXzGamePanel:HideWaitPanel()
    MatchComWaitPanel.Close()
end
...
function MjXzGamePanel:ShowRevivePanel()
    ...
    local game_cfg = GameMatchModel.GetGameIDToConfig(_data.game_id)
    MatchComRevivePanel.Create(data,game_cfg)
end

function MjXzGamePanel:HideRevivePanel()
    MatchComRevivePanel.Close()
end