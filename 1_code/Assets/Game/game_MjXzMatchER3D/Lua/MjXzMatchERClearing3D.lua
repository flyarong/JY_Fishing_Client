...
function MjXzMatchERClearing3D:InitRect()
	...
	for i = 1, 4 do
		...
		if MjXzModel.GetRealPlayerSeat () == seatno then
			...
			if score > 0 then
				-- ParticleManager.JieSuanWin(self.WinNode.transform)
				...
				-- ParticleManager.JieSuanLose(self.LoseNode.transform)
...
function MjXzMatchERClearing3D:OnConfirmClick()
	local config = GameMatchModel.GetGameIDToConfig(MjXzModel.data.game_id)
    if config and config.enter_condi_count then
        if config.enter_condi_count > MainModel.UserInfo.jing_bi then
            HintPanel.Create(
                3,
                "当前场次入场要求为" .. StringHelper.ToCash(config.enter_condi_count) .. "金币以上\n您金币不足，请前往购买",
                function()
