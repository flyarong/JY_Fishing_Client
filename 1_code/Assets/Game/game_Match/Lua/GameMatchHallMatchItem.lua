...
GameMatchHallMatchItem = basefunc.class()

local C = GameMatchHallMatchItem

C.name = "GameMatchHallMatchItem"
...
function C:OnClick()
    ...
    GameMatchHallPanel.ShowMatch(self.config)
end
...
function C:OnSignupClick()
    ...
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
    GameMatchHallPanel.SignupMatch(self.config)
end
...
function C:SetHBSUI(  )
    local item_key, item_count = GameMatchModel.GetMatchCanUseTool(self.config.enter_condi_itemkey, self.config.enter_condi_item_count)
    if item_key then
