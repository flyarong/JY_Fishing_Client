...
local FreeAwardConfig = require "Game.game_Mj3D.Lua.mjxz_drivingrange_award3D"   --奖励配置
...
local function InitMatchData(gameID)
    if not MjXzModel.baseData then
        MjXzModel.baseData = {}
    end
...
function MjXzModel.InitGameData()
    if not m_data then
...
-- 初始化游戏配置Config
function MjXzModel.InitUIConfig()
    this.UIConfig={
        award = {},    }
...
----------------------------------------------------------------------------------------- new
--所有数据
function MjXzModel.on_fg_all_info(proto_name, data)
    ...
    if data.status_no == -1 then
        ...
        if s then
            ...
            --MjXzModel.buf_game_type = s.game_type
            m_data.countdown = s.countdown
            MjXzModel.baseData.room_rent = s.room_rent

            print("<color=yellow>---------------------------- m_data.game_type:</color>",MjXzModel.game_type)
            MjXzModel.setBaseShouPaiNum(MjXzModel.game_type)
            MjXzModel.setMaxPlayerNumber(MjXzModel.game_type)
            MjXzModel.setTotalCardNum(MjXzModel.game_type)
        ...
        mj_algorithm = nor_mj_base_lib.New( kaiguan , multi , MjXzModel.game_type)
        ...
        if m_data.seat_num and s then   -- 
            ...
            if s.zhuan_yu_data then
                ...
                --Event.Brocast("model_zhuan_yu_msg", zyData)
            end
        end
        ...
        if s then
            m_data.init_stake = s.init_stake
            m_data.init_rate = s.init_rate
            MjXzModel.baseData.game_id = s.game_id
        end
        ...
        if data.activity_data then
            ...
            MjXzModel.GetLSCount(data.activity_data)
        else
            m_data.activity_data = nil
            m_data.ls_count = 1
        end

        -- if m_data.model_status == MjXzModel.Model_Status.gameover then
        --     MainLogic.ExitGame()
        -- end
    ...
    if m_data then
        ...
        Event.Brocast("activity_fg_all_info",{activity_data = m_data.activity_data,game_type = MjXzModel.game_type,game_id = MjXzModel.baseData.game_id,model_status = m_data.model_status,status = m_data.status})
    end
...
--进入房间
function MjXzModel.on_fg_enter_room_msg(proto_name, data)
...
--其他玩家进入游戏
function MjXzModel.on_fg_join_msg(proto_name, data)
...
--其他玩家离开游戏
function MjXzModel.on_fg_leave_msg(proto_name, data)
...
--比赛结束
function MjXzModel.on_fg_gameover_msg(proto_name, data)
...
--分数改变
function MjXzModel.on_fg_score_change_msg(proto_name, data)
...
---- 自动取消报名
function MjXzModel.on_fg_auto_cancel_signup_msg(proto_name, data)
    InitMatchData()
    MainLogic.ExitGame()
...
---- 自动退出游戏报名
function MjXzModel.on_fg_auto_quit_game_msg(proto_name, data)
    InitMatchData()
    MainLogic.ExitGame()
...
---- 准备
function MjXzModel.on_fg_ready_msg(proto_name, data)
...
-------------------------------------------------------------------response
--比赛报名结果
function MjXzModel.on_fg_signup_response(_, data)
    dump(data, "<color=yellow>on_fg_signup_response</color>")
    if data.result == 0 then
        MjXzModel.InitGameData()
        ...
        MjXzModel.game_type = data.game_type
...
function MjXzModel.on_fg_switch_game_response(proto_name, data)
    MjXzModel.on_fg_signup_response(proto_name, data)
end

function MjXzModel.on_fg_cancel_signup_response(_, data)
    if data.result == 0 then
...
--再玩一把
function MjXzModel.on_fg_replay_game_response(proto_name, data)
        ...
        if data.result == 1022 then
            ...
            HintPanel.Create(
                ...
                function()
                    ...
                    InitMatchData()
                    MainLogic.ExitGame()
                    --GameManager.GotoSceneName("game_DdzMatch")
                    MjXzLogic.change_panel( MjXzLogic.panelNameMap.hall )
                end
            )
        end
...
--退出游戏
function MjXzModel.on_fg_quit_game_response(proto_name, data)
...
--换桌
function MjXzModel.on_fg_huanzhuo_response(proto_name, data)
...
-- 准备
function MjXzModel.on_fg_ready_response(_, data)
...
function MjXzModel.on_kaiguan_multi_change_msg(proto_name, data)
    dump( data , "<color=yellow> ---- 开关，番数改变 </color>" )

    mj_algorithm:set_kaiguan( basefunc.decode_kaiguan( MjXzModel.game_type , data.game_kaiguan) )
    mj_algorithm:set_multi( basefunc.decode_multi( MjXzModel.game_type , data.game_multi) )
...
function MjXzModel.on_nor_mj_xzdd_settlement_msg(proto_name, data)
    ...
    -- }
    -- Event.Brocast("fg_activity_data_msg","fg_activity_data_msg",m_data)
    MjXzModel.grand_total_settlement()
end
...
-- 换桌检查
function MjXzModel.HZCheck()
    if MjXzModel.checkCondition(MjXzModel.hz_call) then
        MjXzModel.hz_call()
    end
end
-- 准备检查
function MjXzModel.ZBCheck()
    if MjXzModel.checkCondition(MjXzModel.zb_call) then
        MjXzModel.zb_call()
    end
end

function MjXzModel.on_fg_activity_data_msg(proto_name, data)
...
--获取结算时所有玩家数据
function MjXzModel.GetPlayersData()
...
function MjXzModel.GetLSCount(activity_data)
    local aid = 0
    local lsc = 1
...
function MjXzModel.on_nor_mj_xzdd_game_bankrupt_msg(proto_name, data)
...
--资产改变
function MjXzModel.AssetChange(proto_name, data)
...
--[[累计胜负统计]]
function MjXzModel.grand_total_settlement()
    if MjXzModel.IsMyWin()then
