...
function MjXzClearing:UpdateExchange()
    ...
    if is_game_over and MjXzModel.data.exchange_hongbao and MjXzModel.data.exchange_hongbao.is_exchanged == 0 then
        ...
        self.ExChongbao.text = StringHelper.ToRedNum(MjXzModel.data.exchange_hongbao.hong_bao )
        Event.Brocast("global_sysqx_uichange_msg", {key="dh", panelSelf=self})
...
function MjXzClearing:OnExchangeClick()
    ...
    if not iss then
        local rr = StringHelper.ToRedNum(MjXzModel.data.exchange_hongbao.hong_bao )
...
function MjXzClearing:CalcMyResult(data)
	...
	if MjXzModel.GetRealPlayerSeat () == seatno then
		...
		if isWin and MjXzModel.data.ls_count > 1 and OperatorActivityModel.IsActivated(MjXzModel.baseData.game_id, ActivityType.Consecutive_Win) and ActivityConsecutiveWin.CanBeAwarded() then
...
function MjXzClearing:SetBackAndConfirmBtn()
    ...
    if is_game_over then
		--红包
		GameManager.GotoUI({gotoui = "sys_total_red",goto_scene_parm = "panel",transform = self.transform,game_id = MjXzModel.baseData.game_id})
    end
