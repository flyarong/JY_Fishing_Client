...
MjXzHallPanel = basefunc.class()

MjXzHallPanel.name = "MjXzHallPanel3D"
local lister
local listerRegisterName="mjXzFreeHallListerRegister"
local curSwitchMatch
local instance

function MjXzHallPanel.Create()
	instance=MjXzHallPanel.New()
	return createPanel(instance,MjXzHallPanel.name)
end
function MjXzHallPanel.Bind()
	local _in = instance
	instance = nil
	return _in
end

function MjXzHallPanel:Awake()
	MainModel.gameHallLocation = "MjXzHallPanel"
	ExtendSoundManager.PlaySceneBGM(audio_config.mj.majiang_bgm_hall.audio_name)
	local tran = self.transform
	self.typeContent =  GetPrefab("MjXzHallPanelTypeContent3D")
	self.typeItem =  GetPrefab("MjXzHallPanelTypeItem3D")
	...
	self.behaviour:AddClick(self.BackButton.gameObject, MjXzHallPanel.OnBackClick, self)
	self.behaviour:AddClick(self.NextButton.gameObject, MjXzHallPanel.OnNextClick, self)
	self.behaviour:AddClick(self.goldBtn.gameObject, MjXzHallPanel.OnGlodClick, self)
...
function MjXzHallPanel:Start()
	self:MyInit()
	self:MyRefresh()
end

function MjXzHallPanel:OnDestroy()
	Event.RemoveListener("AssetChange", self.updateMoney)
end

--[[初始化功能，创建好panel后在Award中调用，只做一次]]
function MjXzHallPanel:MyInit()
	self:MakeLister()
	MjXzLogic.setViewMsgRegister(lister,listerRegisterName)
end

--[[刷新功能，供Logic和model调用，重复性操作]]
function MjXzHallPanel:MyRefresh()
	self:RefreshMoney()
	--self:RefreshGameList(MjXzModel.gameList)
end

-- 刷新钱
function MjXzHallPanel:RefreshMoney()
	self.MoneyText.text = StringHelper.ToCash(MainModel.UserInfo.jing_bi)
end

--[[退出功能，供logic和model调用，只做一次]]
function MjXzHallPanel:MyExit()
	MjXzLogic.clearViewMsgRegister(listerRegisterName)
	lister = nil
	--closePanel(MjXzHallPanel.name)
	self.typeContent = nil
	self.typeItem = nil
end
function MjXzHallPanel:MyClose()
    self:MyExit()
    closePanel(MjXzHallPanel.name)
end

-- UI关注的事件
function MjXzHallPanel:MakeLister()
	lister = {}
...
-- 更新游戏列表
function MjXzHallPanel:RefreshGameList(data)
	if data then
...
-- 创建入口Item
function MjXzHallPanel:CreateItem(data)
	local parent
	...
	local v = MjXzModel.UIConfig.entrance[data.game_id]
	...
	self.behaviour:AddClick(bgImage.gameObject, MjXzHallPanel.OnEnterClick, self)
	return go
end

--退出
function MjXzHallPanel:OnBackClick( go )
	ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
	MainLogic.GotoScene("game_Hall")
end

--查看更多比赛
function MjXzHallPanel:OnNextClick( go )
...
function MjXzHallPanel:OnGlodClick( go )
	PayPanel.Create(GOODS_TYPE.jing_bi, "normal")
end

-- 清空游戏列表
function MjXzHallPanel:CloseCellList()
...
function MjXzHallPanel:OnEnterClick(go)
	if MjXzModel.gameList then
		...
		local ss = MjXzModel.IsRoomEnter(id)
		if ss == 1 then
			PayFastFreePanel.Create(MjXzModel.UIConfig.entrance[id],signup)
			return
		end
		if ss == 2 then
			HintPanel.Create(1, "您的金币持有量过高，请选择更高级别场次")
			return
		end
...
*********************--]]
-- 收到游戏列表数据
function MjXzHallPanel:RecGameList(result)
	if result == 0 then
		--self:RefreshGameList(MjXzModel.gameList)
	else
...
function MjXzHallPanel:RecSignup(result)
