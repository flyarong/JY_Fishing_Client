...
function C:MyRefresh()
    if self.parm.num > 0 then
        ...
        dump(self.parm,"<color>===============assets=============</color>")
        dump(self.revive_item,"<colorrevive_itemcolor>")
        ...
        dump(self.revive_item.item,"<color>+++++++++++++++++++++++++++++++++++++</color>")
        if self.revive_item.item == "cash" then     
            self.revive_txt.text = string.format( "x%s", self.revive_item.count / 100)
            dump(self.revive_item.count,"<color>00000000000000000000000000</color>")
        else
            dump(self.revive_item.count,"<color>5555555555555555555555555555</color>")
        ...
        dump(FishingMatchModel.data,"<color>++++++++++++++++FishingMatchModel.data+++++++++++++<color>")
        dump(vv,"<color>++++++++++++++++vv+++++++++++++<color>")
        self.award_pool_txt.text = math.floor(vv / 100) .. "福利券"
    else
        self.hint1_rect.gameObject:SetActive(false)
        self.hint2_rect.gameObject:SetActive(false)
