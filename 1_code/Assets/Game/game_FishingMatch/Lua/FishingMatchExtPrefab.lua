...
function C:ctor(tran, panelSelf)
    ...
    self.rank_btn.onClick:AddListener(function ()
    	...
    	FishingMatchRankPanel.Create({game_id = FishingMatchModel.data.game_id, game_type="djs"})
    end)
	...
	if m_data and m_data.model_status == FishingMatchModel.Model_Status.gaming then
		...
		if cfg then
			...
			local ok, arg = xpcall(function ()
				...
				ew = ew * math.floor(total_award_pool / 100)
				self.rank_award_txt.text = "当前排名 " .. StringHelper.ToCash(StringHelper.ToRedNum(gd + ew)) .. "福利券"
		    end,
		    function (error)
				self.rank_award_txt.text = "当前排名 " .. StringHelper.ToCash(StringHelper.ToRedNum(gd)) .. "福利券"
			end)
		else
			self.rank_award_txt.text = "当前排名 0福利券"
		end
	else
		self.rank_award_txt.text = "当前排名 0福利券"
	end
...
function C:on_fsmg_query_total_award_pool(_, data)
	...
	if data.result == 0 then
		...
		self.red_award_txt.text = StringHelper.ToCash(math.floor(FishingMatchModel.data.total_award_pool / 1000)) .. "福利券"
		self:RefreshRankRed()
	end
end
