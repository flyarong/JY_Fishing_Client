...
CityMatchActionUiManger = basefunc.class()


function CityMatchActionUiManger.Create(gamePanel,playerOperateUI,dizhuCardUI)
	local instance=CityMatchActionUiManger.New()
...
function CityMatchActionUiManger:Refresh()
	--全部隐藏
	...
	if CityMatchModel.data then
		...
		local hash={}
		if act_list and #act_list>0 and CityMatchModel.data.s2cSeatNum then
			local len= #act_list
				...
				if i>0 then
					...
					if not hash[act.p] then
						if not m_data.cur_p or  (act.p~=m_data.cur_p and m_data.cur_p~=6) or m_data.status==nor_ddz_base_lib.get_action_status(act) then
							hash[act.p]=true
							self:RefreshAction(CityMatchModel.data.s2cSeatNum[act.p],act)
						end
					end
				else
					break
...
--根据当前状态调整动作的UI的显示
function CityMatchActionUiManger:changeActionUIShowByStatus()
	--如果上一个已经是不同类型
	local m_data=CityMatchModel.data
	if m_data then
		if m_data.status==CityMatchModel.Status.cp then
			local cur_p=m_data.cur_p
			if cur_p and CityMatchModel.data.s2cSeatNum then 
			 	local other_p={}
			 	other_p[1]=CityMatchModel.data.s2cSeatNum[cur_p]
			 	for i=2,3 do
...
function CityMatchActionUiManger:RefreshAction(seatNum,act)
	--清空之前残留
	...
	if act.type>99 or act.type==0 then
		...
		CityMatchModel.ddz_algorithm:get_cpInfo_by_action(act)
		...
		if CityMatchModel.data then
			if CityMatchModel.data.seatNum[seatNum] == CityMatchModel.data.dizhu then
...
--带动画和音效刷新
function CityMatchActionUiManger:RefreshActionWithAni(seatNum,act)
	...
	if act.type>99 or act.type==0 then
		...
		sound=sound ..  AudioBySex(CityMatchModel, CityMatchModel.data.seatNum[seatNum])
			...
			if idx==#self.showCardList[seatNum] then
				if act.type == 13 or act.type == 14 then
					...
					if  CityMatchModel.data.seatNum[seatNum] == CityMatchModel.data.dizhu then
						...
						for i=1,3 do
							if i ~= seatNum then
								if CityMatchModel.data.seatNum[i] == CityMatchModel.data.dizhu then
									SpineManager.BeiZha(i)
									ParticleManager.DDZBeiZha(i)
								else
...
function CityMatchActionUiManger:PlayBombBGM()
...
function CityMatchActionUiManger:ClearCardList()
end

function CityMatchActionUiManger:MyExit()
