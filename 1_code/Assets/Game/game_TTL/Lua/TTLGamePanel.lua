...
function C:on_bullet_behit_by_bullet_msg(no)
	...
	elseif cfg.type == "switch" then
		...
		if self.switch_on==3 and not self.already_roll and self.random_award > 0 then -- 灯全亮
...
function C:ctor()
	...
	MainModel.SetGameBGScale(self.TTLBG)
	...
	self.wealth_txt.text = StringHelper.ToCash(MainModel.UserInfo.jing_bi)--刷新金币显示
...
function C:MyRefresh()
	dump(GameItemModel.GetItemCount("prop_tiny_game_coin"),"<color>+++++++++++++++++小游戏币++++++++++++++++</color>")
...
function C:Shoot()
	...
	Network.SendRequest("tantanle_kaijiang",{angle=zz, game_id=self.game_power_id}, "", function (data)
		...
		if data.result == 0 then
			...
			--[[local cost = TTLModel.Defines.game_power[self.game_power_id]
			if cost >= GameItemModel.GetItemCount("prop_tiny_game_coin") then
				self.wealth_txt.text = StringHelper.ToCash(MainModel.UserInfo.jing_bi - (cost - GameItemModel.GetItemCount("prop_tiny_game_coin")) - self.all_money)--刷新金币显示
				self.coin_txt.text = 0
			else
				self.wealth_txt.text = StringHelper.ToCash(MainModel.UserInfo.jing_bi - self.all_money)--刷新金币显示
				self.coin_txt.text = StringHelper.ToCash(GameItemModel.GetItemCount("prop_tiny_game_coin") - cost)--刷新小游戏币显示
			end--]]
--[[			self.wealth_txt.text = StringHelper.ToCash(MainModel.UserInfo.jing_bi)--刷新金币显示
			self.coin_txt.text = StringHelper.ToCash(GameItemModel.GetItemCount("prop_tiny_game_coin"))--刷新小游戏币显示--]]
		else
...
function C:on_Shoot_Down()
	if self.btn_can_use1 then
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		if  TTLModel.Defines.game_power[self.game_power_id] > MainModel.GetMiniGameCoinAndJingBiAllNum() then
	        GameButtonManager.RunFun({ gotoui="sys_jjj"}, "CheckAndRunJJJ", function ()
	            Event.Brocast("show_gift_panel")
	        end)
			return
		...
		self.seq4:AppendCallback(function ()
			local condi_key = TTLModel.DW_config.auto[self.game_power_id].key
...
function C:on_Shoot_Up()
	if self.btn_can_use1 then
		...
		if not self.is_Auto then
			local condi_key = TTLModel.DW_config.auto[self.game_power_id].key
			local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key= condi_key, is_on_hint = false}, "CheckCondition")
		    if a and not b then
		        return
	    	end
			self.is_Auto=false
...
function C:updateAssetInfoHandler(data)
    ...
    if data.change_type then
    	if (data.change_type == "ttl_game_award") or (data.change_type == "task_award_no_show") then
