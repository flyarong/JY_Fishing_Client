...
function TTLLogic.change_panel(panelName, pram)
    ...
    if not cur_panel then
        if panelName == TTLLogic.panelNameMap.hall then
            ...
            GameManager.GotoSceneName(GLC.ttl_back_scene or "game_MiniGame")
...
function TTLLogic.on_tantanle_enter_game_response()
    print("on_tantanle_enter_game_response","<color=blue>0000000000000000000000000000</color>")
    SendRequestAllInfo()
end
...
function TTLLogic.Init(pram)
    ...
    if not pram then
        ...
        Network.SendRequest("tantanle_enter_game",nil,"",function (data)
            dump(data)
        end)
    end
...
function TTLLogic.guide_goto_scene(call)
