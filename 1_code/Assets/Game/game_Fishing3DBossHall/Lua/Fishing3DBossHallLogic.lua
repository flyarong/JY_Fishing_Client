-- 创建时间:2020-07-23
local cur_path = "Game.game_Fishing3DBossHall.Lua."
ext_require(cur_path .. "Fishing3DBossHallModel")
ext_require(cur_path .. "Fishing3DBossHallGamePanel")

Fishing3DBossHallLogic = {}
local L = Fishing3DBossHallLogic
L.panelNameMap = {
    game = "game",
    hall = "hall"
}
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>Fishing3DBossHallLogic Init parm</color>")
    --初始化model
    local model = Fishing3DBossHallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  Fishing3DBossHallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = Fishing3DBossHallGamePanel.Create()}
        end
    end
end
