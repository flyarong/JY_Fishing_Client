...
function C:MakeLister()
    ...
    self.lister["fsqmg_match_rank_data_response"] = basefunc.handler(self, self.fsmg_match_rank_data_response)
end
...
function C:MyRefresh()
	...
	if self.parm.game_type == "qys" then
		...
		self.game_id = FishingManager.GetLastMatchGameID()
	end

	local cfg
	if self.parm.game_type == "qys" then
		cfg = FishingManager.GetLastQysToConfig()
	else
		cfg = FishingManager.GetLastDjsToConfig()
	end
	if cfg then
		-- 20分钟一场比赛
...
function C:RefreshRankInfo()
	if self.parm.game_type == "qys" then
		Network.SendRequest("fsqmg_match_rank_data",  {index = self.query_index, id = self.game_id}, "请求数据")
	else
	...
	if data.result == 0 and data.my_rank then
		...
		if data.my_rank.rank == -1 then
			...
			if data.my_rank.award_hb and data.my_rank.award_hb > 0 then
				self.my_award_txt.text = StringHelper.ToRedNum(data.my_rank.award_hb) .. "福利券"
...
function C:fsmg_match_rank_data_response(_, data)
	...
	if data.result == 0 then
		...
		if self.parm.game_type == "qys" then
			FishingManager.CacheQYSRankData(data, self.query_index)
		else
		...
		local data
		if self.parm.game_type == "qys" then
			data = FishingManager.GetQYSRankData(self.query_index)
		else
			data = FishingManager.GetRankData(self.query_index)
		end
...
function C:CreateRankItems(data)
	...
	for i,v in ipairs(data) do
		...
		if v.award_hb and v.award_hb > 0 then
			...
			t.award_txt.text = StringHelper.ToRedNum(v.award_hb) .. "福利券"
		else
...
function C:OnLookJSClose()
	local cfg
	if self.parm.game_type == "qys" then
		cfg = FishingManager.GetQYSCfgByRank(self.game_id, self.my_rank.rank)
	else
		cfg = FishingManager.GetCfgByRank(self.game_id, self.my_rank.rank)
	end
	if cfg then
		local ew = cfg.fixed_value or 0
		ew = (self.my_rank.award_hb - ew) * 100
		local reward
		if ew > 0 then
			reward = { [1]={asset_type="shop_gold_sum", value=ew} }
		end
	    local parm = {}
	    parm.game_type = self.parm.game_type
