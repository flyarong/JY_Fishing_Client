...
function C:UpdateTransform(pos, r, time_elapsed)
	...
	if not (self.fish_cfg and self.fish_cfg.close_rota and self.fish_cfg.close_rota == 1) then
		...
		if self.face_obj then
			...
			self.face_obj.prefab.gameObject.transform.localPosition = Vector3.New(0,0,-10)
		end
	end
	...
	if self.is_one_pos then
		...
		if isDebug and self.fish_cfg.id == 20 then
			print("<color=red><size=16>EEEEEEEE ooooooooooooo create </size></color>")
			print("x=" .. self.transform.localPosition.x .. "   y=" .. self.transform.localPosition.y)
		end
...
function C:MyExit(b)
	if isDebug and self.fish_cfg.id == 20 then
		print("<color=red><size=16>EEEEEEEE ooooooooooooo </size></color>")
	...
	self.transform.localScale = self.scale_old
...
function C:ctor(parent, data, parm, is_game_create)
	...
	self:MakeLister()
	self:AddMsgListener()
	self.scale_old = self.transform.localScale
	...
	if isDebug and self.fish_cfg.id == 20 then
		...
		dump(self.data)
		dump(self.parm)
		dump(FishingModel.data.system_time)
		dump(FishingModel.data.begin_time)
	end
end
...
function C:ChangeLayer(obj, scale, isUp)
	...
	local canvas = obj.gameObject:GetComponentsInChildren(typeof(UnityEngine.Canvas))
	...
	for i = 0, canvas.Length - 1 do
		canvas[i].sortingOrder = canvas[i].sortingOrder + max
	end
end
function C:InitUI()
	self:SetBox2D(true)
...
function C:ShowDead(_dead_index, call)
	...
	self.anim_time = Timer.New(function ()
		...
		if scale then
			local s = scale * b + self.scale_old.x
