...
function C:MakeLister()
    ...
    self.lister["fsqmg_match_gaming_rank_data_response"] = basefunc.handler(self, self.fsmg_match_gaming_rank_data_response)
    self.lister["fsqmg_my_match_gaming_rank_data_response"] = basefunc.handler(self, self.fsmg_my_match_gaming_rank_data_response)
end
...
function C:ctor(parm)
	...
	if self.parm.game_type == "qys" then
		config = FishingManager.GetQYSGameIDToAward(self.parm.game_id)
	else
...
function C:RefreshRankInfo()
	if self.parm.game_type == "qys" then
		Network.SendRequest("fsqmg_match_gaming_rank_data",  {index = self.data.query_index},"请求数据")
	else
...
function C:RefreshRankInfoMy()
	...
	if self.parm.game_type == "qys" then
		Network.SendRequest("fsqmg_my_match_gaming_rank_data", nil, "请求数据")
	else
