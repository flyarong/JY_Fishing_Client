...
function C:UpdateTransform(pos, r, time_elapsed)
	self.fish_base:UpdateTransform(pos, r, time_elapsed)
end
...
function C:ctor(parent, data, is_game_create)
	...
	if self.fish_cfg.id == 27 then
		parm.fish_list = {"node/box1", "node/box2", "node/box3", "node/box5", "node/glow", "node/jb1", "node/fish_pp"}
		parm.fish_tran = "box_size"
		parm.fish_anim = "node"
	elseif self.fish_cfg.animator_node then
		parm.fish_list = {"fish3d/fish"}
		parm.fish_tran = "fish3d/fish"
		parm.fish_shadow = "fish3d/fish/shadow"		
	else
		parm.fish_list = {"fish"}
		parm.fish_tran = "fish"
		parm.fish_shadow = "fish/shadow"
	end
...
function C:UpdateStatus(b)
	if self.data.status and self.data.status > 0 then
		...
		if self.fish_cfg.id == 28 then
			if b then
				self.fish_base.fish[1].transform.localScale = Vector3.New(m_scale, m_scale, m_scale)
				return
			end

			ExtendSoundManager.PlaySound(audio_config.by.bgm_by_siwang28.audio_name)
			self.bianda_seq = DoTweenSequence.Create()
			self.bianda_seq:Append(self.fish_base.fish[1].transform:DOScale(m_scale, 2):SetEase(DG.Tweening.Ease.InQuint))
			self.bianda_seq:AppendCallback(function ()
				VehicleManager.PlaySpeedUp(self.data.fish_id, 4)
			end)
			self.bianda_seq:AppendInterval(0.3)
			self.bianda_seq:OnKill(function ()
				VehicleManager.PlaySpeedUp(self.data.fish_id, 1)
				self.bianda_seq = nil
			end)
		else
		end
