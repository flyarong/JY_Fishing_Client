...
function OperatorActivityLogic.Init()
	if true then return end
    OperatorActivityLogic.Exit()
    this = OperatorActivityLogic
    MakeLister()
    AddLister()
    return this
end
function OperatorActivityLogic.Exit()
	if this then
		model.Exit()
		model = nil
		RemoveLister()
...
--正常登录成功
function OperatorActivityLogic.OnLoginResponse(result)
    if result==0 then
    	if model then
    		model.Exit()
    	end
...
function OperatorActivityLogic.OnExitScene()
	if cur_panel then
		cur_panel.instance:MyClose()
		cur_panel=nil
	end
end
function OperatorActivityLogic.open_operator_activity(panelName)
	if panelName then
		OperatorActivityLogic.change_panel(panelName)
	end
end

function OperatorActivityLogic.close_operator_activity(panelName)
	if panelName then
		if cur_panel then
...
function OperatorActivityLogic.change_panel(panelName)
	if cur_panel then
		if cur_panel.name == panelName then
	...
	if not cur_panel then
		...
		elseif panelName == panelNameMap.ls then
			cur_panel = {name=panelName, instance=ActivityConsecutiveWin.Create()}
...
function OperatorActivityLogic.CloseCurPanel()
	if cur_panel then
		cur_panel.instance:MyClose()
		cur_panel=nil
	end
...
function OperatorActivityLogic.IsShowPanel(name)
...
function OperatorActivityLogic.GetCurPanel()
	if cur_panel then
...
function OperatorActivityLogic.GetData()
	return m_data
end

--活动数据改变
function OperatorActivityLogic.on_activity_fg_activity_data_msg(data)
	...
	if data then
		if m_data and data.activity_data then
			...
			OperatorActivityLogic.activity_fg_all_info(m_data)
		end
...
--模式
function OperatorActivityLogic.activity_fg_all_info( data)
	...
	if data.activity_data and #data.activity_data > 0 then
		...
		print("<color=green>活动id:</color>",activity_id,OperatorActivityLogic.curActivityId)
		if OperatorActivityModel.IsActivated(m_data.game_id, activity_id) then
			if OperatorActivityLogic.curActivityId and activity_id ~= OperatorActivityLogic.curActivityId then
				OperatorActivityLogic.CloseCurPanel()
			end

			OperatorActivityLogic.curActivityId = activity_id
...
function OperatorActivityLogic.activity_fg_gameover_msg()
	print("<color=yellow>activity_fg_gameover_msg</color>")
    Event.Brocast("logic_activity_fg_gameover_msg")
end

function OperatorActivityLogic.activity_fg_close_clearing()
	print("<color=yellow>activity_fg_close_clearing</color>")
	if cur_panel and m_data and OperatorActivityLogic.curActivityId and not OperatorActivityModel.IsActivated(m_data.game_id, OperatorActivityLogic.curActivityId) then
		OperatorActivityLogic.CloseCurPanel()
	else
...
function OperatorActivityLogic.IsBigUI()
	if cur_panel then
...
function OperatorActivityLogic.CanLeaveGameBeforeEnd(showHint, cb)
	local ret = true
	if cur_panel and cur_panel.name == panelNameMap.ls and ActivityConsecutiveWin.CanBeAwarded() and OperatorActivityModel.IsActivated(m_data.game_id, ActivityType.Consecutive_Win) then
		-- 需求：匹配场退出时不再弹出挑战提示界面
		-- if showHint then
	...
	elseif cur_panel and cur_panel.name == panelNameMap.djhb and OperatorActivityModel.IsActivated(m_data.game_id, ActivityType.DuiJuHongBao) then
		...
		if showHint and haveTimes then
			if nn > 0 then
				GameExitHintPanel.Create("确认离开吗？", "再胜" .. nn .. "场即可抽奖，高概率获得福利券奖励！", cb)
			elseif canBeAwarded then
				GameExitHintPanel.Create("您有一次抽奖机会", "确认要返回大厅吗？", cb)
			else
				GameExitHintPanel.Create("确认离开吗？", "再胜几场即可抽奖，高概率获得福利券奖励！", cb)
			end
		end
		ret = false or not haveTimes
	elseif cur_panel and cur_panel.name == panelNameMap.cs and OperatorActivityModel.IsActivated(m_data.game_id, ActivityType.TianJiangCaiShen) and OperatorActivityLogic.CheckCS() then
		if showHint then
...
function OperatorActivityLogic.CheckCS()
	if m_data and m_data.activity_data and #m_data.activity_data > 0 then
		local have_cs = false
...
function OperatorActivityLogic.CheckCSActivity()
