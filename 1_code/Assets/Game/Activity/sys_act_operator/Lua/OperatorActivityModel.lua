...
local function InitMatchData()
    OperatorActivityModel.data={
    }
    m_data = OperatorActivityModel.data
end

function OperatorActivityModel.Init()
    this = OperatorActivityModel
    InitMatchData()
    MakeLister()
    AddLister()
    this.InitUIConfig()
...
function OperatorActivityModel.Exit()
    if this then
        RemoveLister()
        lister=nil
        this=nil
...
function OperatorActivityModel.InitUIConfig()
	this.Config = {}
...
-- 根据匹配场ID获取活动列表
function OperatorActivityModel.GetActivityConfig(activity_id)
...
-- 根据匹配场ID获取活动列表
function OperatorActivityModel.GetActivityList(id)
	if not StringHelper.is_double_week() then
...
-- 根据匹配场ID和活动ID获取活动
function OperatorActivityModel.GetActivity(id, activity_id)
	local data = OperatorActivityModel.GetActivityList(id)
	if data then
...
-- 根据匹配场ID获取当前进行中的活动
function OperatorActivityModel.GetActivityUnderway(id)
...
function OperatorActivityModel.IsActivated(gameId, activityId)
	local ret = false
	local ac = OperatorActivityModel.GetActivityConfig(activityId)
	if ac then
		local config = OperatorActivityModel.GetActivity(gameId, activityId)
...
function OperatorActivityModel.GetActivatedActivityList()
	local data = {}
	if this then
		...
		for _, d in ipairs(data) do
			...
			for _, list in ipairs(fa) do
				for _, act in ipairs(list.activity_list) do
					if act.activity_id == d.id and OperatorActivityModel.IsActivated(list.game_id, act.activity_id) then
						isActivated = 1
						break
					end
				end
	...
	--dump(ac, "<color=yellow>OperatorActivityModel.GetActivatedActivityList:</color>")
	--dump(fa, "<color=yellow>OperatorActivityModel.GetActivatedActivityList:</color>")
	--dump(data, "<color=yellow>OperatorActivityModel.GetActivatedActivityList: isDoubleWeek:" .. (StringHelper.is_double_week() and "Yes" or "No") .. "</color>")

	return data
end

function OperatorActivityModel.IsBannerShown()
	return isBannerShown
end

function OperatorActivityModel.SetShowBannerDone()
	isBannerShown = true
end

-- 根据匹配场ID获取对应活动的状态
function OperatorActivityModel.GetActivityStateByFreeID(id)
	...
	local list = OperatorActivityModel.GetActivityList(id)
	local curt = os.time()
	local onet = StringHelper.getThisWeekMonday()
