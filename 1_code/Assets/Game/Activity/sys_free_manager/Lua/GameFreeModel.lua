...
local fish_game_config = SysFishingManager.fish_hall_config
...
function GameFreeModel.GetGameIDToConfig(gameid)
    ...
    dump(gameid, "<color=red>GetGameIDToConfig gameid </color>")
end
...
function GameFreeModel.GetGameTypeToConfig(game_type)
    ...
    dump(game_type, "<color=red>GetGameIDToConfig game_type </color>")
end
-- 判断是否能进入
...
function GameFreeModel.on_fg_signup_response(_, data)
    ...
    if data.result == 0 then
        ...
        -- local v = GameFreeModel.GetGameIDToConfig(data.game_id)
        -- local game_type = MainModel.ServerToClientScene[v.game_type]
        -- HintPanel.Create(1, errorCode[data.result] , function() 
        --     GameManager.GotoSceneName("game_Free", {game_type=game_type, game_id=v.game_id} )
        -- end)

        HintPanel.ErrorMsg(data.result,function(  )
            GameManager.GotoSceneName("game_Free")
        end)
    end
end
...
function GameFreeModel.SendRedAward()
    if m_data.all_info.store_award == 0 and m_data.all_info.today_award >= m_data.all_info.today_max_award then
        local mm = StringHelper.ToCash(m_data.all_info.today_max_award)
        LittleTips.Create("今日对局福利券已领满" .. mm .. "，请明日再来")
    elseif m_data.all_info.store_award > 0 then
        Network.SendRequest("fg_get_hongbao_award", nil, "领取奖励")
    else
        LittleTips.Create("对局进度不足，任意匹配场打满5局可领取对局福利券！")
    end
end
-- 对局奖励
...
function GameFreeModel.CheckRapidBeginGameID ()
    local gametypeid = PlayerPrefs.GetInt(MainModel.FreeRapidBeginKey, 13)
    ...
    if not gameCfg or (gameCfg and gameCfg.isOnOff and gameCfg.isOnOff == 0) then
        PlayerPrefs.SetInt(MainModel.FreeRapidBeginKey, 13)
        gameCfg = GameFreeModel.GetGameIDToConfig(13)
    end
...
function GameFreeModel.RapidBeginGame()
    ...
    local xsyd_signup = function ()
        ...
        GameManager.GotoSceneID( GameFreeModel.data.sceneID , true , nil , function()
            Network.SendRequest("fg_signup", {id = this.data.gameid, xsyd = 1}, "正在报名")
        end)
    end
    local signup = function()
        --- 先跳场景，再报名
        GameManager.GotoSceneID( GameFreeModel.data.sceneID , true , nil , function()
            Network.SendRequest("fg_signup", {id = this.data.gameid}, "正在报名")
        end)
    end
    local check
    check = function ()
...
function GameFreeModel.CheckRapidBeginGameIDLevel(level)
    local gametypeid = PlayerPrefs.GetInt(MainModel.FreeRapidBeginKey, 14)
    local gameCfg = GameFreeModel.GetGameIDToConfig(gametypeid)
    if not gameCfg or gameCfg.isOnOff == 0 then
        PlayerPrefs.SetInt(MainModel.FreeRapidBeginKey, 14)
        gameCfg = GameFreeModel.GetGameIDToConfig(14)
    end
    return GameFreeModel.GetRapidBeginGameIDLevel(gameCfg.game_type,level)
end

-- 快速开始游戏
function GameFreeModel.RapidBeginGameLevel(t_level)
