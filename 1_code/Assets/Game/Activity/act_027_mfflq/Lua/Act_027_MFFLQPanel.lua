...
Act_027_MFFLQPanel = basefunc.class()
local C = Act_027_MFFLQPanel
C.name = "Act_027_MFFLQPanel"
...
function C:MakeLister()
	...
	self.lister["027MFFLQ_query_gift_bag_status_msg"] = basefunc.handler(self,self.on_027MFFLQ_query_gift_bag_status_msg)
	self.lister["main_query_gift_bag_data_msg"] = basefunc.handler(self,self.on_027MFFLQ_query_gift_bag_status_msg)
end
...
function C:OnFinshName(animationName, startPos)
	...
	if animationName == "Step2" then
		self:StopAllAnim()
		local award_index = self.award_id - M.award_index + 1
...
function C:on_027MFFLQ_query_gift_bag_status_msg()
	self:MyRefresh()
end
