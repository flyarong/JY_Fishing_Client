...
function C:ctor(data)
	dump(data, "Fishing3DActZhuanpanPanel init!")
...
function C:initTimeoutTimer()
	...
	timeoutTimer = Timer.New(function()
		...
		if self.curTimeout <= 0 then
			...
			self.lottery_txt.transform.localPosition = Vector3.New(0,self.lottery_txt.transform.localPosition.y,self.lottery_txt.transform.localPosition.z)
			self:OnLotteryBtnClick()
		end

		self.timeout_txt.text = "(" .. self.curTimeout .. "s)"
	end, 1, -1, nil, true)
