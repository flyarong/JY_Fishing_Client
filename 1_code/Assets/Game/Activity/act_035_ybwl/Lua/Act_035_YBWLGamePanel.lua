...
Act_035_YBWLGamePanel = basefunc.class()
local C = Act_035_YBWLGamePanel
C.name = "Act_035_YBWLGamePanel"
local M = Act_035_YBWLManager
local help_info = {
"一本万利规则：",
"1.购买一本万利后，即激活礼包任务。",
"2.每期共有8个礼包可购买，其中王者与荣耀礼包可多次购买，其余礼包每个周期(十天)限购一次。",
"3.可同时购买激活所有礼包，获得最大收益。",
"4.每个礼包在购买激活后才会累计任务进度。",
}
...
function C:CreateItemPrefab()
	...
	for i=1,#self.BaseCfg do
		local pre = Act_035_YBWLLeftPage.Create(self,self.Content_left.transform,i,self.BaseCfg[i])
...
function C:CreateRightPrefab()
	...
	for i=1,7 do
		local pre = Act_035_YBWLItemBase.Create(self,self.Content_right.transform,i,data[self.index])
...
function C:CheckDJS()
	if M.CheckGiftWasBought(self.BaseCfg[self.index].gift_id) and self.BaseCfg[self.index].gift_id ~= 10390 and self.BaseCfg[self.index].gift_id ~= 10391 then--买过了
		self.value_time = M.GetGiftValueTime() - os.time()
		self.djs.gameObject:SetActive(true)
		self:StartDJSTimer(true)
	else
		self.djs.gameObject:SetActive(false)
	end
end
