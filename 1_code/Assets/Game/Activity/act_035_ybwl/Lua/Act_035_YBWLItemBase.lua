...
Act_035_YBWLItemBase = basefunc.class()
local C = Act_035_YBWLItemBase
C.name = "Act_035_YBWLItemBase"
...
function C:InitUI()
	...
	self.taskdesc_txt.text = StringHelper.ToCash(self.config.need_num[self.index])
...
function C:OnGoClick()
	if M.CheckGiftWasBought(self.config.gift_id) then--买了
		...
		LittleTips.Create("您当前还未购买此礼包,无法完成此累计消耗任务")
	end
end
