...
--
local qudao_tips = {
	[11] = "\n3.成为VIP11立即获得30次至尊礼包领取资格，每日可领1次;\n4.每获得4点财富值增加1次至尊礼包领取资格，上限100次。\n5.成为V11立即获得30次尊享礼包领取资格，每日可领1次。\n6.每累计4点财富值增加1次尊享礼包领取资格，上限100次。",
	[12] = "\n3.成为VIP12立即获得30次至尊礼包领取资格，每日可领1次;\n4.每获得5点财富值增加1次至尊礼包领取资格，上限100次。\n5.成为V12立即获得30次尊享礼包领取资格，每日可领1次。\n6.每累计5点财富值增加1次尊享礼包领取资格，上限100次。"
}
local normal_tips = {
	[11] = "\n3.3D捕鱼、街机打鱼赢金数据按50%计入累计赢金得财富值;\n4.成为V11立即获得30次尊享礼包领取资格，每日可领1次。\n5.每累计2点财富值增加1次尊享礼包领取资格，上限100次。",
	[12] = "\n3.3D捕鱼、街机打鱼赢金数据按50%计入累计赢金得财富值;\n4.成为V12立即获得30次尊享礼包领取资格，每日可领1次。\n5.每累计3点财富值增加1次尊享礼包领取资格，上限100次。"
}

local VIPLevel = {"A","B","C","D","E","F","G","H","I","J","K","L"}
...
function C:OnGetInfo(_,data)
	if IsEquals(self.gameObject)  and data then 	
		...
		self.CurVIPText.text = VIPLevel[self.showlevel]
		if data.vip_level==VIPManager.GetUserMaxVipLevel() then 
			...
			if config.dangci[self.showlevel+1].total then
				...
				self.tips_txt.text = "每赢金1000万加1点财富值"
				self.MoneyRect.gameObject:SetActive(false)
			end
		...
		if self.showlevel >= VIPManager.GetUserMaxVipLevel() then
...
function C:SetRightAnim()
	...
	if self.index==VIPManager.GetUserMaxVipLevel()  then
		self.RightButton.gameObject:SetActive(false)
	end
	if   1 <self.index and  self.index < VIPManager.GetUserMaxVipLevel()  then
...
function C:InitChild()
	...
	for i = 1, #config.dangci do
		...
		PointerEventListener.Get(tip.gameObject).onDown = function ()
			local str = "1.游戏内带有超值标签的礼包不计入vip累计金额;\n2.福利券超出容量后会自动转换为金币。"
		...
		for j = 1, #config.dangci[i].info do
			...
			bb.transform:Find("Text3"):GetComponent("Text").text = j
			bb.transform:Find("Text"):GetComponent("Text").text = cc.desc--string.sub(cc.desc,5,#cc.desc)

			bb.transform:Find("Text"):GetComponent("Text").color = COLOR_VIPDanLS_Color
			
			local qw = bb.transform:Find("Text/QWButton"):GetComponent("Button")
			if (cc.gotoUI and #cc.gotoUI > 0) or cc.gotoShop then
				qw.gameObject:SetActive(true)
				qw.onClick:AddListener(function ()
					ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
					self:MyExit()
					if (cc.gotoUI and #cc.gotoUI > 0) then
						local gotoparm = {gotoui=cc.gotoUI[1], goto_scene_parm=cc.gotoUI[2],data="vip"}
			            GameManager.GuideExitScene(gotoparm)
			        elseif cc.gotoShop then
			        	MainModel.OpenDH()
			        end
				end)
			else
