...
Act_005_YMFLManager = basefunc.class()
local M = Act_005_YMFLManager
M.key = "act_005_ymfl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_005_ymfl_config")
local lister
local task_ids
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21205", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
