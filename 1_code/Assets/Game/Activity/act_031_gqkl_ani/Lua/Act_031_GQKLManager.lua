-- 创建时间:2020-06-09
-- Act_031_GQKLManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_031_GQKLManager = {}
local M = Act_031_GQKLManager
M.key = "act_031_gqkl_ani"
...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and data.change_type == "task_award_1000069" then
...
function M.PrefabCreator(value)
    ...
    local obj = newObject("Act_031_GQKLPrefab",GameObject.Find("Canvas/LayerLv5").transform)
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_award_1000069" == type  then
            return true
        end
    end
