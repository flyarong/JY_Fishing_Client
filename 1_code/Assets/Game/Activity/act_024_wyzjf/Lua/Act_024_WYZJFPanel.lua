...
Act_024_WYZJFPanel = basefunc.class()
local C = Act_024_WYZJFPanel
C.name = "Act_024_WYZJFPanel"
...
function C:InitUI()
	self.phb_btn.onClick:AddListener(
		function ()
			ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
			ActivityYearPanel.Create(nil,nil,{ID = 1})
		end
	)
		...
		function ()
			...
			local reset_func = function()
				...
				if PlayerPrefs.GetInt(MainModel.UserInfo.user_id..M.key.."renwu"..os.date("%Y%m%d",os.time()),0) == 0 then
					...
					b:ChangeTitleImg("wyzjf_imgf_czrw")
					b:ShowGou()
...
function C:MyRefresh()
	self:RefreshSX()
	self:RefreshItems()
	self.curr_txt.text = "当前积分："..(Act_024_JFCDJManager.GetMyJiFen() or 0)
end
...
function C:CreateTaskItem(task_id,ui_data)
	...
	temp_ui.recharge_btn.onClick:AddListener(
		function()
			ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
			local key = 0
			if ui_data.goto_ui == "game_Eliminate" then
				key = "sgxxl_level"
				local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key = key, is_on_hint = true}, "CheckCondition")
			    --dump({a = a,b = b},"<color=yellow>++++++++++++++++a+++++++++++++++++++</color>")
			    if a and not b then
			        return
			    end
				...
				if not Network.SendRequest("xxl_enter_game", nil ,"正在进入") then
		                HintPanel.Create(1, "网络异常", function()
		                    GameManager.GotoSceneName("game_MiniGame",basefunc.handler(self,self.MyExit))
		                end)
					end
				end},ActivityYearPanel.Close())
			elseif ui_data.goto_ui == "game_EliminateSH" then
				key = "shxxl_level"
				local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key = key, is_on_hint = true}, "CheckCondition")
			    --dump({a = a,b = b},"<color=yellow>++++++++++++++++a+++++++++++++++++++</color>")
			    if a and not b then
			    ...
			    GameManager.GuideExitScene({gotoui="game_EliminateSH", goto_scene_parm=true, enter_scene_call=function()
			...
			--dump({goto_ui = ui_data.goto_ui,key = key},"<color=yellow>++++++++++++++++key+++++++++++++++++++</color>")		
		end
	)
...
function C:RefreshSX()
	...
	local func = function ()
		...
		if offtime >= 5 then			
			local t = M.GetRefreshTime()
			--dump({offtime= offtime,t=t},"<color=red>+++++++++++++++++++///++++++++++++++++</color>")
			if t < 3 then
				self.xh_txt.text = (t).."/3".."\n免费"
			else
				self.xh_txt.text = "消耗\n2000金币"
			end	
		else
...
function C:RefreshItems()
	...
	for i = 1,#t do
		...
		if data then
			temp_ui.bar.transform.sizeDelta = Vector2.New(data.now_process/data.need_process*276,42)
