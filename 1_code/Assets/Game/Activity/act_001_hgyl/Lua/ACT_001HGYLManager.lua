...
ACT_001HGYLManager = basefunc.class()
local M = ACT_001HGYLManager
M.key = "act_001_hgyl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_ymfl_regression_config")
local lister
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21159", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
