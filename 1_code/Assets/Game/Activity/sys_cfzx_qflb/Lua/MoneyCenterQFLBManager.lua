...
function M.on_qflb_qysclear_uichange(data)
    if data and data.panelSelf then 
        if M.IsShowQFLBEnterInQYSClear() and IsEquals(data.panelSelf.gameObject)  then 
            ...
            if data.panelSelf.win_one_more_btn.gameObject.activeSelf and data.panelSelf.game_type == "gms" then 
                data.panelSelf.qflb_btn.gameObject:SetActive(true)
                --data.panelSelf.win_one_more_btn.gameObject.transform.localPosition = Vector2.New(-299,-435)
            end 
        else
