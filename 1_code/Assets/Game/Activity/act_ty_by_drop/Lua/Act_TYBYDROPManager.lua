    ...
    if true then
        ...
        seq:OnKill(function ()
            if IsEquals(obj) then 
                ...
                if IsEquals(temp_ui.icon_img) then
                    temp_ui.icon_img.gameObject:SetActive(false)
                end
                if IsEquals(temp_ui.glow_01) then
                    temp_ui.glow_01.gameObject:SetActive(false)
                end
                if IsEquals(temp_ui.num_txt) then
...
function M.CalcCurStylePath()
    local tab = {}
    local dd_cfg = this.UIConfig.style_list[#this.UIConfig.style_list] -- 默认选择最后一个
    tab.cur_style = dd_cfg.path
    tab.item_key = dd_cfg.item_key
    local cur_t = os.time()
    for k,v in ipairs(this.UIConfig.style_list) do
        if v.beginTime <= cur_t and cur_t <= v.endTime then
            ...
            if a and b then
                tab.cur_style = v.path
                tab.item_key = v.item_key
                break
            end
        end
    end
...
function M.UpdateAllModuleConfig()
    local tab = M.CalcCurStylePath()
    local style_path = tab.cur_style
    local item_key = tab.item_key or "prop_fish_drop_act_0"
    ...
    dump(cfg, "<color=red><size=16>|||||||||||||||||||||||||</size></color>")
end
