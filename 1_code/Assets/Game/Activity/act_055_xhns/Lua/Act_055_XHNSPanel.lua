...
Act_055_XHNSPanel = basefunc.class()
local C = Act_055_XHNSPanel
local M = Act_055_XHNSManager
C.name = "Act_055_XHNSPanel"
...
function C:ctor(parent)
	...
	self.view = self.transform:Find("View/Viewport/content")
...
function C:InitContentPre()
	self.content_pre = {}
	for i = 1, #self.cfg do
		local b = newObject("Act_055_XHNSItem", self.content_2.transform)
		local b_ui = {}
...
function C:RefreshContentUI()
	...
	for i = 1, #self.cfg do
		...
		if self.data.award_status[i] == 1 and is_check then
			is_check = false
			self.view.localPosition = Vector3.New(- ((i - 1) * 230),self.view.localPosition.y,self.view.localPosition.z) 
		end
		if self.data.award_status[i] == 2 and is_check then
			self.view.localPosition = Vector3.New(- ((i - 1) * 230),self.view.localPosition.y,self.view.localPosition.z) 
		end
...
function C:GetCurHelpInfor()
	...
	if a and b then
		...
		"1.每在游戏中消耗10万金币获取1积分。",
		"2.每日0点重置积分和奖励。",
		"3.苹果大战不计入数据统计，捕鱼类游戏及疯狂捕鱼消耗只计50%。"}
	else
		help_desc = {
		"1.每在游戏中消耗10万金币获取1积分。",
		"2.每日0点重置积分和奖励。",
		"3.苹果大战不计入数据统计，疯狂捕鱼消耗只计50%。"}
	end
