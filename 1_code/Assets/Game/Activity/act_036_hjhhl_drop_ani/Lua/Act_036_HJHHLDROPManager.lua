-- 创建时间:2020-06-09
-- Act_036_HJHHLDROPManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_036_HJHHLDROPManager = {}
local M = Act_036_HJHHLDROPManager
M.key = "act_036_hjhhl_drop_ani"
...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and data.change_type == "task_award_1000116" then
...
function M.PrefabCreator(value)
    ...
    local obj = newObject("Act_036_HJHHLDROPPrefab",GameObject.Find("Canvas/LayerLv5").transform)
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_award_1000116" == type  then
            return true
        end
    end
