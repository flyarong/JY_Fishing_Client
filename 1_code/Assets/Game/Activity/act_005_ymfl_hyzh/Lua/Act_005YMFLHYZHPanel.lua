...
Act_005YMFLHYZHPanel = basefunc.class()
local M = Act_005YMFLHYZHPanel
M.name = "Act_005YMFLHYZHPanel"
local Mgr = Act_005YMFLHYZHManager
...
function M:InitUI()
	...
	if not table_is_null(Mgr.children_list) then
		for i,vv in ipairs(Mgr.children_list) do
			...
			local obj = newObject("Act_005YMFLHYZHPlayerItem", self.sv_content)
			self.cld_item[i] = {}
			...
			self.cld_item[i].zh_btn.onClick:AddListener(function()
				...
				if not table_is_null(v.gotoUI)  then
					goto_ui = v.gotoUI[1]
					if self.goto_scene_call and type(self.goto_scene_call) == "function" and GameManager.GetSceneNameByGotoui(goto_ui) then
						if GameManager.CheckActivityYear() then
							ActivityYearPanel.Close()
						else
							GameActivityPanel.Close()
						end
						self.goto_scene_call()
					else
						local gotoparm = {gotoui=goto_ui, goto_scene_parm=v.gotoUI[2]}
