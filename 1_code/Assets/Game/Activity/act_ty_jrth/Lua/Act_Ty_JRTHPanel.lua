...
Act_Ty_JRTHPanel = basefunc.class()
local C = Act_Ty_JRTHPanel
C.name = "Act_Ty_JRTHPanel"
...
function C:on_model_cjlb_unrealy_change_msg()
	...
	local award = self.config[M.GetUnrealyAwardName()].award_descs[M.GetUnrealyAwardID()] .. "金币"
	...
	self.seq1:OnKill(function ()
		if IsEquals(self.Lantern1_txt) then
			self.Lantern1_txt.text = self.Lantern2_txt.text
		end
		if IsEquals(self.Lantern) then
...
function C:CreateItemPrefab()
	...
	for i=1,#tab do
		local pre = Act_Ty_JRTHItemBase.Create(self.Content.transform,self.index,tab[i])
...
function C:SpecialHandling()
	local temp1 = self.spawn_cell_list[1].transform:GetSiblingIndex()
	local temp2 = self.spawn_cell_list[2].transform:GetSiblingIndex()
	local temp3 = self.spawn_cell_list[3].transform:GetSiblingIndex()
	local temp4 = self.spawn_cell_list[4].transform:GetSiblingIndex()
	local temp5 = self.spawn_cell_list[5].transform:GetSiblingIndex()
	local temp6 = self.spawn_cell_list[6].transform:GetSiblingIndex()
	self.spawn_cell_list[2].transform:SetSiblingIndex(temp4)
	self.spawn_cell_list[1].transform:SetSiblingIndex(temp2)
	self.spawn_cell_list[4].transform:SetSiblingIndex(temp1)
	self.spawn_cell_list[3].transform:SetSiblingIndex(temp6)
	self.spawn_cell_list[6].transform:SetSiblingIndex(temp3)
end
...
function C:on_model_cjlb_bx_change_msg(id)
	...
	if self.config[self.index].type == 3 then--钻石宝箱
		if remain == 3 then
			if self.config[self.index].lxq then
				self.one.gameObject:SetActive(true)
				self.one_txt.text = "每日首次最多赠"..self.config[self.index].lxq
			end
		else
			self.one.gameObject:SetActive(false)
		end
	else
		self.one.gameObject:SetActive(false)
	end
