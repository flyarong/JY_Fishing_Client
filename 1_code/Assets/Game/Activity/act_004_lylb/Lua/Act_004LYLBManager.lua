...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1584979199
    local s_time = 1584401400
...
function M.OnEnterScene()
    ...
    if MainModel.myLocation == "game_Match" then 
        if PlayerPrefs.GetInt(MainModel.UserInfo.user_id.."game_Match"..M.key..os.date("%Y%m%d", os.time()),0) == 0 then 
            --打开页面
            Act_004LYLBPanel.Create()
            PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."game_Match"..M.key..os.date("%Y%m%d", os.time()),1)
        end
    end
end
