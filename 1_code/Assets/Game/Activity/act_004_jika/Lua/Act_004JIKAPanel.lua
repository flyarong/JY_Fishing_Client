...
function C:InitUI()
	...
	if a and b then
		for i = 1,#M.config.base_cjj do 
			self["award"..i.."_img"].sprite = GetTexture(M.config.base_cjj[i].award_image)
			self["award"..i.."_txt"].text = M.config.base_cjj[i].award_text
		end
	else
	...
	self.quanfan_btn.onClick:AddListener(
		function ()
			self:GoToQuanFan()
		end
	)
...
function C:RefreshUI(data)
	...
	if data.total_remain_num > 0 then
	    ...
	    local quanfanstate=Act_022_QFLBManager.GetQuaterGiftBuyState()
		self.maihou.gameObject:SetActive(quanfanstate)
		self.meiJiKa.gameObject:SetActive(not quanfanstate)
	else
		self.maiqian.gameObject:SetActive(true)
		self.maihou.gameObject:SetActive(false)
		self.meiJiKa.gameObject:SetActive(false)
...
function C:getNameByID(award_id)
    ...
    if a and  b then
        for i = 1,#M.config.base_cjj do 
			if M.config.base_cjj[i].award_id == award_id then 
				return M.config.base_cjj[i].award_text
			end
		end  
...
function C:GoToQuanFan()
	dump("点击跳转！！！")
	local gotoui={"act_022_qflb","panel",}
	local parm = {gotoui="act_022_qflb",goto_scene_parm="panel",chooseData="Quarter"}
