...
function HallBannerWidget:Springback()
	...
	if jindian then
		...
		self.forSeq:OnForceKill(function ()
			self.isScrollLock = false
...
function HallBannerWidget:RefreshData(x)
	while(true)	do
		...
		elseif self.rightX < x then
			...
			self.rightX = self.rightX + self.CellSize.x
			self.leftIndex = self.leftIndex + 1
			if self.leftIndex > #BannerModel.data.hallBannerList then
				self.leftIndex = 1
			end
			self.rightIndex = self.rightIndex + 1
			if self.rightIndex > #BannerModel.data.hallBannerList then
				self.rightIndex = 1
			end
...
function HallBannerWidget:PlayAnim()
	...
	self.bannerSeq = DoTweenSequence.Create()
	...
	self.bannerSeq:OnComplete(function ()
		...
		if self.selectIndex > #BannerModel.data.hallBannerList then
			self.selectIndex = 1
		end
		self:UpdateSelect(self.selectIndex)
	end)
	self.bannerSeq:OnForceKill(function ()
		if IsEquals(self.Content) then
