...
function C:RefreshRankInfo()
	Network.SendRequest("query_buyu_rank",  {page_index = self.data.query_index},"请求数据",function(data)
...
function C:CreateRankItems(data)
	...
	for i,v in ipairs(data) do
		...
		t.num_txt.text = StringHelper.ToCash(tonumber(v.profit_num))
		...
		if v.player_id == MainModel.UserInfo.user_id then
			...
			t.num_img.sprite = GetTexture("bydrb_bg_jbzj")
...
function C:RefreshRankInfoMy()
	...
	Network.SendRequest("query_buyu_rank_base_info", {},"请求数据",function(data)
		...
		if data.result == 0 then
			...
			if data.rank == -1 then
				...
				if not award then
					...
					for k,v in ipairs(award) do
						if v.asset_type == "shop_gold_sum" then
							self.ui.my_award_txt.text = v.value
						else
				...
				if data.rank > 20 and tonumber(data.total_profit) >= 50000000 then
					self.ui.my_ew_award.gameObject:SetActive(true)
					self.ui.my_ew_award_txt.text = "1"
				end
			end
