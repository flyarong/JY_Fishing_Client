...
function C.Create(onlogin,backcall)
	return C.New(onlogin,backcall)
end
...
function C:ctor(onlogin,backcall)
...
function C:GetUpShopLevelAwards(data)
	if  data and data.money_value then
		...
		self.ALL_SJBC.text="剩余"..(self:GetTimes()*30000).."金币每天领取！"
		self.SJBC.text="金币 X"..data.money_value
		self.IFZero.text="剩余<color=yellow>"..(self:GetTimes()*30000).."</color>金币每天领取！"
