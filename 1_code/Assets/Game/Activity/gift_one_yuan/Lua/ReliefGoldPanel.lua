...
function C.Create()
	if MainModel.myLocation == "game_Hall"
		or string.sub(MainModel.myLocation, 1, 9) == "game_Fish" then
			local pre = GameManager.GotoUI({gotoui = "sys_jjj", goto_scene_parm = "panel"})
			if pre then
				return pre
			else
				return C.New()
...
function C:OnClick()
	SharePanel.Create("1YuanGift", "1YuanGift_share", function()
