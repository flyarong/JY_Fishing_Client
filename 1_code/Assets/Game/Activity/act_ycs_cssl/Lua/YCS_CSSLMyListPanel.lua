...
YCS_CSSLMyListPanel = basefunc.class()
local C = YCS_CSSLMyListPanel
C.name = "YCS_CSSLMyListPanel"
local Page_Index  =  1
function C.Create()
	return C.New()
end
...
function C:ctor()
	...
	self:AddMsgListener()
	self:InitUI()
	Network.SendRequest("common_lottery_query_my_award_logs",{lottery_type = "box_exchange_11",is_shiwu = 1})
end
...
function C:onGetInfo(_,data)
	...
	if data and data.result == 0 then
		local task_data = GameTaskModel.GetTaskDataByID(21125)
		if task_data then 
			if task_data.award_status == 2 then 
				data.award_logs[#data.award_logs + 1] = {award_name = "鲸鱼玩偶",count = 1}
			end 
		end  
