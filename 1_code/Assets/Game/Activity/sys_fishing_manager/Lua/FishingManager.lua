...
local fishing_match_hall_config = SysFishingManager.fishing_match_hall_config
...
local function MakeLister()
    ...
    lister["fsmg_query_game_id_response"] = this.on_fsmg_query_game_id_response
    lister["fsqmg_query_game_id_response"] = this.on_fsqmg_query_game_id_response
end
...
function C.QueryData()
    --NetMsgSendManager.SendMsgQueue("fsmg_query_game_id")
end
function C.QueryQYSData()
    --NetMsgSendManager.SendMsgQueue("fsqmg_query_game_id")
end

function C.on_fsmg_query_game_id_response(_, data)
    dump(data, "<color=red>on_fsmg_query_game_id_response</color>")
    this.data.game_id_result = data.result
    if data.result == 0 then
        this.data.game_id = data.game_id
        this.data.last_game_id = data.last_game_id
        Event.Brocast("model_fsmg_query_game_id_msg_finish")
    end
end
function C.on_fsqmg_query_game_id_response(_, data)
    dump(data, "<color=red>on_fsqmg_query_game_id_response</color>")
    this.data.qys_game_id_result = data.result
    if data.result == 0 then
        this.data.qys_game_id = data.game_id
        this.data.qys_last_game_id = data.last_game_id
        Event.Brocast("model_fsqmg_query_game_id_msg_finish")
    end
end
...
function C.InitConfig()
    ...
    -- 捕鱼千元赛
    C.Config.fish_match_qys_map = {}
    C.Config.fish_match_qys_list = {}
    C.Config.fish_match_qys_award = {}
    for k,v in ipairs(fishmatch_ui.qys_config) do
        C.Config.fish_match_qys_list[#C.Config.fish_match_qys_list + 1] = v
        C.Config.fish_match_qys_map[v.game_id] = v
    end
    for k,v in ipairs(fishmatch_ui.qys_award) do
        if not C.Config.fish_match_qys_award[v.award_id] then
            C.Config.fish_match_qys_award[v.award_id] = {}
        end
        C.Config.fish_match_qys_award[v.award_id][#C.Config.fish_match_qys_award[v.award_id] + 1] = v
    end

    C.Config.fish_match_hall_tag = {}
    for k,v in ipairs(fishing_match_hall_config.game) do
        if v.isOnOff == 1 then
            ...
            if not aa or bb then
                C.Config.fish_match_hall_tag[#C.Config.fish_match_hall_tag + 1] = v
            end
        end
    end
end
...
-- 街机捕鱼大厅配置
function C.GetPPIDToConfig(id)
    return C.Config.fish_hall_map[id]
end
-- 是否有奖励可领取
...
function C.SignFishing(config)
    if config.asset_type == "obj_fish_free_bullet" or
        ...
        if is_can then
            ...
            if result == 1 then
                LittleTips.Create("您的金币不足，请购买金币")
...
-- 获取当前大奖赛ID
function C.GetCurDjsID()
    local t = C.CalcMatchTime(C.Config.fish_match_list)
    return t.game_id
end
-- 获取最近的比赛 ID
function C.GetRecentlyGameID()
    return C.GetCurDjsID()
end
function C.GetLastMatchGameID()
    return C.GetCurDjsID()
end

function C.IsFirstMatchDJS()
    if not C.GetCurDjsID() then
        return true
    end
    return false
end
...
function C.GetGameIDToConfig(game_id)
    if C.Config.fish_match_map[game_id] then
        return C.Config.fish_match_map[game_id]
    end
    return C.Config.fish_match_map[1]
end
function C.GetGameIDToAward(award_id)
    if C.Config.fish_match_award[award_id] then
        return C.Config.fish_match_award[award_id]
    end
    return C.Config.fish_match_award[1]
end
...
-- 返回比赛消耗满足的道具
function C.GetMatchCanUseTool(cfg)
...
-- 捕鱼比赛大厅
function C.GetMatchHallConfigAndSort()
    local cfg = C.Config.fish_match_hall_tag
    MathExtend.SortList(cfg, "order", true)
    return cfg
end

-- 获取最近的比赛 ID
function C.GetQYSRecentlyGameID()
    local t = C.CalcMatchTime(C.Config.fish_match_qys_list)
    return t.game_id
end
function C.GetQYSLastMatchGameID()
    return C.GetQYSRecentlyGameID()
end
-- 捕鱼千元赛
function C.IsFirstMatchQYS()
    if not C.GetQYSLastMatchGameID() then
        return true
    end
    return false
end
-- 获取最近的比赛
function C.GetQYSRecentlyGameConfig()
    local t = C.CalcMatchTime(C.Config.fish_match_qys_list)
    return t
end

function C.GetQYSZHGameID()
    if C.Config.fish_match_qys_list then
        return C.Config.fish_match_qys_list[#C.Config.fish_match_qys_list].game_id
    end
end

-- 返回比赛消耗满足的道具
function C.GetMatchQYSCanUseTool(cfg)
    local itemkey = cfg.enter_condi_itemkey
    local item_count = cfg.enter_condi_item_count
    for k,v in ipairs(itemkey) do
        if GameItemModel.GetItemCount(v) >= item_count[k] then
...
function C.GetCurQysToConfig()
    local t = C.CalcMatchTime(C.Config.fish_match_qys_list)
    local cc = C.Config.fish_match_qys_map[t.game_id]
    cc = basefunc.deepcopy(cc)
    cc.start_time = t.start_time
    cc.over_time = t.over_time
    return cc
...
-- 上一场比赛配置
function C.GetLastQysToConfig()
    local t = C.CalcLastMatchTime(C.Config.fish_match_qys_list)
    local cc = C.Config.fish_match_qys_map[t.game_id]
    cc = basefunc.deepcopy(cc)
    cc.start_time = t.start_time
    cc.over_time = t.over_time
...
function C.GetQYSGameIDToConfig(game_id)
    if C.Config.fish_match_qys_map[game_id] then
        return C.Config.fish_match_qys_map[game_id]
    end
    return C.Config.fish_match_qys_map[1]
end

function C.GetQYSGameIDToAward(award_id)
    if C.Config.fish_match_qys_award[award_id] then
        return C.Config.fish_match_qys_award[award_id]
    end
    return C.Config.fish_match_qys_award[1]
end
function C.CacheQYSRankData(data, index)
    if not this.qys_rank_data then
        this.qys_rank_data = {}
    end
    this.qys_rank_data[index] = data
end
function C.GetQYSRankData(index)
    if not this.qys_rank_data then
        return
    end
    return this.qys_rank_data[index]
end
function C.GetQYSCfgByRank(game_id, rank)
    local cfg = C.GetQYSGameIDToAward(game_id)
    for k,v in ipairs(cfg) do
        if (v.min_rank == -1 or rank >= v.min_rank) and (v.max_rank == -1 or rank <= v.max_rank) then
            return v
        end
...
function C.GetQYSAwardByRank(game_id, rank)
    local cfg = C.GetQYSGameIDToAward(game_id)
    for k,v in ipairs(cfg) do
        if (v.min_rank == -1 or rank >= v.min_rank) and (v.max_rank == -1 or rank <= v.max_rank) then
            return v.award_desc, v.award_icon
