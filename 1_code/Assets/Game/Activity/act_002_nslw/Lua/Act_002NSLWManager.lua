...
Act_002NSLWManager = basefunc.class()
local M = Act_002NSLWManager
M.key = "act_002_nslw"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_nslw_task_config")
local lister
local task_ids
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21162", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
