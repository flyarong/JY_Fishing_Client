...
function C:UpdateCfg()
	...
	self.buy_all_gift_id = self.cfg.buy_all_gift_id
...
function C:UpdateStatusCfg()
    ...
    if self.buy_all_gift_id then
    	self.all_gift_status = MainModel.GetGiftShopStatusByID(self.buy_all_gift_id)
    else
    	self.all_gift_status = 1
    end
end
...
function C:InitUI()
	...
	self.award_l_z.gameObject:SetActive(self.cfg.is_zs[self.zs_key[1]] == 1)
	self.award_r_z.gameObject:SetActive(self.cfg.is_zs[self.zs_key[2]] == 1)
...
function C:InitBtn()
	...
	self.buy_btn.gameObject:SetActive(self.gift_status == 1 and self.all_gift_status == 1)
	self.buy_no_img.gameObject:SetActive(self.gift_status ~= 1 or self.all_gift_status ~= 1)
...
function C:BuyGift()
	...
	if self.gift_status ~= 1 or self.all_gift_status ~= 1 then
