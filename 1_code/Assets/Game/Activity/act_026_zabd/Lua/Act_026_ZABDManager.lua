-- 创建时间:2020-05-25
-- Act_026_ZABDManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_026_ZABDManager = {}
local M = Act_026_ZABDManager
M.key = "act_026_zabd"
GameButtonManager.ExtLoadLua(M.key, "Act_026_ZABDPanel")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_026_ZABDPanel.Create(parm.parent)
        end
...
function M.ShowNoticePrefab()
    ...
    if MainModel.myLocation == "game_Fishing3D" or MainModel.myLocation == "game_Eliminate" or MainModel.myLocation == "game_EliminateSH" or MainModel.myLocation == "game_EliminateCS" then
        M.StopNoticeTime()
        if btn_gameObject and IsEquals(btn_gameObject) and IsEquals(notice_prefab) then
            local time_index = 0
            local space = 60
...
function M.query_rank_base_info_response(_,data)
    dump(data,"<color=green><size=30>我的排行榜数据-4444-----</size></color>")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        Event.Brocast("act_026_zabd_base_info_get",{rank_type = data.rank_type})
    end
end
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_026_ZABDPrefab",btn_gameObject.transform)
        notice_prefab.transform.localPosition = Vector2.New(80,100)
        notice_prefab.gameObject:SetActive(false)
