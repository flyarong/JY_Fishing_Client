...
Act_026_ZABDPanel = basefunc.class()
local C = Act_026_ZABDPanel
C.name = "Act_026_ZABDPanel"
local M = Act_026_ZABDManager
local HGList={
	[1] ="localpop_icon_1",
	[2] ="localpop_icon_2",
	[3] ="localpop_icon_3",
...
local Award={
	100000,30000,10000,3000,3000,3000,2000,2000,2000,2000,1000,1000,1000,1000,1000,500,500,500,500,500
}
local base_txt = {
	[1] = {"击杀龙虾券BOSS和部分附有龙虾券兑换券的鱼会掉落龙虾兑换券，可参与排行榜","炮倍越高上榜的机会越大"},
}
local curr_index = 1
local DESCRIBE_TEXT = {
	[1] = "1.活动时间：8月25日7:30-8月31日23:59:59",
	[2] = "2.活动期间3D捕鱼除试炼场以外的场次中会出现接吻鱼，击杀后获得爱心，可参与排行榜（表白礼包中赠送的爱心可计入排行）",
	[3] = "3.爱心数量越多排名越靠前，数量相同时，上榜时间越早排名越靠前",
...
function C:MakeLister()
	self.lister = {}
	self.lister["act_026_zabd_base_info_get"] = basefunc.handler(self,self.on_act_026_zabd_base_info_get)
...
function C:onInfoGet(data)
	if data and data.result ==0 and IsEquals(self.gameObject) then 
		for i = 1, #data.rank_data do
			...
			if i%2 == 1 then
				self["I"..curr_index.."_info1_img"].sprite = GetTexture("zabd_bg_zabd3")
			else
				self["I"..curr_index.."_info1_img"].sprite = GetTexture("zabd_bg_zabd")
			end
...
function C:on_act_026_zabd_base_info_get()
	self:onMyInfoGet()
end
