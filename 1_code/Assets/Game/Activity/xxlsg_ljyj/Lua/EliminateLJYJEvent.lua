...
package.loaded["Game.game_Eliminate.Lua.eliminate_ljyj_award_config"] = nil
local BigAwardConfig = require "Game.game_Eliminate.Lua.eliminate_ljyj_award_config"

EliminateLJYJEvent = basefunc.class()
local C = EliminateLJYJEvent
C.name = "EliminateLJYJEvent"
...
function C:ctor(parent)
	...
	-- 		self.thisObj.SetActive(true)
	-- 		return 
	-- 	end
	-- end
    -- self.thisObj.SetActive(false)
    if os.time()> 1560355199 then
...
function C:InitRect()
	...
	for k, v in ipairs(BigAwardConfig.stage) do
		...
		-- elseif v.isgoods==true and v.id==7 then 
		-- 	item_btn.onClick:AddListener(function ()				
		-- 		GameManager.GotoUI({gotoui = "sys_service_gzh",goto_scene_parm = "panel",desc="奖品:时尚太阳眼镜，奖励达成后请联系客服领取奖励\n客服微信：%s", gzh="JYDDZ03"})
		-- 		Network.SendRequest("get_task_award_new", {id = TASK_ID,award_progress_lv=v.id})
		-- 		Network.SendRequest("query_one_task_data", {task_id = TASK_ID})
		-- 		self:RefreshItems()
		-- end	)		
...
function C:RefreshItems()
	...
	for i = 1, #self.ItemList do
	   ...
	   if	self.task_data.award_status_all[i]==2 then
			...
			for j = 1, #BigAwardConfig.stage do
				--print(BigAwardConfig.stage[j].isgoods)
				if BigAwardConfig.stage[j].isgoods==true then
					local item = self.ItemList[j]
					local item_mask = item.transform:Find("bg/getmask")
					item_mask.gameObject:SetActive(true)    
...
function C:handle_one_task_data_response(data)
	--dump(data, "<color=red>---C.handle_one_task_data_response-----</color>")
	self.task_data = data
	...
	dump(self.task_data.award_status_all, "<color=red>---C.handle_one_task_data_response-----</color>")
	--[[self.task_data.now_process = 120000
	self.task_data.need_process = 120000
	self.task_data.now_lv = 6
	self.task_data.award_status = 1
	self.task_data.task_round = 4]]--
	if instance then
		instance:Refresh()
	end
end
