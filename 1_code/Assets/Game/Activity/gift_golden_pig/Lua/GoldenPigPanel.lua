...
function GoldenPigPanel:InitShow()
    ...
    self.xh_499_title_txt.text = string.format("金币续航<size=32>（剩<color=#ed8813ff>%d</color>次可领）</size>", GoldenPigModel.data.pig2_num)
end
...
function GoldenPigPanel:PayUI()
    ...
    self.xh_499_title_txt.text = ""--string.format("金币续航<size=32>（剩<color=#ed8813ff>%d</color>次可领）</size>", 25)
    self.xh_499_desc_txt.text = "每日首次登录游戏自动领取20万金币,每日只能领1次"
...
function GoldenPigPanel:SetTaskUI(config)
    ...
    self.task_info_txt.text = string.format( "<color=#ED8813FF>%s</color>微信红包任务",config.task_price / 100)
