...
Act_005_TNSHManager = basefunc.class()
local M = Act_005_TNSHManager
M.key = "act_005_tnsh"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_005_tnsh_config")
local lister
local task_ids
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21204", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
