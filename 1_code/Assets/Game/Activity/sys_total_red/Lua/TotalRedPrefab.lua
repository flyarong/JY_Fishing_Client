...
function TotalRedPrefab:InitUI()
	...
	if all_info.today_game_race[game_id] == 0 and all_info.store_award > 0 then
		...
		if all_info.today_award >= all_info.today_max_award  then
			self.AwardHintText.text = "今日领取福利券达到上限"
		else
			local mm = StringHelper.ToCash(all_info.today_hb_award[game_id])
			self.AwardHintText.text = "再玩" .. (all_info.today_hb_condition[game_id]-all_info.today_game_race[game_id]) .."局奖".. mm .. "福利券"
		end
	end
