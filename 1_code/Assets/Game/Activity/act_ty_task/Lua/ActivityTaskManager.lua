...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        ...
        dump(configs[parm.goto_type],"<color=red>XXXXXXXXXXXXXXXXXXXXXX</color>")
...
function M.Refresh(taskid)
    for k,v in pairs(M.config.task) do
        if v.task and taskid == v.task then
            SYSACTBASEManager.InitActiveRedHint("weekly")
            break
        end
    end
    for k ,v in pairs(task_ids) do
        for k1,v1 in pairs(v) do
            if taskid == v1 then
                Event.Brocast("global_hint_state_change_msg",{gotoui = M.key,goto_type = k})
            end
        end
    end
end
