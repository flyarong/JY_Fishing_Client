...
function C:RefreshJD()
	if self.anim_state == CPL_LJYJCFKLotteryPanel.AnimState.AS_Null and self.xz_lv > self.wc_lv then
		self.hint_txt.text = string.format("再赢金<color=red>%s</color>，可抽取<color=red>%s福利券！</color>", StringHelper.ToCash(self.task_data.need_process - self.task_data.now_process),M.config.base[self.task_data.now_lv].hb[3])	
	end
end
-- 中间福袋显示
function C:RefreshFD()
	...
	self.asset_config = M.GetAssetByIDIndex(self.xz_lv)
	dump({self.xz_lv,self.config},"<color=yellow>CPL抽福利券</color>")
...
function C:AnimBegin()
	...
	self.seq:OnForceKill(function (force_kill)
		if force_kill and IsEquals(self.gameObject) then
			self.isPlayAnim = false
			self.cj_btn.gameObject:SetActive(true)
			self.anim_state = CPL_LJYJCFKLotteryPanel.AnimState.AS_Null
			self:MyRefresh()
...
function C:on_background_msg()
	if self.anim_state == CPL_LJYJCFKLotteryPanel.AnimState.AS_TheCard
		...
		for i = 1, self.cell_num do
			if self.CellList and self.CellList[i] and IsEquals(self.CellList[i].gameObject) then
...
function C.GetIndexByAward(award,config)
	...
	for i,v in ipairs(config) do
	    if tonumber(v.value) == tonumber(award.value) and v.type == award.asset_type then
			return i
		end
	end
	return 1
...
function C:OnAssetChange(data)
	if data.change_type and (data.change_type == "little_game_yingjin_task") then
		self.Award_Data = data
		self.award_index = C.GetIndexByAward(data.data[1], self.asset_config)
	    ...
	    ,function (err)
	        ...
	        if AppDefine.IsEDITOR() then
				HintPanel.Create(1, "请找客户端程序看一下")
				dump(self.index_list, "<color=red>检查一下</color>")
				dump(self.index)
				dump(self.award_index)
