...
function C:InitUI()
	self.b_btn.onClick:AddListener(function()
		...
		PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."ljyjcfk",1)
		self:OnEnterClick()
		self:MyRefresh()
	end)
	self:MyRefresh()
...
function C:MyRefresh()
	...
	if data then
		...
		if data.award_status ~= 1 then
			...
			dump(self:CheckIsTheFirstTimeGet(),"<color=red>++++++++++++++++++</color>")
			if self:CheckIsTheFirstTimeGet() then
...
function C:CheckIsTheFirstTimeGet()
	if PlayerPrefs.GetInt(MainModel.UserInfo.user_id.."ljyjcfk",0) == 0 then
		return true
	else
		return false
	end
