...
GameComGiftT4Panel = basefunc.class()
local C = GameComGiftT4Panel
C.name = "GameComGiftT4Panel"
local cz_shopid = {[10087] = 1,[10088] = 1,[10346] = 1, [10347] = 1,}
...
function C:InitUI()
	...
	for i = 1, 3 do
		...
		ui.buy_img.sprite = GetTexture(self.config.db_img)
		...
		for j = 1, 3 do
			...
			--特殊处理
			if j == 2 or j == 3 then
				ui["icon"..j.."_img"].gameObject:GetComponent("RectTransform").sizeDelta = Vector2.New(256,256)
			end
		...
		PointerEventListener.Get(self["tip_"..i].gameObject).onDown = function ()
			GameTipsPrefab.ShowDesc(self:GetContentStr(self.shopid[i]), UnityEngine.Input.mousePosition)
		end
