...
local DESCRIBE_TEXT = {
    [1] = "1.活动时间：3月30日8:00-4月5日23:59:59",
    [2] = "2.活动期间，分别统计所有消消乐游戏中24万及以上档次且倍数≥5的累计赢金，参与排行榜。",
    [3] = "3.档次×倍数的总和越大，排名越靠前，数量相同时上榜时间越早排名越靠前",
    [4] = "4.排行榜每180秒刷新一次",
    [5] = "5.活动结束后，排行榜奖励通过邮件发放，请注意查收",
}

function C.Create(_,parent)
    return C.New(_,parent)
end
...
function C:InitUI()
    ...
    self.extra_btn.onClick:AddListener(function()
        ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
        Act_045_XXLBDEXTRAPanel.Create()
    end)

    M.GetBaseDataFromNet()
    M.GetRankDataFromNet()
    self:MyRefresh()
    self.cutdown_txt.text = self.config.refreshtime
...
function C:AddToRankLis(_add_rank_data)
    for i = 1, #_add_rank_data do
        ...
        if _add_rank_data[i].rank_game == "xiaoxiaole_award" then
            temp_ui.rank_game_txt.text = "萌宠消消乐"
        ...
        elseif _add_rank_data[i].rank_game == "xiaoxiaole_shuihu_award" then
        ...
        elseif _add_rank_data[i].rank_game == "xiaoxiaole_caishen_award" then
        ...
        elseif _add_rank_data[i].rank_game == "xiaoxiaole_xiyou_award" then
        ...
        temp_ui.rank_mult_txt.text = StringHelper.ToCash(_add_rank_data[i].rank_mult)

        if _add_rank_data[i].rank_award == nil then
            temp_ui.rank_award_txt.text = "- -"
            temp_ui.rank_extra_award_txt.text = "- -"
            temp_ui.rank_extra_award_hui_txt.text = "- -"
        else
            temp_ui.rank_award_txt.text = _add_rank_data[i].rank_award
            temp_ui.rank_extra_award_txt.text = _add_rank_data[i].rank_extra_award
            temp_ui.rank_extra_award_hui_txt.text = _add_rank_data[i].rank_extra_award
        end
        temp_ui.rank_extra_award_txt.gameObject:SetActive(_add_rank_data[i].is_can)
        temp_ui.rank_extra_award_hui_txt.gameObject:SetActive(not _add_rank_data[i].is_can)
        if _add_rank_data[i].is_can then
            temp_ui.rank_extra_award_img.color = Color.New(1, 1, 1, 1)
        else
            temp_ui.rank_extra_award_img.color = Color.New(110/255, 110/255, 110/255, 1)
        end
...
function C:GotoGameUI(_game_tpe)
    ...
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
    if _game_tpe == "xiaoxiaole_award" then
        dump("<color>+++++++++++++++前往-水果消消乐+++++++++++++++++</color>")
        --GameManager.GoGameByKey({ key = "xxl" })
        GameManager.CommonGotoScence({gotoui = "game_Eliminate"})
    elseif _game_tpe == "xiaoxiaole_shuihu_award" then
        dump("<color>++++++++++++++前往-水浒消消乐++++++++++++++++++</color>")
        --GameManager.GoGameByKey({ key = "shxxl" })
        GameManager.CommonGotoScence({gotoui = "game_EliminateSH"})
    elseif _game_tpe == "xiaoxiaole_caishen_award" then
        dump("<color>++++++++++++++++前往-财神消消乐++++++++++++++++</color>")
        --GameManager.GoGameByKey({ key = "csxxl" })
        GameManager.CommonGotoScence({gotoui = "game_EliminateCS"})
    elseif _game_tpe == "xiaoxiaole_xiyou_award" then
        dump("<color>++++++++++++++++前往-西游消消乐++++++++++++++++</color>")
       -- GameManager.GoGameByKey({ key = "xyxxl" })
       GameManager.CommonGotoScence({gotoui = "game_EliminateXY"})
    else
        dump(_game_tpe, "<color>++++++++++++++++前往XXX++++++++++++++++</color>")
    end
end
...
function C:InitRankUser()
    ...
    if user_dataUI.rank_game == "xiaoxiaole_award" then
        self.user_rank_game_txt.text = "萌宠消消乐"
    ...
    elseif user_dataUI.rank_game == "xiaoxiaole_shuihu_award" then
    ...
    elseif user_dataUI.rank_game == "xiaoxiaole_caishen_award" then
    ...
    elseif user_dataUI.rank_game == "xiaoxiaole_xiyou_award" then
    ...
    self.user_rank_mult_txt.text = StringHelper.ToCash(user_dataUI.rank_mult)
    if user_dataUI.rank_award == nil then
        self.user_rank_award_txt.text = "- -"
        self.user_rank_extra_award_txt.text = "- -"
        self.user_rank_extra_award_hui_txt.text = "- -"
    else
        self.user_rank_award_txt.text = user_dataUI.rank_award
        self.user_rank_extra_award_txt.text = user_dataUI.extra_awd_cfg
        self.user_rank_extra_award_hui_txt.text = user_dataUI.extra_awd_cfg
    end
    self.user_rank_extra_award_txt.gameObject:SetActive(user_dataUI.is_can)
    self.user_rank_extra_award_hui_txt.gameObject:SetActive(not user_dataUI.is_can)
    if user_dataUI.is_can then
        self.user_extra_redbg.transform:GetComponent("Image").color = Color.New(1,1,1,1)
    else
        self.user_extra_redbg.transform:GetComponent("Image").color = Color.New(110/255,110/255,110/255,1)
    end
    user_dataUI = {}
end
