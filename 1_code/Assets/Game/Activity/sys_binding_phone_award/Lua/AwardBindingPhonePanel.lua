...
function C.Create(parent,tips,binding_callback,backcall)
	return C.New(parent,tips,binding_callback,backcall)
end
...
function C:ctor(parent,tips,binding_callback,backcall)
...
function C:InitUI()
    self.tips_txt.text = self.tips or "提示： 绑定后可大幅提高帐号安全"
    if MainModel.LoginInfo.channel_type == "youke" then
        self.close_btn.gameObject:SetActive(false)
        self.tips_txt.text = self.tips or "<color=#FF1926FF>尊敬的用户，游客登陆将于3月16号更新后取消，为了保障您的账号和财产安全，请进行手机号绑定。</color>"
    end
...
function C:OnClickSureBinding(go)
    ...
    Network.SendRequest("verify_bind_phone_code", {code=self.code_ipf.text}, "发送请求" , function (data)
        if data.result == 0 then
            ...
            LoginModel.SetLoginData({channel="phone", token=data.token, id=phong_number})
