...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Sys_011_YueKaPanel.Create(parm.parent, true)
    elseif parm.goto_scene_parm == "panel1" then
        return Sys_011_YueKaPanel.Create(parm.parent, false)
...
function M.IsAwardCanGet()
    if m_data then
        if m_data.total_remain_num_2 and m_data.is_receive_2 and m_data.total_remain_num_2 > 0 and m_data.is_receive_2 == 0 then
            return true
        end
    end
end
--- 0 都没购买或过期 1 只购买了豪华月卡  2 只购买了至尊月卡   3：全购买
function M.YueKaBuyState()
    if m_data then
        local haohua=0
        local zhizun=0
        if m_data.total_remain_num_1>0 then
            haohua=1
        end
        if m_data.total_remain_num_2>0 then
            zhizun=2
        end
        return haohua+zhizun
    end
end

function M.IsBuySmallYueKa()
    if m_data and m_data.total_remain_num_1 and m_data.total_remain_num_1 > 0 then
        return true
    end
end

function M.IsBuyBigYueKa()
    if m_data and m_data.total_remain_num_2 and m_data.total_remain_num_2 > 0 then
        return true
    end
end
