...
function C:MakeLister()
	...
	self.lister["close_flqcj_pannel"] = basefunc.handler(self, self.MyExit)
    self.lister["model_xycj_ptcj_status_msg"] = basefunc.handler(self, self.MyRefresh)
end
...
function C:MyRefresh()
    ...
    for k,v in ipairs(ll) do
        if v.gotoUI[1] == "xycj" and v.gotoUI[2] == "pt_cj" then
        	local b,c = GameButtonManager.RunFun({gotoui=v.gotoUI[1], goto_scene_parm=v.gotoUI[2]}, "IsShowPTCJ")
        	if b and c then
        		if (not self.parm or not self.parm.type or self.parm.type ~= 1) and not c then
        			-- 隐藏
        		else
