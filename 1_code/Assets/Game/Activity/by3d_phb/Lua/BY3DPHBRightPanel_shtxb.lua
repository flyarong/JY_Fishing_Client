...
BY3DPHBRightPanel_shtxb = basefunc.class()
local C = BY3DPHBRightPanel_shtxb
C.name = "BY3DPHBRightPanel_shtxb"
...
function C:MakeLister()
    ...
    self.lister["shtxb_rank_data_msg"] = basefunc.handler(self,self.RefreshRank)
    self.lister["shtxb_myrank_data_msg"] = basefunc.handler(self,self.RefreshMyRank)
end
...
function C:InitUI()
	self.config = HotUpdateConfig("Game.Activity.by3d_phb.Lua.right_shtxb_config")
	dump(self.config,"<color=green>++++++++right_shtxb_config+++++++</color>")
	self.refreshtime_txt.text = self.config.refreshtime

	M.QueryMyData_shtxb()
	M.QueryData_shtxb(self.page_index)
