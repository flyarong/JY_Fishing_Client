...
BY3DPHBRightPanel_djs = basefunc.class()
local C = BY3DPHBRightPanel_djs
C.name = "BY3DPHBRightPanel_djs"
...
function C:MakeLister()
    ...
    self.lister["djs_rank_data_msg"] = basefunc.handler(self,self.RefreshRank)
    self.lister["djs_myrank_data_msg"] = basefunc.handler(self,self.RefreshMyRank)
end
...
function C:InitUI()
	self.config = HotUpdateConfig("Game.Activity.by3d_phb.Lua.right_djs_config")
	local award_list = HotUpdateConfig("Game.Activity.sys_fishing_manager.Lua.fishmatch_ui")
	self.award_list = award_list.award
	dump(self.config,"<color=green>++++++++right_djs_config+++++++</color>")
	self.refreshtime_txt.text = self.config.refreshtime

	M.QueryData_djs(self.game_id,self.page_index)
	...
	if data then
		...
		for i=1,#self.award_list do
			if data.rank >= self.award_list[i].min_rank and data.rank <= self.award_list[i].max_rank then
				...
				self.rank_award1_img.sprite = GetTexture(self.award_list[i].icon)
				local dd = string.format("%.1f", self.award_list[i].extra_award_desc * 100)
				if self.award_list[i].award then
					self.rank_award1_txt.text = self.award_list[i].award .. "+额外" .. dd .. "%的奖池金"
				else
					self.rank_award1_txt.text = dd .. "%的奖池金"
				end
			end
		end
	else
