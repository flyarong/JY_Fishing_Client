-- 创建时间:2020-10-22
-- Panel:BY3DPHBRightPanel_sgxxlb
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
BY3DPHBRightPanel_sgxxlb = basefunc.class()
local C = BY3DPHBRightPanel_sgxxlb
C.name = "BY3DPHBRightPanel_sgxxlb"
...
function C:MakeLister()
    ...
    self.lister["sgxxlb_rank_data_msg"] = basefunc.handler(self,self.RefreshRank)
    self.lister["sgxxlb_myrank_data_msg"] = basefunc.handler(self,self.RefreshMyRank)
end
...
function C:InitUI()
	self.config = HotUpdateConfig("Game.Activity.by3d_phb.Lua.right_sgxxlb_config")
	dump(self.config,"<color=green>++++++++right_sgxxlb_config+++++++</color>")
	self.refreshtime_txt.text = self.config.refreshtime

	M.QueryMyData_sgxxlb()
	M.QueryData_sgxxlb(self.page_index)
...
function C:RefreshMyRank(data)
	...
	if data.rank == -1 then
		...
		self.jl_img.sprite = GetTexture("ty_icon_flq1")
	else
		if data.rank == 1 then
			self.my_rank_img.gameObject:SetActive(true)
			self.my_rank_txt.gameObject:SetActive(false)
			self.my_rank_img.sprite = GetTexture("localpop_icon_1")
			self.jl_img.sprite = GetTexture("ty_icon_flq5")
		elseif data.rank == 2 then
			self.my_rank_img.gameObject:SetActive(true)
			self.my_rank_txt.gameObject:SetActive(false)
			self.my_rank_img.sprite = GetTexture("localpop_icon_2")
			self.jl_img.sprite = GetTexture("ty_icon_flq4")
		elseif data.rank == 3 then
			self.my_rank_img.gameObject:SetActive(true)
			self.my_rank_txt.gameObject:SetActive(false)
			self.my_rank_img.sprite = GetTexture("localpop_icon_3")
			self.jl_img.sprite = GetTexture("ty_icon_flq3")
		else
			self.my_rank_img.gameObject:SetActive(true)
			...
			self.jl_img.sprite = GetTexture("ty_icon_flq2")
		end
	end
