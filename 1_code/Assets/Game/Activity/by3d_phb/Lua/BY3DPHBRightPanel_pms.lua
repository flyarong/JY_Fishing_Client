...
BY3DPHBRightPanel_pms = basefunc.class()
local C = BY3DPHBRightPanel_pms
C.name = "BY3DPHBRightPanel_pms"
...
function C:MakeLister()
    ...
    self.lister["pms_rank_data_msg"] = basefunc.handler(self,self.RefreshRank)
    self.lister["pms_myrank_data_msg"] = basefunc.handler(self,self.RefreshMyRank)
end
...
function C:InitUI()
	...
	self.config = HotUpdateConfig("Game.Activity.by3d_phb.Lua.right_pms_config")
	self.help_config = HotUpdateConfig("Game.Activity.sys_by_pms.Lua.sysbypms_rules_config")
	local pms_config = HotUpdateConfig("Game.Activity.sys_by_pms.Lua.sysbypms_rank_config")
	self.rank_config = {}
	for i=1,#pms_config.config do
		self.rank_config[pms_config.config[i].game_id] = self.rank_config[pms_config.config[i].game_id] or {}
		self.rank_config[pms_config.config[i].game_id][#self.rank_config[pms_config.config[i].game_id] + 1] = pms_config.config[i]
	end
	dump(self.config,"<color=green>++++++++right_pms_config+++++++</color>")
	...
	self:OnTitleClick(1)
dump(self.rank_config,"<color=yellow><size=15>++++++++++data++++++++++</size></color>")
	...
	for i=1,#data do
		...
		if _G[panelName] then
			if _G[panelName].Create then 
				self.spawn_cell_list[#self.spawn_cell_list + 1] = _G[panelName].Create(self,self.Content.transform,self.rank_config[self.game_id],data[i])
			else
...
function C:RefreshRankInfo()
	M.QueryData_pms(self.game_id,self.page_index)
end
...
function C:RefreshMyRank(data)
	...
	if data then
		...
		for i=1,#self.rank_config[self.game_id] do
			if data.rank >= self.rank_config[self.game_id][i].rank_range[1] and data.rank <= self.rank_config[self.game_id][i].rank_range[2] then
				for j=1,#self.rank_config[self.game_id][i].award_icon do
					self["rank_award_icon"..j].gameObject:SetActive(true)
					self["rank_award"..j.."_img"].sprite = GetTexture(self.rank_config[self.game_id][i].award_icon[j])
					local item = GameItemModel.GetItemToKey(self.rank_config[self.game_id][i].award_type[j])
					self["rank_award"..j.."_txt"].text = StringHelper.ToCash(self.rank_config[self.game_id][i].award_num[j]) .. item.name
				end
			end
		end
	else
