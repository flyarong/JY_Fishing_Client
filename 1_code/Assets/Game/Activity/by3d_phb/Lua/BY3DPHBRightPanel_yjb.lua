...
BY3DPHBRightPanel_yjb = basefunc.class()
local C = BY3DPHBRightPanel_yjb
C.name = "BY3DPHBRightPanel_yjb"
...
function C:MakeLister()
    ...
    self.lister["yjb_rank_data_msg"] = basefunc.handler(self,self.RefreshRank)
    self.lister["yjb_myrank_data_msg"] = basefunc.handler(self,self.RefreshMyRank)
end
...
function C:InitUI()
	self.config = HotUpdateConfig("Game.Activity.by3d_phb.Lua.right_yjb_config")
	dump(self.config,"<color=green>++++++++right_yjb_config+++++++</color>")
	self.refreshtime_txt.text = self.config.refreshtime

	M.QueryMyData_yjb()
	M.QueryData_yjb(self.page_index)
