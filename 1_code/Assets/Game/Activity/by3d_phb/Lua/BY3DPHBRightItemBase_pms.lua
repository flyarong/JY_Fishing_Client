-- 创建时间:2020-10-22
-- Panel:BY3DPHBRightItemBase_pms
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
BY3DPHBRightItemBase_pms = basefunc.class()
local C = BY3DPHBRightItemBase_pms
C.name = "BY3DPHBRightItemBase_pms"
...
function C:InitUI()
	...
	for i=1,#self.config do
		if self.data.rank >= self.config[i].rank_range[1] and self.data.rank <= self.config[i].rank_range[2] then
			for j=1,#self.config[i].award_icon do
				self["rank_award_icon"..j.."_img"].gameObject:SetActive(true)
				self["rank_award"..j.."_img"].sprite = GetTexture(self.config[i].award_icon[j])
				local item = GameItemModel.GetItemToKey(self.config[i].award_type[j])
				self["rank_award"..j.."_txt"].text = StringHelper.ToCash(self.config[i].award_num[j]) .. item.name
			end
		end
	end
