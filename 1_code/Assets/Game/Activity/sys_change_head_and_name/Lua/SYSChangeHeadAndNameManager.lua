...
M.config = GameButtonManager.ExtLoadLua(M.key, "head_image_server")
...
function M.on_set_head_image_response(_,data)
    ...
    if data then
        if data.result == 0 then
            MainModel.UserInfo.head_image = M.config.head_images[data.img_type].url
...
function M.InitImgList()
    ...
    this.m_data.vipList = {}
    for i=1,#M.config.head_images do
        if M.config.head_images[i].vip_permission == 0 then
            this.m_data.freeList[#this.m_data.freeList + 1] = M.config.head_images[i]
        else
            this.m_data.vipList[#this.m_data.vipList + 1] = M.config.head_images[i]
        end
    end
end