...
function C:InitUI()
	...
	dump(MainModel.UserInfo.udpate_name_num,"<color=yellow>----------||||----------</color>")
	if MainModel.UserInfo.udpate_name_num == 0 then
		self.cost_txt.text = "首次免费修改"
	else
		self.cost_txt.text = "10万金币"
	end
	self.Text.gameObject:GetComponent("Text").color = COLOR_XGNCHS_Color
	self.Text1.gameObject:GetComponent("Text").color = COLOR_XGNCHS_Color
	self.Text2.gameObject:GetComponent("Text").color = COLOR_XGNCLS_Color
	self.cost_txt.gameObject:GetComponent("Text").color = COLOR_XGNCJS_Color
	self:MyRefresh()
end
