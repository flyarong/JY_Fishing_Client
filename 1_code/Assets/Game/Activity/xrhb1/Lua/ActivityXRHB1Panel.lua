...
function C:InitUI()
    ...
    self.jhlb_txt.text = "激活立马获得<color=#fffce6><size=48>60万</size></color>金币，另有累计<color=#fffce6><size=48>3000</size></color>福利券可领！"
...
function C:RefreshUI()
	...
	self.total_hb_txt.text = ActivityXRHB1Model.data.total_hongbao_num
    self.now_get_hb_txt.text = string.format("已领取    <color=#c02d1fff><size=44>%.2f</size></color>    红包", ActivityXRHB1Model.data.now_get_hongbao_num)
    self.countdown = ActivityXRHB1Model.data.over_time - os.time()
    self.over_time_txt.text = StringHelper.formatTimeDHMS(self.countdown)
    ...
    if show_jhlb then
        self.TaskNodeSV.transform.localPosition = Vector3.New(self.TaskNodeSV.transform.localPosition.x,-95,self.TaskNodeSV.transform.localPosition.z)
        self.TaskNodeSV.gameObject:GetComponent("RectTransform").sizeDelta = Vector2.New(1300,497.5) 
    end
end
...
function C:UpdateTask(is_no_anim)
    -- self:RefreshVIP()
    self:ClearTaskCellList()
    ...
    for k, v in ipairs(self.task_list) do
        local pre = ActivityXRHB1TaskPrefab.Create(self.TaskNode.transform, v, self)
        self.TaskCellList[#self.TaskCellList + 1] = pre
        pre.gameObject.name = "ActivityXRHB1TaskPrefab" .. i_name
        i_name = i_name + 1
    end
    if GuideLogic.IsHaveGuide("xrhb1_panel") then
        GuideLogic.CheckRunGuide("xrhb1_panel")
    else
        if not is_no_anim and self.TaskCellList and next(self.TaskCellList) then
            local i = 0
            local tt = 0.1
...
function C:on_model_get_stepstep_money_task_award_response(_, now_get_hongbao_num)
    ...
    self.now_get_hb_txt.text = string.format("已领取 <color=#c02d1fff><size=35>%.2f</size></color> 福利券", ActivityXRHB1Model.data.now_get_hongbao_num )
end
