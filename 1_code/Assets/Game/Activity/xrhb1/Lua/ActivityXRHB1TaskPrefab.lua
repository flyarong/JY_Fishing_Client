...
ActivityXRHB1TaskPrefab = basefunc.class()

local C = ActivityXRHB1TaskPrefab

C.name = "ActivityXRHB1TaskPrefab"
...
function C:InitUI()
    ...
    if self.cur_config then
        self.get_btn.gameObject.name = "xrhb_btn_" .. self.cur_config.task_id
    end 
end

function C:RefreshUI()
    if not self.cur_config then
        return
    end
    self.item_icon_img.sprite = GetTexture(self.cur_config.icon_image)
    if ActivityXRHB1Model.IsNewVersion() then
        ...
        self.item_icon_txt.text = string.format("%.2f", self.cur_config.award_num)
    end
    ...
    local now_process = self.cur_config.now_process
    local need_process = self.cur_config.need_process
...
function C:RefreshCurIndex(is_check_process)
    ...
    if self.show_more_btn then
        ...
        for i, v in ipairs(self.config) do
            local cur_config = ActivityXRHB1Model.GetTaskToID(v)
...
function C:RefreshCurConfig()
    ...
    self.cur_config = ActivityXRHB1Model.GetTaskToID(self.config[self.cur_index])
...
function C:PlayAnimIn(t)
    ...
    seq:Append(self.UINode.transform:DOLocalMoveX(0, 0.2):SetEase(DG.Tweening.Ease.Linear))
    --OutBack
...
function C:PlayAnimOut(t)
    ...
    local seq = DoTweenSequence.Create()
    seq:AppendInterval(t)
    seq:Append(self.UINode.transform:DOLocalMoveX(1600, 0.2):SetEase(DG.Tweening.Ease.Linear))
    --OutBack
...
function C:OnClickGoto(gotoUI)
    ...
    if gotoUI then
        local gotoparm = {gotoui = gotoUI[1], goto_scene_parm = gotoUI[2]}
...
function C:on_model_stepstep_money_task_change_msg(id)
    ...
    for i, v in ipairs(self.config) do
        if v == id then
            self.cur_config = ActivityXRHB1Model.GetTaskToID(id)
            self:RefreshCurIndex()
            self:RefreshUI()
            self:PlayAnimRefresh(0.02)
        end
