...
function C:MyRefresh()
	...
	if self.parm.key == "dh" then
		...
		local rr = StringHelper.ToRedNum(model.data.exchange_hongbao.hong_bao )
		local rr2 = StringHelper.ToRedNum(model.data.exchange_hongbao.hong_bao2 )
        local jb = 0
        ...
        local str = string.format("为您将本局赢得的%s金币兑换成%s福利券", jb, rr)
		...
		self.desc_txt.text = "单笔充值6元及以上(不包含礼包),免视频领取双倍奖励"
	elseif self.parm.key == "djhb" then
		...
		if nor_key == "shop_gold_sum" then
			self.award_txt.text = StringHelper.ToRedNum(nor_val )
		else
		...
		if ext_award[1].asset_type == "shop_gold_sum" then
			self.ew_award_txt.text = StringHelper.ToRedNum(ext_award[1].value )
		else
		...
		self.desc_txt.text = "单笔充值6元及以上(不包含礼包),免视频领取额外奖励"

		-- 除了免费用户，其它不需要额外操作即可领取额外奖励
		if not tag_vec_map[TagVecKey.tag_free_player] then
			self:OnXRClick()
	        if self.parm.call then
	        	self.parm.call(self.parm.panelSelf)
	        end
	...
	elseif self.parm.key == "ls" then
		...
		if self.nor_award[1].asset_type == "shop_gold_sum" then
			self.award_txt.text = StringHelper.ToRedNum(self.nor_award[1].value )
		else
