...
function M.IsNeedWatchAD()
    ...
    -- Ios没有广告  vivo无广告
    if (not AppDefine.IsEDITOR() and gameRuntimePlatform == "Ios") or platform == "vivo" or platform == "xiaomi" or platform == "yyb" then
        return
    end
...
function M.InitQXGlobalVariate()
    ...
    if b then
        Global_GZH = "彩云新世界"
        Global_GZH_ID = "彩云新世界"
        GameSceneCfg[31].GameTitle = "冲金鸡"
        GameSceneCfg[31].GameName = "冲金鸡"
