...
function C.ExtLogic(parm)
	...
	if parm.key == "match_js" then
        config = GameMatchModel.GetGameIDToConfig(parm.panelSelf.parm.game_id)
    end
    ...
    local on_click = function ()
        if AdvertisingManager.IsCloseAD() then
            ...
            ii.sprite = GetTexture("ggw_btn_lq")
            pre.close_txt.transform.localPosition = Vector3.New(32, 14, 0)
        end
    end

    local item_key, item_count = GameMatchModel.GetMatchCanUseTool(config.enter_condi_itemkey, config.enter_condi_item_count)
    if (item_key and item_key == "jing_bi") or (not item_key and config.enter_condi_count) then

		local new_signup_jing_bi
		if tag_vec_map[TagVecKey.tag_free_player] then
	        if item_key then
        ...
        local signup_call = function ()
            local signup = function ()
                ...
                GameMatchModel.SetCurrGameID(config.game_id)
                local scene_id = GameMatchModel.GetGameIDToScene(config.game_id)
        ...
        if parm.key == "match_hall" then
        	...
        	if tag_vec_map[TagVecKey.tag_new_player] or tag_vec_map[TagVecKey.tag_free_player] then
        ...
        elseif parm.key == "match_detail" then
            ...
            if tag_vec_map[TagVecKey.tag_new_player] then
                parm.panelSelf.selected_txt.text = "金币x5000(新人专属已打折)"
            elseif tag_vec_map[TagVecKey.tag_free_player] then
                parm.panelSelf.selected_txt.text = "金币x1万(看广告打5折)"
            else
                parm.panelSelf.selected_txt.text = "金币x5000(已充值打折)"
            end
            if tag_vec_map[TagVecKey.tag_free_player] then
