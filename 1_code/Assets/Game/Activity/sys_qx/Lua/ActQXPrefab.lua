	...
	if not parm.panelSelf.is_ext then
	    ...
	    if parm.key == "dh" then
	    	...
	    	local call = function ()
			    ...
			    if not iss then
			        local rr = StringHelper.ToRedNum(parm.panelSelf.ext_model.data.exchange_hongbao.hong_bao2 )
	                local jb = 0
	    	...
	    	if not tag_vec_map[TagVecKey.tag_free_player] and parm.panelSelf.ext_model.data.exchange_hongbao then
    			local rr = parm.panelSelf.ext_model.data.exchange_hongbao.hong_bao2
    			if IsEquals(parm.panelSelf.ExChongbao) then
    				parm.panelSelf.ExChongbao.text = StringHelper.ToRedNum(rr )
    			end
	    	end
