...
function M.InitCfg()
    M.gfit_config = {}
    M.collect_config = {}
    for i=1,#M.config.platform_or_channel_or_level[M.now_level].gift_config do
        M.gfit_config[#M.gfit_config + 1] = M.config.gift_config[M.config.platform_or_channel_or_level[M.now_level].gift_config[i]]
    end
    for i=1,#M.config.platform_or_channel_or_level[M.now_level].collect_config do
        M.collect_config[#M.collect_config + 1] = M.config.collect_config[M.config.platform_or_channel_or_level[M.now_level].collect_config[i]]
    end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data and data.result == 0 then
        if table_is_null(M.care_types_and_ids) then return end
...
function M.on_finish_gift_shop(id)
    if id then
        if table_is_null(M.care_gift_ids) then return end
