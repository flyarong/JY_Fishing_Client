...
YCS_BSYYPanel = basefunc.class()
local C = YCS_BSYYPanel
C.name = "YCS_BSYYPanel"
local t 
	...
	self.yuyue_btn.onClick:AddListener(
		function ()
			...
			if GameGlobalOnOff.BindingPhone and self.is_not_bind then
				...
				if MainModel.UserInfo.jing_bi >= 10000 then 
					Network.SendRequest("get_gns_ticket")					
				else
					HintPanel.Create(1,"金币不足哦，快去商城充值吧！",function ()
