...
function C:ctor(parent, backcall)
	...
	self.share_desc_txt.text = string.format("每邀请一位好友完成新人红包奖 <color=#ca1919ff><size=45>%.0f</size></color> 元",5)
	self:InitUI()
end
...
function C:on_query_stepstep_money_big_step_data(day)
	if not self.selectDay then
		...
		self.total_hb_txt.text = string.format("%.1f",ActivitySevenDayModel.data.total_hongbao_num)
		self.now_get_hb_txt.text = string.format("已领取    <color=#c02d1fff><size=44>%.2f</size></color>    福利券",ActivitySevenDayModel.data.now_get_hongbao_num )
	else
...
function C:on_model_get_stepstep_money_task_award_response(_,now_get_hongbao_num)
	...
	self.now_get_hb_txt.text = string.format("已领取 <color=#c02d1fff><size=35>%.2f</size></color> 福利券",ActivitySevenDayModel.data.now_get_hongbao_num )
end
