-- 创建时间:2020-07-21
-- Act_027_QYZLManager 管理器

--功能修改  金秋送豪礼 -----> 冬日送豪礼----->七月赠礼
local basefunc = require "Game/Common/basefunc"
Act_027_QYZLManager = {}
local M = Act_027_QYZLManager
M.key = "act_027_qyzl"
GameButtonManager.ExtLoadLua(M.key,"Act_027_QYZLPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_027_QYZLEnterPrefab")
M.config = GameButtonManager.ExtLoadLua(M.key,"act_027_qyzl_config")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if  M.IsActive() then 
            return Act_027_QYZLPanel.Create(parm.parent,parm.backcall)
        end 
    elseif parm.goto_scene_parm == "enter" then
        if M.IsActive() then
            return Act_027_QYZLEnterPrefab.Create(parm.parent,parm.backcall)
        end
    else
...
function M.InitConfigData()
    ...
    dump(config,"<color=yellow>七月赠礼配置表数据</color>")
    if config then
        --充值任务ID
...
function M.GetCurConfig()
    local cur_t = os.time()
    if cur_t > M.config.other_data[1].end_time or cur_t < M.config.other_data[1].sta_time then
        return nil
    end
