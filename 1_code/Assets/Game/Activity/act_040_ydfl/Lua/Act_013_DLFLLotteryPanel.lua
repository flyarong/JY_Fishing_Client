-- 创建时间:2020-05-12
-- Panel:Act_013_DLFLLotteryPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
Act_013_DLFLLotteryPanel = basefunc.class()
local C = Act_013_DLFLLotteryPanel
C.name = "Act_013_DLFLLotteryPanel"
...
function C:MakeLister()
    ...
    self.lister["model_dlfl_unrealy_change_msg"] = basefunc.handler(self,self.on_model_dlfl_unrealy_change_msg)
    self.lister["model_dlfl_lottery_change_msg"] = basefunc.handler(self,self.on_model_dlfl_lottery_change_msg)
...
function C:InitUI()
	self.lottery_btn.onClick:AddListener(function ()
		Network.SendRequest("login_benefits_lottery")
...
--虚假数据跑马灯
function C:on_model_dlfl_unrealy_change_msg()
...
function C:on_model_dlfl_lottery_change_msg()
	self.times_txt.text = "剩余次数:"..(M.GetLotteryNum() or "0")
...
function C:OnAssetChange(data)
	...
	if data.change_type and data.change_type == "login_benefits_lottery_award" and not table_is_null(data.data) then
