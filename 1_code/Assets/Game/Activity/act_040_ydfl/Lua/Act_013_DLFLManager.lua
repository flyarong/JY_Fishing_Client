...
local this
local lister
M.qfxl_gift_id = 10705
M.qfxl_gift_id2 = 10706
local s_time = 1640649600
M.s_time7Day = 1641225599
...
function M.IsActive()
    ...
    if dlfl_time and dlfl_time == 0 and os.time() >= M.s_time7Day then
...
function M.CheckIsShow(parm, type)
    if parm.goto_scene_parm == "enter" and os.time() >= M.s_time7Day then
        return false
    elseif parm.goto_scene_parm == "jyfl_enter" and os.time() < M.s_time7Day then
        return false
    end
    return M.IsActive()
end
...
function M.on_query_player_login_benefits_base_info_response(_,data)
    dump(data,"<color=yellow><size=15>++++++++++013data++++++++++</size></color>")
