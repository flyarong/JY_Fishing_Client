...
Act_002HBFXWalletPanel = basefunc.class()
local C = Act_002HBFXWalletPanel
C.name = "Act_002HBFXWalletPanel"
			...
			MainModel.GetBindZFB(function(  )
				if table_is_null(MainModel.UserInfo.zfbData) or MainModel.UserInfo.zfbData.name == "" then
					...
					if GameItemModel.GetItemCount("prop_npca_hb")/100 >= 1 then 
						HintPanel.Create(2,"可提现"..Mathf.Floor((GameItemModel.GetItemCount("prop_npca_hb")/100)).."元红包,确认提现吗？",function ()
...
function C:on_model_query_npca_wallet_data_got()
	...
	if data and data.result == 0 then
		self.num_txt.text = (GameItemModel.GetItemCount("prop_npca_hb")/100).."红包"
		for i = 1,#data.data do
			if data.data[i].type > 1 then
				local b = GameObject.Instantiate(self.hb_item,self.hb_parent)
				b.gameObject:SetActive(true)
				LuaHelper.GeneratingVar(b.transform, temp_ui)
				temp_ui.hb_money_txt.text = "+"..(data.data[i].value / 100).."红包"
				...
				b.gameObject:SetActive(true)
				LuaHelper.GeneratingVar(b.transform, temp_ui)
				temp_ui.tx_money_txt.text = "-"..(data.data[i].value / 100).."红包"
