...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1585584000
    local s_time = 1585006200
...
function M.on_hbfx_clear_uichange(parm)
    dump(parm,"<color=red>红包分享 结算-------------</color>")
...
function M.goMatch()
    GameManager.GotoSceneID(4 , true , nil , function() 
        GameFreeModel.SetCurrGameID(101)
        Network.SendRequest("fg_signup", {id = 101}, "正在报名")
    end)
end 
...
function M.npca_slave_get_award_msg(_,data)
    dump(data,"<color=red>红包领奖推送</color>")
    this.show_data = data
