...
local help_info = {
	...
	{
		id = 1,
		text = "1.活动时间：3月24日7:30-3月30日24：00;",
	},
	[2]=
	{
		id = 2,
		text = "2.活动期间，您可以将活动分享给微信好友，邀请好友前来挑战;",
	},
	[3]=
	{
		id = 3,
		text = "3.每2位好友通过您的分享下载游戏，<color=#BB301FFF>完成新手引导后，参与本活动的斗地主挑战取胜即为组队挑战成功</color>，届时您可以获得8元现金红包，2位好友各获得1元;",
	},
	[4]=
	{
		id = 4,
		text = "4.每位玩家仅有1次帮好友挑战的机会，进入斗地主挑战场次开始发牌即算消耗挑战机会，无论对局胜负，均会消耗挑战机会;",
	},
	[5]=
	{
		id = 5,
		text = "5.组队有效期为24小时，即当您的一位好友挑战成功后，另一位好友必须在24小时内挑战成功，成功则完成组队，失败则组队会超时失败;",
	},
	[6]=
	{
		id = 6,
		text = "6.本活动中获得的现金红包可在活动页面“我的红包”中进行提现，请于活动结束前提取，活动结束后未提取的红包视为放弃奖励;",
	},
	[7]=
	{
		id = 7,
		text = "7.本公司拥有对上述活动规则的最终解释权利。",
	},
...
function C:on_model_query_npca_main_data_got()
	...
	dump(data,"<color=red>红包分享主数据=================</color>")
	...
	if data then 
		...
		if data.result == 0 then 
			...
			if data.challenging_num > 0 then 
				...
				self.share_btn.gameObject.transform:GetComponent("Image").sprite = GetTexture("yhy_btn_yddsx")
			...
			if #data.challenging_player_info >= 2 then 
				self.canLj = true
				self.share_btn.gameObject.transform:GetComponent("Image").sprite = GetTexture("yhy_btn_lqbyjl")
				self.share_btn.enabled = true
			end 
