...
function C:GetBtnList()
    local btn_list = {}
    local cfg = BY3DTopQHManager.GetConfig(self.parm.goto_scene_parm)

    for k, v in pairs(cfg) do
        local module_cc = GameButtonManager.GetLoadModuleByKey(v.gotoui[1])
        if module_cc then
            if module_cc.lua and _G[module_cc.lua] then
                local cclua = _G[module_cc.lua]
                local parm = {}
                SetTempParm(parm, v.gotoui, "bytop_area")
...
function C:MyRefresh()
    ...
    for k,v in ipairs(self.act_cfg) do
        self.act_map[ v.gotoui[1] ] = self.act_map[ v.gotoui[1] ] or {}
        self.act_map[ v.gotoui[1] ][ v.gotoui[2] ] = v
    end
    for k,v in ipairs(self.act_cfg) do
        local key = v.gotoui[1]
        local pp = v.gotoui[2] or "bytop_area"
        if not self.cell_map[key] or not self.cell_map[key][pp] then
            local pre = GameManager.GotoUI({gotoui=key, goto_scene_parm=pp, parm=v.parm, parent=self.content})
            if pre then
                self.cell_map[key] = self.cell_map[key] or {}
                self.cell_map[key][pp] = {prefab = pre, key = key, cfg=v}
    ...
    for k,v in pairs(self.cell_map) do
        for kk,vv in pairs(v) do
            if not self.act_map[k] or not self.act_map[k][kk] then
                if vv.prefab.OnDestroy then
                    vv.prefab:OnDestroy()
                    self.cell_map[k][kk] = nil
                else
                    if AppDefine.IsEDITOR() then
                        dump(self.act_map)
                        dump(self.cell_map)
        ...
        if table_is_null(self.cell_map[k]) then
            self.cell_map[k] = nil
        end
    end
...
function C:RefreshSelect()
    if GameGlobalOnOff.IsOpenGuide and MainModel.UserInfo.xsyd_status ~= -1 then return end
    ...
    for k,v in ipairs(self.act_cfg) do
        if k == self.select_index then
            if self.cell_map[ v.gotoui[1] ] and self.cell_map[ v.gotoui[1] ][ v.gotoui[2] ] then
                self.cell_map[ v.gotoui[1] ][ v.gotoui[2] ].prefab.transform.localPosition = Vector3.zero
            end
        else
            if self.cell_map[ v.gotoui[1] ] and self.cell_map[ v.gotoui[1] ][ v.gotoui[2] ] then
                self.cell_map[ v.gotoui[1] ][ v.gotoui[2] ].prefab.transform.localPosition = Vector3.New(0, 1000, 0)
            end
        end
    end
end
...
function C:RefreshHint()
    ...
    if l then
        ...
        local module_cc = GameButtonManager.GetLoadModuleByKey(key)
...
function C:global_select_top_index_msg(parm)
    if self.cell_map[parm.gotoui] then
        local key = parm.gotoui
        local pp = parm.goto_scene_parm
        if pp then
            for k,v in ipairs(self.act_cfg) do
                if v.gotoui[1] == key and v.gotoui[2] == pp then
                    self.select_index = k
                    self:RefreshSelect()
                    break
                end
