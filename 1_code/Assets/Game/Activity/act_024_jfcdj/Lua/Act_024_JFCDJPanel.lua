...
Act_024_JFCDJPanel = basefunc.class()
local C = Act_024_JFCDJPanel
C.name = "Act_024_JFCDJPanel"
--配置相关
local config = Act_024_JFCDJManager.config
--排行榜相关
--改动数据相关
local LOTTERY_TYPE = { "summer_gift_day" }
local Timers = {}
local Anim_Data = {
	step1_time = 1.4,
	step2_time = 3.0,
...
local LotteryStr = "积分"
local His_Prefab = "CommonlotteryinfoPrefab_jfcdj"
-------------------
...
function C:ctor(_parent,backcall)
	...
	self:RandomToSent()
	self:InitUI()
	self.anim_guang = self.start_lottery_btn.transform:GetComponent("Animator")
	self.guang = self.start_lottery_btn.transform:Find("guang")
...
function C:InitUI()
	...
	self.start_lottery_btn.onClick:AddListener(function()
		--抽奖
		if not self.CanStart_By_Info_Num then 
			local pre = HintPanel.Create(1,"恭喜你已获得全部奖励，3D捕鱼中还可以开炮送红包哦",function ()
		...
		if not self.CanStart_By_Info_Ticket then 
			HintPanel.Create(1,"当前积分不足，可以在“我要赚积分”中获得更多积分哦。",function ()
				ActivityYearPanel.Create(nil,nil,{ID = 2})
			end)
			return 
		end
...
function C:ReFreshInfo(_, data)
	...
	Act_024_JFCDJManager.SetMyJiFen(data.ticket_num)
	...
	self.expend_txt.text = "(消耗:  "..self.need_credits..LotteryStr..")"--.."(剩余次数:"..5-self.now_game_num..")"
	self.current_txt.text = self.ticket_num
	self.CanStart_By_Info_Ticket = self.ticket_num >= self.need_credits
	if self.ticket_num >= self.need_credits then
		self.guang.gameObject:SetActive(true)
		self.anim_guang:Play("@start_lottery_btn", -1, 0)
	else
		self.guang.gameObject:SetActive(false)
		self.anim_guang:Play("null", -1, 0)
	end
...
function C:EndLottery()
	if self.award_id ~= nil then
		...
		if config.Award[award_index].real == 1 then
			...
			local pre = RealAwardPanel.Create({ text = config.Award[award_index].type, image = config.Award[award_index].img ,qq = "畅游新世界"})	
			pre:SetQQtext("恭喜获得"..config.Award[award_index].type..",请关注公众号\"畅游新世界/\"联系在线客服领奖")	
