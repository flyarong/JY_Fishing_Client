-- 创建时间:2020-05-12
-- Act_028_MFHFManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_028_MFHFManager = {}
local M = Act_028_MFHFManager
M.key = "act_028_mfhf"
GameButtonManager.ExtLoadLua(M.key, "Act_028_MFHFPanel")
GameButtonManager.ExtLoadLua(M.key, "Act_028_MFHFEnterPanel")
M.config = GameButtonManager.ExtLoadLua(M.key, "act_028_mfhf_config").data[1]
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        ...
        return Act_028_MFHFPanel.Create(parm.parent,Is_Out)
    elseif parm.goto_scene_parm == "enter" then
        return Act_028_MFHFEnterPanel.Create(parm.parent, parm.cfg)
    end
...
function M.IsAwardCanGet()
    if GameItemModel.GetItemCount("prop_mfcjq") >= 1 then
        return true
    end
end
...
function M.on_query_gift_bag_status_response(_,data)
    if data and data.result == 0 then
        ...
        if data.gift_bag_id == M.shopid then
            ...
            Event.Brocast("028MFHF_query_gift_bag_status_msg")
        end
    end
end