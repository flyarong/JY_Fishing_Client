...
function C:MakeLister()
    self.lister = {}
    self.lister["nor_fishing_3d_touzi_stake_response"] = basefunc.handler(self, self.RefreshSettlement)
    self.lister["nor_fishing_3d_touzi_get_score_response"] = basefunc.handler(self, self.on_nor_fishing_3d_touzi_get_score)
...
function C:RefreshSettlement(_, data)
	dump(data,"<color>++++++++++++++++++++nor_fishing_3d_touzi_stake_response+++++++++++++++++</color>")
...
function C:OnGetClick()
	if self.win_or_lose ~= SzwgState.SS_isLose then
		Network.SendRequest("nor_fishing_3d_touzi_get_score", nil, "")
	end
end

function C:on_nor_fishing_3d_touzi_get_score(_, data)
	dump(data,"<color>+++++++++++++++nor_fishing_3d_touzi_get_score_response++++++++++++++</color>")
	if data.result ~= 0 then
        HintPanel.ErrorMsg(data.result)
        self:MyExit()
        return
		...
		for i=1,30 do
			...
			self.seq3:AppendCallback(function ()
				if i == 30 then
					...
					Network.SendRequest("nor_fishing_3d_touzi_stake", {stake = self.tag}, "")	
					self.seq3:Kill()
					self.seq3 = nil
				end
			end)
