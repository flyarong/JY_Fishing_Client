...
Act_002NSCJPanel = basefunc.class()
local C = Act_002NSCJPanel
C.name = "Act_002NSCJPanel"
local config = 	Act_002NSCJManager.config
...
function C:ctor(parent)
	...
	Network.SendRequest("query_one_task_data", {task_id = 21170})
end
	...
	self.lottery1_btn.onClick:AddListener(
		function ()
			if  MainModel.GetHBValue() < 1 then 
				HintPanel.Create(1,"您的福利券不足！")
			else
				Network.SendRequest("box_exchange",{id = 13,num = 1})
			end 
		end
	)
	self.lottery10_btn.onClick:AddListener(
		function ()
			if MainModel.GetHBValue() < 10 then 
				...
				Network.SendRequest("box_exchange",{id = 13,num = 10})
			end 
		end
	)
	self.show_list_btn.onClick:AddListener(
		function ()
			Act_002NSCJListPanel.Create()
		end
	)
	...
	for i=1, 5 do
		self["can"..i.."_get_btn"].onClick:AddListener(
			function ()
				...
				Network.SendRequest("get_task_award_new", {id = 21170, award_progress_lv = i})
...
function C:on_model_task_change_msg(data)
	...
	if data and data.id == 21170 then
...
function C:OnAssetChange(data)
	...
	if data.change_type and data.change_type == "box_exchange_active_award_13" and not table_is_null(data.data) then
