...
function C:InitUI()
	...
	MainModel.GetShareStatusARG(function()
		self:RefreshShares()
	end)
...
function C:DoShare(idx)
	-- local url = ""
	local shareLink
	if gameMgr:IsIOS() then
		shareLink = string.format(share_link_config.share_link[8].link[1])
		...
		shareLink = string.format(share_link_config.share_link[9].link[1])
		-- url = "http://m.www.jyhd919.cn/?from=singlemessage&isappinstalled=0"
	end
	-- local icon = "app_icon"

	-- local shareLink = '{"type": 3, ' .. 
    --               '"title": "万元大奖赛即将开启，门票免费，参与人均奖50元，名额有限速来!", ' .. 
    --               '"description": "万元大奖赛即将开启，门票免费，参与人均奖50元，名额有限速来!", ' .. 
    --               '"url": "' .. url .. '", ' .. 
	-- 	  '"icon": "' .. icon .. '", ' .. 
    --               '"isCircleOfFriends":"true"}'

	ShareLogic.ShareGM(shareLink, function (str)
		print("<color=red>分享完成....str = " .. str .. "</color>")
		if str == "OK" then
			Network.SendRequest("shared_finish", {type="wys_shared", arg = idx})
			MainModel.UserInfo.shareARGtatus = 0
	                MainModel.UserInfo.shareARGvalue = SetBit(MainModel.UserInfo.shareARGvalue, idx)
			self:RefreshShares()
		end
	end)
end
