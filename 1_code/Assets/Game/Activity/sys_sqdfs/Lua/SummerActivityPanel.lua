 ...
 *               代码无BUG!
 ...
 SummerActivityPanel = basefunc.class()
 local C = SummerActivityPanel
 C.name = "SummerActivityPanel"
 local instance
 function C.Create(parent,types)
	 local time_start=config.time[1].starttime
	 local time_end=config.time[1].endtime
	 Event.Brocast("summer_activity_hadclick")
 ...
 function C:MakeLister()
	 ...
	 self.lister["old_player_lottery_base_info_change"] = basefunc.handler(self, self.ReFreshInfo)
	 self.lister["query_old_player_lottery_base_info_response"]=basefunc.handler(self,self.ReFreshInfo)
	 self.lister["old_player_lottery_kaijiang_response"]=basefunc.handler(self,self.Get_KAIJIANG_info)  
	 self.lister["old_player_lottery_get_broadcast_response"]=basefunc.handler(self,self.GetBroadcast)
	 self.lister["ExitScene"] = basefunc.handler(self, self.MyExit)
 end
 
 function C:ReConnecteServerSucceed()
	 Network.SendRequest("query_old_player_lottery_base_info",nil,"")
 end
 
 function C:ReFreshInfo(_,data)
	 --dump(data,"<color=red>----------67-----------</color>")
	 ...
	 self.time=config.time[1].endtime-os.time()
 ...
 function C:MyExit()
	 ...
	 PlayerPrefs.SetInt("summer_lottery",ShowTextLengh)
	 for i=ShowTextLengh,1,-1 do
		 if self.ShowText[i]~="" then
		 PlayerPrefs.SetString("summer_lottery"..i,self.ShowText[i])  
		 end
	 end
	 self.ShowText={}
	 self.ShowFP=nil
 ...
 function C:ctor(parent,types)
	...
	config = CJSQDFSNManager.GetConfigRank()
	 local obj
	 if parent~=nil then 
		 ...
		 if types ~=nil then        
			 obj= newObject("SummerActivityPanelBIG",GameObject.Find("Canvas/LayerLv5").transform)
			 self.deng1="fqj_deng"
 ...
 function C:InitUI()
	 ...
	 self.getawardnum=PlayerPrefs.GetInt("summer"..MainModel.UserInfo.user_id.."alldone",0)
	 PlayerPrefs.SetInt("summer"..os.date("%Y%m%d", os.time())..MainModel.UserInfo.user_id,1)
	 ...
	 self.PHBbutton=self.transform:Find("PHBButton"):GetComponent("Button")
	 self.PHBbutton.onClick:AddListener(
		 function ()
			SummerJFRankPanel.Create()
		 end
	 )
	 ...
	 --     self:ChangeLV(2)
	 -- end)
	 for i = 1, PlayerPrefs.GetInt("summer_lottery",0) do
		 if PlayerPrefs.GetString("summer_lottery"..i,"")~="" then
		 local fp=newObject("CommonlotteryinfoPrefab2",self.transform:Find("RectBG/BG2/Viewport/TaskNode"))
			 fp.transform:Find("Text"):GetComponent("Text").text=PlayerPrefs.GetString("summer_lottery"..i,"")
			 self.ShowFP[#self.ShowFP+1]=fp
			 self.ShowText[#self.ShowText+1]= PlayerPrefs.GetString("summer_lottery"..i,PlayerPrefs.GetString("summer_lottery"..i,""))         
		 end     
	 end
	 self.isUP=false
 ...
 function C:OnStartLottery()
	 ...
	 if  self.JFcurrent<self.JFxiaohao then
		 local h=HintPanel.Create(3,"每充值10元可获得12积分,是否充值?",
		 function ()
	 ...
	 --print("--------2---------")
	 Network.SendRequest("old_player_lottery_kaijiang",nil,"")    
 end
 ...
 function C:Get_KAIJIANG_info(_,data)
	 ...
	 if data.result==0 then
		 ...
		 PlayerPrefs.SetInt("summer"..MainModel.UserInfo.user_id.."alldone",self.getawardnum)
 ...
 function C:ShowLotteryInfo()
	 if   IsEquals(self.gameObject) then
		 Network.SendRequest("old_player_lottery_get_broadcast")		
	 end
 end
 function C:OnAssetChange(data)
	 dump(data,"<color=red>----奖励类型-----</color>")
	 if data.change_type and data.change_type == "old_player_lottery_award" then
		 self.cur_award = data
	 end
 end
 function C:GetMask()
	 dump(self.now_game_num,"<color=red>---------<color>")
 ...
 function C:choujiang()
	 if self.award_id~= nil then
		 ...
		 if self.JPcfg[self.award_id].real ==1 then
			 ...
			 HintCopyPanel.Create({desc=string1, copy_value="4008882620"})
		 end
		 Network.SendRequest("query_old_player_lottery_base_info")
		 local fp=newObject("CommonlotteryinfoPrefab2",self.transform:Find("RectBG/BG2/Viewport/TaskNode"))
