...
function C:MakeLister()
	...
	shopids = SYSXSFLManager.GetShopIds_1()
	for i=1,#shopids  do
...
function C:RefreshNum()
	...
	for i=1,#ids do
		...
		if self.NumData[ids[i]]~=nil then 
			...
			if  self:IsDuring() then
				if self.NumData[ids[i]]<=0 then 
					self.NumData[ids[i]]=0 
				else	
					YearXSFLpanel.IsGoodsHave = true 
				end
				self.Childs[i].transform:Find("SY"):GetComponent("Text").text="剩余"..self.NumData[ids[i]].."份"
				dump(ids[i],"<color=yellow><size=15>++++++++++ids[i]++++++++++</size></color>")
				dump(MainModel.GetGiftShopStatusByID(ids[i]),"<color=yellow><size=15>++++++++++MainModel.GetGiftShopStatusByID(ids[i])++++++++++</size></color>")
				if not MainModel.GetGiftShopStatusByID(ids[i]) or MainModel.GetGiftShopStatusByID(ids[i]) == 0  then 
