-- 创建时间:2020-05-12
-- Panel:Act_040_MSLBBeforeBuyPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
Act_040_MSLBBeforeBuyPanel = basefunc.class()
local C = Act_040_MSLBBeforeBuyPanel
C.name = "Act_040_MSLBBeforeBuyPanel"
...
function C:MakeLister()
    ...
    self.lister["model_mslb_qfxl_num_change_msg"] = basefunc.handler(self,self.on_model_mslb_qfxl_num_change_msg)
end
...
function C:InitUI()
	...
	self:on_model_mslb_qfxl_num_change_msg()
end
...
function C:on_model_mslb_qfxl_num_change_msg()
	if M.GetQFXLNum() <= 0 then
...
function C:OnBuyClick()
	if M.GetQFXLNum() <= 0 then
		self:BuyShop(M.qfxl_gift_id2)
	else
		self:BuyShop(M.qfxl_gift_id)
	end
end
