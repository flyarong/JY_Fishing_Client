-- 创建时间:2020-05-12
-- Panel:Act_040_MSLBLotteryPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
Act_040_MSLBLotteryPanel = basefunc.class()
local C = Act_040_MSLBLotteryPanel
C.name = "Act_040_MSLBLotteryPanel"
...
function C:MakeLister()
    ...
    self.lister["model_mslb_unrealy_change_msg"] = basefunc.handler(self,self.on_model_mslb_unrealy_change_msg)
    self.lister["model_mslb_lottery_change_msg"] = basefunc.handler(self,self.on_model_mslb_lottery_change_msg)
...
function C:InitUI()
	self.lottery_btn.onClick:AddListener(function ()
		Network.SendRequest("welfare_activity_lottery",{act_type=M.act_type})
		self.now_status = Status.lottery
...
function C:InitAward()
	local config = M.act_config.awardConfig
...
--虚假数据跑马灯
function C:on_model_mslb_unrealy_change_msg()
	...
	self.seq:OnKill(function ()
		...
		M.DeletPMDList(M.GetUnrealyPlayerName(),M.GetUnrealyAwardID())
	end)
...
function C:on_model_mslb_lottery_change_msg()
	self.times_txt.text = "剩余次数:"..(M.GetLotteryNum() or "0")
...
function C:on_model_mslb_data_change_msg()
	self.times_txt.text = "剩余次数:"..(M.GetLotteryNum() or "0")
	if self.now_status == Status.none then
