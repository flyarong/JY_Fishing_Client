...
M.key = "act_025_lxdh"
Act_025_LXDHManager.config = GameButtonManager.ExtLoadLua(M.key,"act_025_lxdh_config")
GameButtonManager.ExtLoadLua(M.key,"Act_025_LXDHItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_025_LXDHPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_025_LXDHPanel_new")
local this
local lister
local gift_ids
local task_map_ids
...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1598284799
    local s_time = 1597707000
    if (e_time and os.time() > e_time) or (s_time and os.time() < s_time) then
        return false
    end

    -- 对应权限的key
    local _permission_key = "lxdh_025_notnew"
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            if M.now_level == 1 then
                return Act_025_LXDHPanel.Create(parm.parent,parm.backcall)
            elseif M.now_level == 2 then
                return Act_025_LXDHPanel_new.Create(parm.parent,parm.backcall)
            end
        end 
    else
...
function M.query_data()
    Network.SendRequest("query_activity_exchange",{type = type})
end
