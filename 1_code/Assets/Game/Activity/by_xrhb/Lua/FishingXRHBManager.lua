...
function M.IsActive()
    ...
    local _permission_key = "drt_block_buyu_hongbao_task" -- 屏蔽捕鱼红包任务
...
function M.InitUIConfig()
    this.UIConfig = {}
    local cfg = {}
    cfg[#cfg + 1] = {game_id = 1, task_id = 68, red_val=1000}
    cfg[#cfg + 1] = {game_id = 2, task_id = 76, red_val=5000}
    cfg[#cfg + 1] = {game_id = 3, task_id = 77, red_val=20000}
...
local function set_ui(obj, b)
	local img1 = obj.transform:Find("byxrhb_hongbao/qipao"):GetComponent("Image")
	...
	local txt1 = obj.transform:Find("byxrhb_hongbao/qipao/Text"):GetComponent("Text")
	local c = 1
	if b then
		c = 1
	else
...
function M.on_query_one_task_data(_, data)
	if data.task_data and data.task_data.id and this.UIConfig.cfg_task_id_map[data.task_data.id] then
		dump(data, "<color=red>BY_XRHB on_query_one_task_data</color>")
		if not m_data then
			m_data = {}
		end
...
function M.on_query_send_list_fishing_msg(tag)
	print(tag)
	dump(m_data, "<color=red>on_query_send_list_fishing_msg</color>")
	if tag == "by_xrhb" then
...
function M.GetTaskData(id)
	if m_data and m_data.task_map then
		if id == 68 then -- 去掉捕鱼中浅水湾的10福利券奖励任务
