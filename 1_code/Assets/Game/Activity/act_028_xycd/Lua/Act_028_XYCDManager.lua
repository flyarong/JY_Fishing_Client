-- 创建时间:2020-04-13
-- Act_028_XYCDManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_028_XYCDManager = {}
local M = Act_028_XYCDManager
M.key = "act_028_xycd"
Act_028_XYCDManager.config = GameButtonManager.ExtLoadLua(M.key,"act_028_xycd_egg_config")
Act_028_XYCDManager.shop_icon_config = GameButtonManager.ExtLoadLua(M.key,"act_028_xycd_shopicon_config")
Act_028_XYCDManager.goodsid_config = GameButtonManager.ExtLoadLua(M.key,"act_028_xycd_shopid_config")
GameButtonManager.ExtLoadLua(M.key,"Act_028_XYCDPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_028_XYCDEnterPrefab")
GameButtonManager.ExtLoadLua(M.key,"Act_028_XYCDItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_028_XYCDIconInGame")
local this
local lister
local gift_ids
local task_map_ids
local gift_data
local start_time = 1599523200
local end_time = 1600099199
M.now_level = 0
M.item_key = "prop_xycd_sunshine"

local permisstions = {
    "actp_buy_gift_bag_class_xycd_028_xycd_v1",
    "actp_buy_gift_bag_class_xycd_028_xycd_v4",
}
...
function M.IsActive()
    ...
    -- 对应权限的key
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_p_028_xycd", is_on_hint = true}, "CheckCondition")
    if a and not b then
        --return false
    end
    dump(M.GetNowPerMiss(),"<color=red>幸运彩蛋权限</color>")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_028_XYCDPanel.Create(parm.parent,parm.backcall)
        end 
    elseif parm.goto_scene_parm == "enter" then
        if M.CheckIsShow() then
            return Act_028_XYCDEnterPrefab.Create(parm.parent, parm.cfg)
        end 
    else
...
function M.InitUIConfig()
    gift_ids = {}
    task_map_ids = {}
    for i=1,#Act_028_XYCDManager.config.Info do
        gift_ids[i] = gift_ids[i] or {}
        for k1, v1 in ipairs(Act_028_XYCDManager.config.Info[i]) do
...
function M.on_AssetChange(data)
    ...
    if M.CheckIsShow() and data then
        if data.change_type then
            if string.sub(data.change_type,1,14) ~= "xycd_ljyj_task" then
                for k,v in ipairs(data.data) do
                    if v.asset_type == M.item_key then
                        M.Refresh()
                        return
                    end
                end
        ...
        if data.data and data.data[1] and data.data[1].value then
            Event.Brocast("xycdManager_CreateSunItemInGame",data.data[1].value)
        end
        M.Refresh()
    end
end
...
function M.GetSunCount()
    --dump(GameItemModel.GetItemCount(M.item_key),"<color=green>++++++++++++++++++</color>")
        ...
        for k,v in pairs(data.pay_data) do
            if v.data.gift_id and MainModel.GetGiftShopStatusByID(v.data.gift_id) == 1 then
                ...
                if M.CheckIsShow() then
                    ...
                    local obj = newObject("Act_028_XYCDIconInShop", temp_ui.act_node)
...
function M.IsTYCDGiftID(id)
    if id == 10292 or id == 10297 then
        return true
    end
end


function M.IsHaveRemainTime()
