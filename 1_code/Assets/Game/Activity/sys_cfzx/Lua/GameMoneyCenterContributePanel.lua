	...
	if data then
		for i,v in ipairs(data.detail_infos) do
			...
			if not self:IsItemExist(itemName) then
				...
				item_table.money_txt.text = string.format("+%s元奖金",StringHelper.ToRedNum(v.treasure_value))
				local is_active = ""
				...
				if v.treasure_type == 1 then
					treasure_type = string.format("完成了新人红包一天所有任务")
				elseif v.treasure_type <= 100 then
					treasure_type = string.format("完成新人红包第%s天", v.treasure_type)
...
function GameMoneyCenterContributePanel:UpdateBaseInfoUI()
	...
	if data and sczd_data then
		...
		self.contribute_info_txt.text = string.format( "新人红包第%s天 ：总贡献%s元", data.son_bbsc_progress, StringHelper.ToRedNum(data.son_bbsc_gx))
		self.vip_info_txt.text = string.format( "购买金猪礼包：贡献%s元", StringHelper.ToRedNum(data.son_tgli_gx))
		if not data.son_vip_lb_gx then data.son_vip_lb_gx = 0 end
		self.vip_gift_info_txt.text = string.format( "购买全返礼包：贡献%s元", StringHelper.ToRedNum(data.son_vip_lb_gx))
