...
function C:RefreshMoney()
	self.redpacket_txt.text = StringHelper.ToRedNum(MainModel.UserInfo.cash)
end
...
function C:UpdateGoldPigProfitCache()
	...
	if profit_cache and profit_cache ~= 0 then
		self.goldpig_cache_txt.text = StringHelper.ToRedNum(profit_cache  )
...
function C:OnGoldPigCacheClick()
	...
	local str = string.format( "购买金猪礼包后可领取%s元奖金，是否立刻前往购买",StringHelper.ToRedNum(goldpig_cache ))
