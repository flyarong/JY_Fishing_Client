...
local phb_begintime = 1572910200
local phb_endtime = 1573574399
...
function C:UpdateFriendCount()
    if self.cur_panel then
        if MainModel.UserInfo.cash then
            self.myfriend_num_txt.text = "<color=#FFFFCBFF>" .. StringHelper.ToRedNum(MainModel.UserInfo.cash) .. "元</color>"
        else
