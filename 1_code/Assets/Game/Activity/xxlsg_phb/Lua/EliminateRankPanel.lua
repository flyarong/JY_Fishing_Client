...
function C:MakeLister()
	self.lister = {}
	self.lister["query_xiaoxiaole_once_game_rank_base_info_response"] = basefunc.handler(self,self.onMyInfoGet)
	self.lister["query_xiaoxiaole_once_game_rank_response"] = basefunc.handler(self,self.onInfoGet)
	self.lister["query_xiaoxiaole_once_game_rank_history_response"] = basefunc.handler(self,self.onHistoryInfoGet)
end
	...
	if data and data.result ==0 and IsEquals(self.gameObject) then 
		for i = 1, #data.rank_data do
			...
			if data.rank_data[i].player_id == MainModel.UserInfo.user_id then 
				b.transform:Find("BG"):GetComponent("Image").sprite=GetTexture("xxl_bg_phb1")
			end 
...
function C:SentQuery()
	Network.SendRequest("query_xiaoxiaole_once_game_rank_base_info")
	Network.SendRequest("query_xiaoxiaole_once_game_rank")
	Network.SendRequest("query_xiaoxiaole_once_game_rank_history")
end
