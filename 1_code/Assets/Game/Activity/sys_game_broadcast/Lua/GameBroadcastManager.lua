...
GameBroadcastManager.dotween_key = "dotween_key_GameBroadcastManager"
...
function GameBroadcastManager.OnExitScene()
    ...
    DOTweenManager.KillAllLayerTween(GameBroadcastManager.dotween_key)
end
...
function GameBroadcastManager.StartManager()
    ...
    this.JcInfoMsg = basefunc.queue.New()
...
function GameBroadcastManager.on_multicast_msg(_, msg)
     if MainModel.IsLowPlayer() then return end
...
--临时添加屏蔽冲金鸡关键字
    ...
    --         "捕鱼","洪福巨奖","周末分大奖","游客",
    --     }
    --     for i,v in ipairs(block_words) do
    --         if string.find(msg.content,v) then
    --            return
    --         end
    --     end
    -- end
...
--]]
    ...
    elseif msg.type == 5 then
        --捕鱼奖池广播
        this.JcInfoMsg:push_back({key=key, msg=msg, time=os.time()})
        if this.JcInfoMsg:size()>50 then
            this.JcInfoMsg:pop_front()
        end
...
function GameBroadcastManager.PlayJCInforFinish(key)
    BY3DPmdPrefabPanel.PlayFinish(key)
end
...
function GameBroadcastManager.GetJCInforFront()
    if this.JcInfoMsg then
        return this.JcInfoMsg:pop_front()
    end
end
