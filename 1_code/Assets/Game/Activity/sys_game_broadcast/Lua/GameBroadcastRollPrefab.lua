...
local instance = nil
function GameBroadcastRollPrefab.Create(data, parent)
	instance = GameBroadcastRollPrefab.New(data, parent)
	return instance
end
function GameBroadcastRollPrefab:ctor(data, parent)
	self.data = data
    self.gameObject = newObject("GameBroadcastRollPrefab", parent)
    self.transform = self.gameObject.transform
    local tran = self.transform
    tran.localPosition = Vector3.New(520, 0, 0)
    self:InitGotoButton()
	local text = tran:Find("Text"):GetComponent("Text")
	text.text = data.msg.content
	local ww = text.preferredWidth

	self.seqMove = DoTweenSequence.Create({dotweenLayerKey=GameBroadcastManager.dotween_key})
