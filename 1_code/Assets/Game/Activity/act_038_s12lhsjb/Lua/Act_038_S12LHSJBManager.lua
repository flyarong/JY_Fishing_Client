-- 创建时间:2020-10-26
-- Act_038_S12LHSJBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_038_S12LHSJBManager = {}
local M = Act_038_S12LHSJBManager
M.key = "act_038_s12lhsjb"
GameButtonManager.ExtLoadLua(M.key, "Act_038_S12LHSJBPanel")
local this
local lister
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        return Act_038_S12LHSJBPanel.Create(parm.parent)
    else
...
function M.query_rank_base_info_response(_,data)
    dump(data,"query_rank_base_info +++++++++++++++++++++")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        Event.Brocast("act_038_s12lhsjb_base_info_get",{rank_type = data.rank_type})
    end
end
...
function M.ShowNoticePrefab()
    ...
    if MainModel.myLocation == "game_EliminateXY" or 
            MainModel.myLocation == "game_Eliminate" or 
            MainModel.myLocation == "game_EliminateSH" or 
            MainModel.myLocation == "game_EliminateCS" or 
            MainModel.myLocation == "game_EliminateXY" or 
            MainModel.myLocation == "game_ZPG" or
            MainModel.myLocation == "game_FishingDR" or
            MainModel.myLocation == "game_Fishing" or 
            MainModel.myLocation == "game_LWZB" or
            MainModel.myLocation == "game_LWZBHall" or
            MainModel.myLocation == "game_Zjd" then
        M.StopNoticeTime()
        if btn_gameObject and IsEquals(btn_gameObject) and IsEquals(notice_prefab) then
            local time_index = 0
            local space = 60
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_038_S12LHSJBNoticePrefab",btn_gameObject.transform)
        notice_prefab.transform.localPosition = Vector2.New(168,64)
        notice_prefab.gameObject:SetActive(false)
        M.StopNoticeTime()
       -- M.ShowNoticePrefab()
    end
