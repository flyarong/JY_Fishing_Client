...
function C.Create(award)
    if not AdvertisingManager.IsHaveAD() then
        Event.Brocast("AssetGet",award)
        return
    end

	local a,b = GameButtonManager.RunFun({gotoui="sys_qx"}, "get_tag_vec_map")
    if a and b then
	    if b[TagVecKey.tag_free_player] then
			return C.New(award)
	    else
	    	Event.Brocast("AssetGet",award)
	    end
...
function C:MyRefresh()
    ...
    if a and b then
        self.ew_hint_txt.text = "额外获得750金币"
    else
        self.ew_hint_txt.text = "额外获得1500金币"
    end
end
