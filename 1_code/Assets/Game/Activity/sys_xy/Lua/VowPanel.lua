	...
	self.LQButton.onClick:AddListener(
		function () 
			...
			if MainModel.UserInfo.jing_bi>=0 then 			
				...
				local h=HintPanel.Create(3,"金币不足，是否充值?",
				function ()
...
function C:UpdateTime()
	...
	if IsEquals(self.Timer) then
		...
		self.TimerText.text =  "每日8点-10点免费领"
...
function C:ShowLotteryInfo(parent,name,award_name)
	if   IsEquals(self.GunDong_Node) and IsEquals(self.GuDing_Node) then 
		...
		b.transform:GetComponent("Text").text=self:data2str(name,award_name)
	end 
end
...
function C:ChangeStatus(time)
	if time then
		if time>7200 and self.b_status~= button_status[2] and self.data and self.data.is_vow==1 then 
			...
			--不可领取状态
		elseif time >0 and time <=7200 and  self.b_status~= button_status[3] and self.data and self.data.is_vow==1 then
