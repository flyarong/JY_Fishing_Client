...
Act_038_S12DHHLPanel = basefunc.class()
local C = Act_038_S12DHHLPanel
C.name = "Act_038_S12DHHLPanel"
local M = Act_038_S12DHHLManager
C.instance = nil

local help_info1 = {
"1.活动时间：12月8日8:00-12月14日23:59:59",
"2.活动期间，玩所有游戏都有几率获得礼盒道具，积累礼盒道具可兑换奖励",
"3.活动结束后，所有的礼盒道具将统一清除，请及时进行奖励兑换",
"4.实物奖励请关注公众号《鲸鱼初纪元》联系在线客服领取",
...
function C:CreateItemPrefab()
	...
	for i=1,#self.cur_data do
		local pre = Act_038_S12DHHLItemBase.Create(self.Content.transform,self.cur_data[i])
		if pre then
...
function C:on_XGDH_toggle_set_false_msg()
	if self.Toggle_tge.isOn then
		...
		et = et + 1
		PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."HJHHL",et)
	end
