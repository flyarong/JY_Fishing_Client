...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1603728000
    local s_time = 1603152000
    if (e_time and os.time() > e_time) or (s_time and os.time() < s_time) then
        return false
    end

    -- 对应权限的key
    local _permission_key = "actp_own_task_1000070"
...
function M.GetHintState(parm)
    if parm and parm.gotoui == M.key then 
        if M.IsCanGetAward()==1 --[[or not PlayerPrefs.HasKey(MainModel.UserInfo.user_id.."yzqj_1020")--]] then
