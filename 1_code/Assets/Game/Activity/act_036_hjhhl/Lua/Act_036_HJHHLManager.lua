-- 创建时间:2
--05-06
-- Act_036_HJHHLManager 管理器

local basefunc = require "Game/Common/basefunc"

Act_036_HJHHLManager = {}
local M = Act_036_HJHHLManager
M.key = "act_036_hjhhl"
Act_036_HJHHLManager.config = GameButtonManager.ExtLoadLua(M.key,"act_036_hjhhl_config")
Act_036_HJHHLManager.shop_icon_config = GameButtonManager.ExtLoadLua(M.key,"act_036_hjhhl_shopicon_config")
Act_036_HJHHLManager.goodsid_config = GameButtonManager.ExtLoadLua(M.key,"act_036_hjhhl_shopid_config")
GameButtonManager.ExtLoadLua(M.key,"Act_036_HJHHLItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_036_HJHHLPanel")
local this
local lister
local gift_ids
local task_map_ids
...
M.now_level = 0
M.item_key = "prop_gej_hj"
local start_time = 1606176000
local end_time = 1606751999
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_036_HJHHLPanel.Create(parm.parent,parm.backcall)
        end 
    else
        ...
        for k,v in pairs(data.pay_data) do
            if v.data.gift_id and MainModel.GetGiftShopStatusByID(v.data.gift_id) == 1 then
                ...
                if M.CheckIsShow() then
                    ...
                    local obj = newObject("Act_036_HJHHLIconInShop", temp_ui.act_node)
