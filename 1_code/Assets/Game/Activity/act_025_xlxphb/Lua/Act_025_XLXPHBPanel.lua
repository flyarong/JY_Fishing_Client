...
Act_025_XLXPHBPanel = basefunc.class()
local C = Act_025_XLXPHBPanel
C.name = "Act_025_XLXPHBPanel"
local M = Act_025_XLXPHBManager
local HGList={
	[1] ="localpop_icon_1",
	[2] ="localpop_icon_2",
	[3] ="localpop_icon_3",
...
local Award={
	20000,10000,5000,3000,3000,3000,2000,2000,2000,2000,1000,1000,1000,1000,1000,500,500,500,500,500
}
local base_txt = {
	[1] = {"击杀龙虾BOSS鱼记录累计赢金，参与排行榜!","炮倍越高上榜的机会越大哟!"},
	[2] = {"击杀背着龙虾券的鱼和龙虾BOSS鱼掉落龙虾兑换券，参与排行榜！","炮倍越高上榜的机会越大哟!"},
}
local curr_index = 1
local DESCRIBE_TEXT = {
	[1] = "1.活动时间：8月18日7:30-8月24日23:59:59",
	[2] = "2.活动期间3d捕鱼中会出现背着龙虾券的鱼和龙虾BOSS鱼，击杀可参与排行榜",
	[3] = "3.排行榜数据相同时，上榜时间越早排名越靠前，炮倍越大上榜机会越大",
...
function C:onInfoGet(data)
	if data and data.result ==0 and IsEquals(self.gameObject) then 
		for i = 1, #data.rank_data do
			...
			self["goto"..curr_index.."_btn"].onClick:AddListener(
				function ()
					GameManager.GuideExitScene({gotoui = "game_Fishing3DHall"},ActivityYearPanel.Close())
				end
			)
			self["goto"..curr_index.."_btn"].gameObject:SetActive(false)
			...
			if data.rank_data[i].rank < 4 then
				...
				self["goto"..curr_index.."_btn"].gameObject:SetActive(true)
			else
			...
			if i%2 == 1 then
				self["I"..curr_index.."_info1_img"].sprite = GetTexture("")
			else
				self["I"..curr_index.."_info1_img"].sprite = GetTexture("lxdhj_bg_lxdhj3")
			end
...
function C:on_query_rank_data_response(_,data)
	dump(data,"<color=red>捕鱼排行榜数据--</color>")
