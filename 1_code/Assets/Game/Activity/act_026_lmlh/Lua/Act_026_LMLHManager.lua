-- 创建时间:2020-05-06
-- Act_026_LMLHManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_026_LMLHManager = {}
local M = Act_026_LMLHManager
M.key = "act_026_lmlh"
Act_026_LMLHManager.shop_icon_config = GameButtonManager.ExtLoadLua(M.key,"act_026_shop_icon_config")
Act_026_LMLHManager.item_config = GameButtonManager.ExtLoadLua(M.key,"act_026_item_config")
Act_026_LMLHManager.goodsid_config = GameButtonManager.ExtLoadLua(M.key,"act_026_goods_id_config")
GameButtonManager.ExtLoadLua(M.key,"Act_026_LMLHIconInGame")
GameButtonManager.ExtLoadLua(M.key,"Act_026_LMLHItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_026_LMLHPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_026_LMLHPanel_New")
local this
local lister
local start_time = 1598311800
local end_time = 1598889599
...
function M.IsActive()
    ...
    -- 对应权限的key
    local _permission_key = "actp_lmqx_026_lmlh"
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            if M.now_level == 1 then
                return Act_026_LMLHPanel.Create(parm.parent,parm.backcall)
            elseif M.now_level == 2 then
                return Act_026_LMLHPanel_New.Create(parm.parent,parm.backcall)
            end
        end 
    else
...
function M.on_AssetChange(data)
    ...
    if M.CheckIsShow() then
        if data.change_type then
            if string.sub(data.change_type,1,20) ~= "task_p_026_lmqx_lmlh" then
...
function M.CreateItemInGame(score,tran)
    local pre = Act_026_LMLHIconInGame.Create(score,tran)
...
function M.on_PayPanel_GoodsChangeToGift(data)
    ...
    if  start_time < os.time() and os.time() < end_time and M.IsActive() then
        ...
        for k,v in pairs(data.pay_data) do
            --dump({v = v,status = MainModel.GetGiftShopStatusByID(v.data.gift_id)},"<color=yellow>++++++++++++</color>")
            if v.data.gift_id and MainModel.GetGiftShopStatusByID(v.data.gift_id) == 1 then
                ...
                if M.CheckIsShow() then
                    ...
                    local obj = newObject("Act_026_LMLHIconInShop", temp_ui.act_node)
...
function M.RefreshItems()
    for k ,v in pairs(items.jing_bi) do
        --dump({k=k,v=v},"<color=red>-----//111//----------</color>")
        local data = GameTaskModel.GetTaskDataByID(k)
        --dump(data,"<color=red>-----//222//----------</color>")
