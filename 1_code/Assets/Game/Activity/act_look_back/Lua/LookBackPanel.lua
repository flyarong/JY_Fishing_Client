...
function C:InitChild()
	...
	self.Money_text.text="金币 x"..self.basedata.player_data.award_jingbi
	...
	self.Button.onClick:AddListener(
		function ()
			...
			ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
			SharePanel.Create("lookback",self.parm,function()
				Network.SendRequest("znq_look_back_kaijiang")
				Network.SendRequest("query_znq_look_back_base_info")
				Network.SendRequest("query_znq_look_back_kaijiang_base_info",nil,"请求数据")
				self:InitChild()
