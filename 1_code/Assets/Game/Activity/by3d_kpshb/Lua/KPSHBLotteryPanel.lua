...
function C.Create(data, parent, gameType)
	return C.New(data, parent, gameType)
end
...
function C:ctor(data, parent, gameType)
...
function C:InitUI()
	if self.gameType == M.GameType.GT_JJ then
		self.title_img.sprite = GetTexture("hbdzp_imgf_wsdby_jj")
	end
	self.task_data = M.GetTaskDataByID( M.GetTaskID(FishingModel.game_id, self.gameType) )
	self.wc_lv = M.GetCurTaskFinishLv(self.gameType)
	...
	self.ui_config = {}
	self.ui_config[#self.ui_config + 1] = {index=1, name = "普通福利券"}
	self.ui_config[#self.ui_config + 1] = {index=2, name = "高级福利券", hint="省5%", hint_img="hbdzp_icon_bql"}
	self.ui_config[#self.ui_config + 1] = {index=3, name = "超级福利券", hint="省10%", hint_img="hbdzp_icon_bqz"}
...
function C:RefreshJD()
	if self.anim_state == KPSHBLotteryPanel.AnimState.AS_Null and self.xz_lv > self.wc_lv then
		self.hint_txt.text = string.format("差<color=red>%s</color>炮可抽<color=red>%s</color>", 
			M.GetGunRateSurNum(self.xz_lv, self.gameType), self.ui_config[self.xz_lv].name)	
	end
end
-- 中间福袋显示
function C:RefreshFD()
	self:CloseCellFD()
	self.index_list = {}
	self.config = M.GetHBRateConfigByIDIndex(FishingModel.game_id, self.xz_lv, self.gameType)
	self.asset_config = M.GetAssetByIDIndex(FishingModel.game_id, self.xz_lv, self.gameType)
...
function C:buyu_spend_lottery_task_award(_, data)
	...
	if data.result == 0 then
		self.award_index = C.GetIndexByAward(data.award_list[1], self.asset_config)
...
function C:SendGetTaskAward()
	Network.SendRequest("get_task_award_new", {award_progress_lv = self.xz_lv, id = M.GetCurrTaskID(self.gameType)}, "发送请求")
end
...
function C.GetIndexByAward(award,config)
	dump(award, "<color=red>award //// ddddddddd </color>")
	dump(config, "<color=red>award //// ddddddddd </color>")
	for i,v in ipairs(config) do
	    if tonumber(v.value) == tonumber(award.asset_value) and v.type == award.asset_type then
			return i
		end
	end
	return 1
...
function C:ShowHelpPanel()
	KPSHBSMPrefabPanel.Create(self.gameType) 
end

function C:OnAssetChange(data)
	if data.change_type and (data.change_type == "buyu_fire_award_hongbao_task" or data.change_type == "buyu_2d_leiji_dapao_new_task") then
