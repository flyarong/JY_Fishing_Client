...
function C.Create(parm, gameType)
	return C.New(parm, gameType)
end
...
function C:InitUI()
    ...
    self.goto_btn.onClick:AddListener(function ()
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		M.GuidePlayerGoGJC(true, self.gameType)
	end)
...
function C:MyRefresh()
	self.ui_config = M.GetConfigByGameID(FishingModel.game_id, self.gameType)

	for k,v in ipairs(self.ui_config.hb) do
		self["jiangli"..k.."_txt"].text = ""..v		
	end
	self.hint_red_txt.text = "最高" .. StringHelper.ToCash(self.ui_config.show_hb)
...
function C:RefreshJD()
	self.need_txt.text = string.format("差%s炮", M.GetGunRateSurNum( M.GetCurTaskLv(self.gameType) , self.gameType))	
end
...
function C:RefreshProgressUI()
	local task = GameTaskModel.GetTaskDataByID( M.GetCurrTaskID(self.gameType) )
	local lv = M.GetCurTaskLv(self.gameType)
	if task then
	...
	if M.GetCurTaskFinishLv(self.gameType) == 3 then
...
function C:RefreshTaskLv()
	...
	local b = M.IsCanGetAward(self.gameType)
	self.choujiang_btn.gameObject:SetActive(b)
	self.choujiang_no_btn.gameObject:SetActive(not b)

	if M.IsRedGetReachMax( FishingModel.game_id , self.gameType) then
...
function C:OnCJClick()
	GameManager.GotoUI({gotoui="sys_flqcj", goto_scene_parm = "panel", data=3})
end
...
function C:NoCJClick()
	...
	LittleTips.Create(string.format( "还差%s炮可抽%s。", M.GetGunRateSurNum( M.GetCurTaskLv(self.gameType), self.gameType ), name))	
end