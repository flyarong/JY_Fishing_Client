...
function C:MyRefresh()
	self.hb_txt.text = StringHelper.ToCash(M.GetTaskMaxNumByLv( M.GetCurTaskLv(self.gameType), self.gameType ))

	self:RefreshTaskLv()
	self:RefreshSYP()
end
function C:RefreshSYP()
	self.need_txt.text = M.GetGunRateSurNum( M.GetCurTaskLv(self.gameType) , self.gameType)	
end
