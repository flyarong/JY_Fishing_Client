-- 创建时间:2021-03-12
-- Act_053_XCNSManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_053_XCNSManager = {}
local M = Act_053_XCNSManager
M.key = "act_053_xcns"
local config = GameButtonManager.ExtLoadLua(M.key,"act_053_xcns_config")
GameButtonManager.ExtLoadLua(M.key,"Act_053_XCNSPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_053_XCNSItem")
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        return Act_053_XCNSPanel.Create(parm.parent)
    end
...
function M.SetTaskData()
	...
	if _cur_task_data then
		...
		SetData(b, _cur_task_data.now_total_process/100, _cur_task_data.need_process)
	else
		local status = {}
