...
ACT_001BSYYManager = {}
local M = ACT_001BSYYManager
M.key = "act_001_bsyy"
GameButtonManager.ExtLoadLua(M.key, "ACT_001BSYYPanel") 
GameButtonManager.ExtLoadLua(M.key, "ACT_001BSYYPanel_Out") 
local lister
local m_data
local activity_id_year = 47
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return ACT_001BSYYPanel.Create(parm.parent)
...
function M.on_act_match_order_msg_change(data)
    if  os.time() > 1582587000 and os.time() < 1582981200 then
        ...
        --只在当日做变化
        if os.time() > 1582905600 and os.time() < 1582981200 then 
            --大厅跳转
