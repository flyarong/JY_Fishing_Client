	...
	if data and data.id== 107 then
		...
		for i = 1, #b do
			...
			if b[i]==2 then 
				...
				self.AwardChilds[i].transform:Find("Button"):GetComponent("Image").sprite=GetTexture("gy_35_7")
				self.AwardChilds[i].transform:Find("Button"):GetComponent("Button").enabled=false
		   	end     
		end
		if os.time()<config.timecfg[1].time_end then
...
function C:goshare()
	SharePanel.Create("activity","hall_share")
end
function C:showHelp()
	...
	.."\n".."2,仅统计每日首次分享;"
	.."\n".."3,完成12次分享任务即可获得瓜分8888红包的资格;"
	.."\n".."4,红包瓜分日期为7月30日12点-7月31日24点;"
...
function  C:Chai()
	...
	if self.now_total_process>=12 then  --config.model[1].testmodel==1 then
		...
		if os.time()>time_end or os.time()<time_start then 
			if config.model[1].testmodel==0 then 
				HintPanel.Create(1, "恭喜您获得瓜分资格,"..os.date("%m月%d日 %H点",time_start).."后可拆红包哦!")
				return 
			end 
		end
		--发送Chai红包请求
		Network.SendRequest("query_everyday_share_award",nil,"正在拆红包",
			function(data)
				dump(data,"<color=red>--------------</color>")
	    ...
	    HintPanel.Create(1,"您还差"..(12-self.now_total_process).."次分享可瓜分红包!")
	end
end
