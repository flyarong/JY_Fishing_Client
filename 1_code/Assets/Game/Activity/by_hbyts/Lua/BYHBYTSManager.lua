...
function M.IsActive()
    ...
    if MainModel.myLocation ~= "game_Fishing3D"
        ...
        or this.m_data.down_t <= os.time() 
        or FishingModel.game_id==2 
        or FishingModel.game_id==3  then
        return false
    end
...
function M.CheckDownTime()
    M.CalcDownTime()
    if this.m_data.down_t and this.m_data.down_t > os.time() then
        Event.Brocast("ui_button_data_change_msg", {key=M.key})
    end
end
-- @dump(BYHBYTSManager.m_data)
-- @dump(BYHBYTSManager.GetDownTime()-os.time())
