...
SYSByPmsGameRankItem_pm = basefunc.class()
local C = SYSByPmsGameRankItem_pm
C.name = "SYSByPmsGameRankItem_pm"

function C.Create(parent,data,id,type)
	return C.New(parent,data,id,type)
end
...
function C:ctor(parent,data,id,type)
	self.data = data
	self.id = id
	self.type = type
...
function C:InitUI()
	...
	elseif self.type == "hks" then
		self.player_best_score_txt.text = "最佳分数:"..(self.data.score / 100)
	end
	...
	self:SetHead()
	local award_list = SYSByPmsManager.GetPMSAwardCfgByRank(self.id,self.data.rank,self.type)
	...
	if #award_list == 1 then
		...
		self.rank_award1_txt.text = StringHelper.ToCash(award_list[1].num)
