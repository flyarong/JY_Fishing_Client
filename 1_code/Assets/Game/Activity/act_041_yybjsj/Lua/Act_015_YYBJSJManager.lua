...
M.key = "act_041_yybjsj"
M.config = GameButtonManager.ExtLoadLua(M.key, "act_041_yybjsj_config")
...
GameButtonManager.ExtLoadLua(M.key, "Act_015_YYBJSJActPanel")
...
function M.IsActive()
    -- 活动的开始与结束时间
    local s_time = 1612828800
    local e_time = 1613422800 -- 2月16号5点

    if (e_time and os.time() > e_time) or (s_time and os.time() < s_time) then
        return false
    end
    --对应权限的key
    local _permission_key = "actp_own_task_1000276"
...
function M.CheckIsShow(parm)
   if parm then
        if parm.goto_scene_parm == "panel_act" then
            return M.IsActive()
        else
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShowByTime() then
            ...
            if et - os.time() < 2.5 * 3600 then
                LittleTips.Create("请于明晚来21:30拆福利券")
            elseif et - os.time() >= 2.5 * 3600 and et - os.time() < 22 * 3600 then
                LittleTips.Create("请于今晚来21:30拆福利券")
    ...
    elseif parm.goto_scene_parm == "panel_act" then 
        return Act_015_YYBJSJActPanel.Create(parm.parent)
    else
...
function M.GetHintState(parm)
	...
	if PlayerPrefs.GetInt(M.key .. MainModel.UserInfo.user_id ..os.date("%x",os.time()),0) ~= 1 and M.CheckIsShowByTime() then
        return ACTIVITY_HINT_STATUS_ENUM.AT_Get
    else
...
function M.InitUIConfig()
    ...
    if a and b then
        for i,v in ipairs(M.config.yybjsj_cjj) do
            M.UIConfig[v.vip] = v
        end
    else
        for i,v in ipairs(M.config.yybjsj) do
            M.UIConfig[v.vip] = v
        end
    end
end
