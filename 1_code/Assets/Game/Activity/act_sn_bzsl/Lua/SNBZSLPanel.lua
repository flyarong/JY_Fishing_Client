...
SNBZSLPanel = basefunc.class()
local C = SNBZSLPanel
C.name = "SNBZSLPanel"
local M = SNBZSLManager
...
function C:InitUI()
	self.config = SNBZSLManager.config
    ...
    self.CellList = {}
    self.cell_size = {w=160, h=210}
    self.b_p = {x=-318, y=94}
    self.w_c = 5
    for i = 1, #self.config.Award do
    	local pre = SNBZSLPrefab.Create(self.center, i, self.OnXZClick, self)
...
function C:MyRefresh()
	local temp_ui = {}
	if SNBZSLManager.GetData() then 
		self.m_data = SNBZSLManager.GetData().at_data
	else
		return 
	end 
	dump(self.m_data,"<color=red>爆竹送礼数据</color>")
	if self.m_data then
		...
		if self.m_data.now_game_num >= 1 then
			self.top.gameObject:SetActive(false)
		else
			self.top.gameObject:SetActive(true)
		end
...
function C:RefreshJF(data)
	if data and SNBZSLManager.key ~= data.key then
		return
	end
	if SNBZSLManager.GetData() then 
		self.m_data = SNBZSLManager.GetData().at_data
	...
	if self.m_data then
		...
		if self.now_game_num >= #self.config.Award then 
			...
			self.xh_hint_txt.text = "<color=#920603FF>所有奖励已获取</color>"
		else
			...
			self.xh_hint_txt.text = "<color=#920603FF>消耗<color=#EF4728FF>" .. self.config.Award[self.now_game_num + 1].need_credits.."</color>积分点燃爆竹</color>"
		end
	else
		self.xh_hint_txt.text = ""
	end
end
