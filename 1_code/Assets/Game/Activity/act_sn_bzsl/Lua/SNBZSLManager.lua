...
M.lottery_type = "firecracker_gifts"
M.config = GameButtonManager.ExtLoadLua(M.key, "activity_snbzsl_config")
GameButtonManager.ExtLoadLua(M.key, "SNBZSLPanel")
GameButtonManager.ExtLoadLua(M.key, "SNBZSLPrefab")
...
function M.IsActive()
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_common_lottery_firecracker_gifts", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
...
function M.CheckIsShow(cfg)
    if cfg and (not cfg.is_on_off or cfg.is_on_off == 0) then
        return
    end
    if M.IsActive() then
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then 
            return SNBZSLPanel.Create(parm)
        end 
    else
