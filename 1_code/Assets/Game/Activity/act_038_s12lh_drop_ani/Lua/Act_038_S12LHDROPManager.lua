-- 创建时间:2020-06-09
-- Act_038_S12LHDROPManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_038_S12LHDROPManager = {}
local M = Act_038_S12LHDROPManager
M.key = "act_038_s12lh_drop_ani"
...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and data.change_type == "task_award_1000129" then
...
function M.PrefabCreator(value)
    ...
    local obj = newObject("Act_038_S12LHDROPPrefab",GameObject.Find("Canvas/LayerLv5").transform)
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_award_1000129" == type  then
            return true
        end
    end
