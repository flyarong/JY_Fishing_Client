...
YX_LJYLPanel = basefunc.class()
local C = YX_LJYLPanel
C.name = "YX_LJYLPanel"
--配置相关
local config = YX_LJYJManager.config
--排行榜相关
--改动数据相关
local LOTTERY_TYPE = { "lantern_festival_lottery" }
...
function C:OnButtonClick()
	...
	function()
		if not self.CanStart_By_Info_Num then 
			HintPanel.Create(1,"您抽奖次数已用尽，请参与其他活动。")
			return 
		end
...
function C:ReFreshInfo(_, data)
	...
	self.CanStart_By_Info_Num = data.now_game_num < #config.Award - 4
	...
	self.expend_txt.text = self.need_credits..LotteryStr.."(剩余次数:"..6-self.now_game_num..")"
