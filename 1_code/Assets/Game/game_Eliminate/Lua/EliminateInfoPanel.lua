...
function M:MakeLister()
	...
	self.lister["view_eliminate_lottery_start"]=basefunc.handler(self,self.FreshList)
	...
	for i = 1, #redata do	
		if  redata[i]~=0 then
			...
			if  _data.type then
				...
				local des=Des.Create(_data.id,"同类暴击",index)   
				self.profablist[#self.profablist+1]=des	
				elseif _data.type==EliminateModel.eliminate_enum.clear_all then				
				local des=Des.Create(_data.id,"全屏暴击",index) 
				self.profablist[#self.profablist+1]=des	  
			    end						
			end
			end
...
function M:ctor()
	...
	self.parent = GameObject.Find("Canvas/GUIRoot").transform
	...
	self.goldtext=GameObject.Find("Canvas/GUIRoot/EliminateInfoPanel/bgs/GoldInfo/GoldText"):GetComponent("Text")
	...
	if not M:CheckIsCjj() then
		self.gameObject.transform:Find("Help").transform.localScale = Vector3.New(1.2,1.2,1.2)
		self.gameObject.transform:Find("Help").transform.localPosition = Vector3.New(1666,393,0)
	end
end

function M:InitUI()
	self.money_pre = EliminateMoneyPanel.Create()
end
...
function M:GetMoney_pre()
	return self.money_pre
end
...
--用于水果消消乐爬塔一步一刷新
function M:SGXXL_Tower_Refresh(data)
	local index
	if #data.cur_del_list % 2 == 0 then
		index = #data.cur_del_list / 2
	else
		index = (#data.cur_del_list + 1) / 2
	end
	local pos = eliminate_algorithm.get_pos_by_index(data.cur_del_list[index].x,data.cur_del_list[index].y)
	local bet = EliminateModel.GetBet()[1] * #(EliminateModel.GetBet())
	Event.Brocast("sgxxl_tower_refresh_change_msg",{num = #data.cur_del_list, pos = pos, id = data.cur_del_list[index].id, bet=bet})
end