...
function M:MyInit()
    ...
    EventTriggerListener.Get(self.lottery_guide_btn.gameObject).onDown = basefunc.handler(self, self.OnClickLotteryDown)
    EventTriggerListener.Get(self.lottery_guide_btn.gameObject).onUp = basefunc.handler(self, self.OnClickLotteryUp)
    ...
    self.info_pre = EliminateInfoPanel.Create()
    self.money_pre = self.info_pre:GetMoney_pre()
    ...
    local btn_map = {}
	btn_map["left_down"] = {self.left_down1,self.left_down2}
    ...
    if a and b then
        if PlayerPrefs.GetInt("guide"..MainModel.UserInfo.user_id.."sgxxl",0) == 0 then
            self.lottery_guide_btn.gameObject:SetActive(true)
            self.guide_pre = EliminateGuidePanel.Create(self)
...
function M:MyExit()
    ...
    if self.guide_pre then
        self.guide_pre:MyExit()
    end
...
function M:MakeLister()
    ...
    lister["eliminate_guide_refresh_msg"] = basefunc.handler(self, self.on_eliminate_guide_refresh_msg)
end
...
function M:RefreshYXCard()
    local open, yxcard, game_level, qp_image = GameButtonManager.RunFun({gotoui = "act_060_yxcard",card_type = EliminateModel.yxcard_type}, "GetCurGameCard")
    dump({
        yxcard = yxcard,
...
function M:Lottery(_card_type)
    ...
    if all_bet > MainModel.GetMiniGameCoinAndJingBiAllNum() then
    ...
    if _card_type then
        Network.SendRequest("xxl_kaijiang",{bets = EliminateModel.data.bet, card_type = _card_type},"连接中")
    else
        Network.SendRequest("xxl_kaijiang",{bets = EliminateModel.data.bet},"连接中")
    end
    Event.Brocast("view_eliminate_lottery_start")
...
function M:OnClickYXCard()
    local open, yxcard, game_level  = GameButtonManager.RunFun({gotoui = "act_060_yxcard",card_type = EliminateModel.yxcard_type}, "GetCurGameCard")
...
function M:on_eliminate_guide_refresh_msg()
    if PlayerPrefs.GetInt("guide"..MainModel.UserInfo.user_id.."sgxxl",0) ~= 0 then
        self.lottery_guide_btn.gameObject:SetActive(false)
    end 
end