...
function M:ctor()
    self.parent = GameObject.Find("Canvas/GUIRoot/EliminateInfoPanel/bgs/Viewport/@TaskNode").transform
    -- print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>DESDESDES")
    ...
    if self._isReCreat==1 then
        self.gameObject.transform.parent = GameObject.Find("Canvas/GUIRoot/EliminateInfoPanel/bgs/Viewport/@TaskNode").transform       
...
function M:PlayAnimIn()
    ...
    seq:Append(self.Gobj:DOLocalMoveX(0, EliminateModel.GetTime(EliminateModel.cfg.time.eliminate_despfb_into_time)):SetEase(DG.Tweening.Ease.Linear))--OutBack
    seq:OnForceKill(     
        function()--这里报过异常
