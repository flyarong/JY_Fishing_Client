...
function PayFastFreePanel:ctor(config, signup)
    ...
    self.CenterRectTransform = self.CenterRect:GetComponent("RectTransform")
   	...
   	if GameGlobalOnOff.PGPay and gameRuntimePlatform == "Ios" then
   	    self.hint_node2_txt.text = "<color=#B98109FF>将自动为您购买<color=#FB9474FF>" .. self.jingbi_config.ui_price .. "</color>的钻石并兑换成金币</color>"
    	self.is_pay_gift = false
        ...
        if self.status == 1 then
            self.hint_node2_txt.text = "<color=#B98109FF>将自动为您购买<color=#FB9474FF>" .. "￥" .. (self.gift_config.price / 100) .. "元</color>的钻石并兑换成金币</color>"
	        ...
	        self.hint_node2_txt.text = "<color=#B98109FF>将自动为您购买<color=#FB9474FF>" .. self.jingbi_config.ui_price .. "元</color>的钻石并兑换成金币</color>"
    		self.is_pay_gift = false
		    self.num_info_txt.transform.localPosition = Vector3.New(105.2, 204, 0)
		    self.hint_node3.gameObject:SetActive(false)
		    self.hint_node1.gameObject:SetActive(false)
    ...
    self.close_btn.onClick:AddListener(function()
        ...
        if self.config.imageIndex < 2 then
            self:MyExit()
...
function PayFastFreePanel:InitRect()
	if self.is_pay_gift then
	    ...
	    self.jing_bi_txt.text = StringHelper.ToCash(self.gift_config.buy_asset_count[1]) .. "金币"
	else
	    self.num_info_txt.text = StringHelper.ToCash(tonumber(self.config.enterMin) - MainModel.UserInfo.jing_bi)
