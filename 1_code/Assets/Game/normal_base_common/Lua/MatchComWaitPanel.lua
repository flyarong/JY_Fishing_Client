...
MatchComWaitPanel = basefunc.class()
local M = MatchComWaitPanel

M.name = "MatchComWaitPanel"
local instance
M.Model_Status = {
...
function M.Create(param)
	dump(param, "<color=white>比赛场等待参数？？？？？？？？？？？？？</color>")
...
function M:ctor(param)
	...
	MatchComWaitProgress.Create(data)
end
...
function M:MyExit()
	self:RemoveListener()
	MatchComRankRewardPanel.Close()
	MatchComWaitProgress.Close()
	self:StopClock()
...
function M:SetRank()
	...
	self.rank_base_txt.text = self.param.match_player_num
end

function M:OnClickRankReawrd()
	MatchComRankRewardPanel.Create(self.param.game_cfg, self.param.award_cfg,self.rank_reward_back_btn)
	self.rank_reward_btn.gameObject:SetActive(false)
...
function M.ShowPromotionInfo(game_id,round_info)
	...
	local baseScore = GameMatchModel.IsGMS(game_id) and round_info.init_stake or nil
				
	local pro_info_item = GameObject.Instantiate(GetPrefab("MatchComWaitProInfoItem"), GameObject.Find("Canvas/LayerLv4").transform)
	local ui = {}
    ...
    seq:Append(tween1):AppendInterval(1):Append(tween2):OnForceKill(
        function()
            if pro_info_item ~= nil then
                Destroy(pro_info_item.gameObject)
                pro_info_item = nil
