...
MatchComWaitProgress = basefunc.class()
local M = MatchComWaitProgress
M.name = "MatchComWaitProgress"
local instance
--[[param = {
    anchor, --挂载节点
    is_pro, --是否晋级
...
function M:Init()
    ...
    if self.param.game_cfg.game_type == GameMatchModel.GameType.game_DdzMatchNaming or
    self.param.game_cfg.game_type == GameMatchModel.GameType.game_MjMatchNaming then
        --有预赛
    ...
    elseif self.param.game_cfg.game_type == GameMatchModel.GameType.game_DdzMatch or
            self.param.game_cfg.game_type == GameMatchModel.GameType.game_DdzPDKMatch or
            self.param.game_cfg.game_type == GameMatchModel.GameType.game_MjXzMatch3D then
        --没有预赛
        self.my_round = self.param.round_info.round - 1
    end
    ...
    for i,v in ipairs(self.nodes.data) do
        local node = GameObject.Instantiate(self.node,self.content)
        table.insert(self.nodes.obj,node)
        ...
        if self.lines.data[i + 1] then
            ...
            table.insert(self.lines.obj,line)
    ...
    if self.param.award_cfg then
        for _, v in ipairs(self.param.award_cfg) do
            if v.rank == "第1名" then
                self.reward_obj.transform:Find("award_name_txt").gameObject:GetComponent("Text").text = v.award
                ...
                iconImg.sprite = GetTexture(v.icon)
    ...
    if self.param.is_pro then
        ...
        seq1:AppendCallback(function ()
            ...
            --     node_light_img.fillAmount = node_light_img.fillAmount + fillAmountDire
            --     end,direction,countNum)
            -- self.timer2:Start()
            -- return self.timer2
        end)
