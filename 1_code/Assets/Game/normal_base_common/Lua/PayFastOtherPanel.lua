...
function PayFastOtherPanel:ctor(config, signup)
    ...
    if is_goto_shop then
        ...
        if IsEquals(self.goto_num_info_txt) then
            self.goto_num_info_txt.text = string.format( "您进入当前场次还需要<color=#FF8109FF><size=60>%s</size></color>金币", StringHelper.ToCash(tonumber(self.jinbi) - MainModel.UserInfo.jing_bi))
        end
...
function PayFastOtherPanel:OnShowPay()
    if GameGlobalOnOff.PayZS then
        ...
        HintPanel.Create(1,"您的钻石不足以兑换本次比赛所需金币")
    end
end
