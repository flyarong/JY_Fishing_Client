...
function DdzPDKClearing:ctor(isdelay)
    ...
    self.goto_match_btn.onClick:AddListener(
        function()
            ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
            local state = gameMgr:CheckUpdate("game_Match")
...
function DdzPDKClearing:UpdateExchange()
    ...
    if is_game_over and DdzPDKModel.data.exchange_hongbao and DdzPDKModel.data.exchange_hongbao.is_exchanged == 0 then
        ...
        self.ExChongbao.text = StringHelper.ToRedNum(DdzPDKModel.data.exchange_hongbao.hong_bao )
        Event.Brocast("global_sysqx_uichange_msg", {key="dh", panelSelf=self})
    else
...
function DdzPDKClearing:OnExchangeClick()
    ...
    if not iss then
        local rr = StringHelper.ToRedNum(DdzPDKModel.data.exchange_hongbao.hong_bao )
        local jb = DdzPDKModel.data.settlement_info.score_data[DdzPDKModel.GetPlayerSeat()].score
        local str = string.format("为你将本局赢得的%s金币兑换成%s福利券", jb, rr)
...
function DdzPDKClearing:InitRect()
    ...
    ExtendSoundManager.PlaySound(self.isWin and audio_config.ddz.sod_game_win.audio_name or audio_config.ddz.sod_game_lose.audio_name)
    ...
    if OperatorActivityModel.IsActivated(DdzPDKModel.baseData.game_id, ActivityType.Consecutive_Win) then
...
function DdzPDKClearing:SetBackAndConfirmBtn()
    ...
    if is_game_over then
        --红包
        GameManager.GotoUI({gotoui = "sys_total_red",goto_scene_parm = "panel",transform = self.transform,game_id = DdzPDKModel.baseData.game_id})
    end
...
function DdzPDKClearing:OnRefreshActivityData()
    if self.isWin and DdzPDKModel.data.ls_count > 1 and OperatorActivityModel.IsActivated(DdzPDKModel.baseData.game_id, ActivityType.Consecutive_Win) and ActivityConsecutiveWin.CanBeAwarded() then
        self.shengli.gameObject:SetActive(false)
