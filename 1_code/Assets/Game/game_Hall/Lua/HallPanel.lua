...
function HallPanel:MakeLister()
	...
	lister["prop_xycj_coin_has_change_msg"] = basefunc.handler(self,self.on_prop_xycj_coin_has_change_msg)
end
...
function HallPanel:SetupBtns()
	...
	self.SHXXLBox.PointerClick:AddListener(function (obj)
		self:OnSHXXLClick(obj)
	end)
	self.XYXXLBox.PointerClick:AddListener(function (obj)
		self:OnXYXXLClick(obj)
	end)
...
function HallPanel:ctor(call)
	...
	self.SHXXLBox = self.SHXXLBox:GetComponent("PolygonClick")
	self.XYXXLBox = self.XYXXLBox:GetComponent("PolygonClick")
	...
	btn_map["right_top"] = {self.hall_btn_share,self.hall_btn_4, self.hall_btn_3, self.hall_btn_2, self.hall_btn_1,self.hall_btn_107,self.hall_btn_106,self.hall_btn_105,}
	btn_map["top"] = {self.hall_btn_top}
	btn_map["fish3d_node"] = {self.Fishing3DNode}
	btn_map["left_bottom"] = {self.hall_btn_tgzq}
	btn_map["right_bottom"] = {self.hall_btn_right_bottom}
	...
	self.GZHButton_btn.gameObject:SetActive(false)
end
...
function HallPanel:SHXXL_zhezhao()
	local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key = "shxxl_level", is_on_hint = true}, "CheckCondition")
    if a and b then
        self.lock_SHXXL.gameObject:SetActive(false)
    else
    	self.lock_SHXXL.gameObject:SetActive(true)
    end
end
function HallPanel:XYXXL_zhezhao()
	local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key = "xyxxl_level", is_on_hint = true}, "CheckCondition")
    if a and b then
        self.lock_XYXXL.gameObject:SetActive(false)
    else
    	self.lock_XYXXL.gameObject:SetActive(true)
    end
end
...
function HallPanel:UpdateDHHint()
	if MainModel.GetHBValue() >= 10 then
		...
		if oldtime ~= newtime then
			if IsEquals(self.duihuan_hint_txt.gameObject) then
...
function HallPanel:OnMiniGameClick(go)
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	if IsEquals(self.MINIGAME) then
...
function HallPanel:OnSHXXLClick()
...
function HallPanel:OnXYXXLClick()
	local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="xyxxl_level", is_on_hint = false}, "CheckCondition")
    if a and not b then
        return
    end
	if GameGlobalOnOff.InternalTest then
...
function HallPanel:OnBagClick(go)
	...
	self.ServiceNode.gameObject:SetActive(false)
	PlayerPrefs.SetInt("NewGunGet_hallPanel"..MainModel.UserInfo.user_id,0)
	self:RefreshGunRed()
	self:RefreshBagGet()
end
...
--玩家每次获得新炮台后，大厅背包按钮上都出现红点提示
function HallPanel:on_AssetGet(data)
	if data and data.data and data.data[1] and data.data[1].asset_type then
		if string.sub(data.data[1].asset_type,1,11) == "gun_barrel_" then
			self:RefreshGunRed()
			--玩家获得感恩节宝箱
		elseif string.sub(data.data[1].asset_type,1,9) == "prop_gej_" then
			self:RefreshBagGet()
		elseif data.data[1].asset_type == "prop_xycj_coin" then
			self:RefreshBagCDJ()
		end
	end
end

--玩家每次获得新炮台后，炮台切页上都出现红点提示
function HallPanel:RefreshGunRed()
    dump(PlayerPrefs.GetInt("NewGunGet_hallPanel"..MainModel.UserInfo.user_id,0),"++++++++++++++++++++++++++++")
    if PlayerPrefs.GetInt("NewGunGet_hallPanel"..MainModel.UserInfo.user_id,0) ~= 0 then
        self.bag_red.gameObject:SetActive(true)
    end
end
...
function HallPanel:RefreshBagGet()
    ...
    if Act_Ty_LB1Manager.IsHaveItemCount() then
        self.bag_get.gameObject:SetActive(true)
        self.can_get.gameObject:SetActive(true)
	else
		self.bag_get.gameObject:SetActive(false)
...
function HallPanel:RefreshBagCDJ()
	local num = GameItemModel.GetItemCount("prop_xycj_coin")
	self.bag_cdj.gameObject:SetActive(num > 0)
end

function HallPanel:on_prop_xycj_coin_has_change_msg()
	self:RefreshBagCDJ()
end