...
function FishingDRBetPanel:InitRect()
	...
	-- local lastPower = PlayerPrefs.GetInt(LOCAL_POWER_KEY, -1)
	-- if lastPower == -1 then
	-- 	lastPower = 1
	-- 	local cur = MainModel.UserInfo.jing_bi / 30
	-- 	for i=#BET_POWER_TBL,1,-1 do
	-- 		if cur >= BET_POWER_TBL[i].value then
	-- 			lastPower = i
	-- 			break
	-- 		end
	-- 	end
	...
	local lastPower = PlayerPrefs.GetInt(LOCAL_POWER_KEY, 2)
	...
	local lastMode = PlayerPrefs.GetInt(LOCAL_MODE_KEY, 0)
	lastMode = 0
	...
	for k, v in ipairs(BET_RATE_TBL) do
		...
		EventTriggerListener.Get(bg.gameObject).onClick = function()
			...
			--[[self.rateTable[rate_idx] = self.rateTable[rate_idx] + self.rateFactor
			self:RefreshMiddle()]]--
	...
	self.cdTimer = Timer.New(function()
		...
		if self.cdCount < 6 then
			ExtendSoundManager.PlaySound(audio_config.by_dr.sod_game_timeout.audio_name)
		end
...
function FishingDRBetPanel:SwitchMode(newMode)
	...
	PlayerPrefs.SetInt(LOCAL_MODE_KEY, newMode)
...
function FishingDRBetPanel:CheckMoney(rateTable, rate_idx, rateFactor)
	...
	if jing_bi < total then
		HintPanel.Create(1, "金币不足，请先充值", function()
...
function FishingDRBetPanel:LoadLastRates()
	...
	if not PlayerPrefs.HasKey(LOCAL_BET_KEY) then return end

	local str = PlayerPrefs.GetString(LOCAL_BET_KEY, "")
	local bets = basefunc.string.split(str, "#")
	if #bets ~= #BET_RATE_TBL then
		PlayerPrefs.DeleteKey(LOCAL_BET_KEY)
		return
	end
...
function FishingDRBetPanel:SaveLastRates()
	...
	PlayerPrefs.SetString(LOCAL_BET_KEY, str)
end
...
function FishingDRBetPanel:handle_bet_pop_menu_click(data)
	...
	if key == "power" then
		...
		PlayerPrefs.SetInt(LOCAL_POWER_KEY, idx)
