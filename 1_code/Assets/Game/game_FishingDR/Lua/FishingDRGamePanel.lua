...
function C:ctor()
    ...
    self.hall_btn_top = tran:Find("@hall_btn_top")
    self.ybh_list = {}
    self.gun_list = {}
    for i = 1, 7 do
        self.ybh_list[#self.ybh_list + 1] = tran:Find("UINode/Player" .. i.."/ybh")
    ...
    self.GoldText = tran:Find("UINode/RectTop/JBBG/GoldText"):GetComponent("Text")

    self.BackButton = tran:Find("UINode/RectTop/BackButton"):GetComponent("Button")
    self.BackButton.onClick:AddListener(function()
        ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
        self:OnBack()
    end)

    self.JBBG = tran:Find("UINode/RectTop/JBBG"):GetComponent("Button")
    ...
    self.TallyNode = tran:Find("UINode/RectTop/TallyNode")
    self.record_list_node = tran:Find("UINode/RectTop/TallyNode/record_list_node")
    self.record_tmpl = tran:Find("UINode/RectTop/TallyNode/record_tmpl")
    self.record_list = {}
...
function C:RefreshGameover(is_ani)
    ...
    --self:SceneGunExit(is_ani)
    if is_ani then
        ExtendSoundManager.PlaySound(audio_config.by_dr.bgm_xxl_jiangli2.audio_name)
    end
    FishingDRPlayerManager.RefreshAll()
    C.ManagerRemoveAll()
    -- if self.update_time then
    ...
    if table_is_null(data.fish.trigger) then
        ...
        FishingDRAnimManager.PlayDRFishDeadFX(self.FXNode, pos, fishobj, function ()
            ...
            if data.fish.is_fj ~= nil then
                ExtendSoundManager.PlaySound(audio_config.by_dr.bgm_by_jiangli6.audio_name)
        ...
        elseif data.fish.trigger_type == 2 then
            ...
            ExtendSoundManager.PlaySound(audio_config.by_dr.bgm_dapiaojinbi.audio_name)
            ExtendSoundManager.PlaySound(audio_config.by_dr.bgm_by_shandianyu.audio_name)
            --闪电
...
function C:FishWaveEvent()
    if FishingDRModel.data.fish_map_id % 2 == 0 then
        FishingDRAnimManager.PlayWaveHint(self.FXNode, false, "by_imgf_ycll")
        FishingDRAnimManager.PlayWave(self.FXNode, false)
    else
        FishingDRAnimManager.PlayWaveHint(self.FXNode, false, "by_imgf_ycjs")
        FishingDRAnimManager.PlayWave(self.FXNode, false)
    end

    ExtendSoundManager.PlaySound(audio_config.by.bgm_by_yuchao.audio_name)

    VehicleManager.PlayFlee(FishingDRModel.data.clear_level)
    FishManager.PlayFlee(FishingDRModel.data.clear_level)
