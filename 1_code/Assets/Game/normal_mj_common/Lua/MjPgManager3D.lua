...
function C:create3DPengPai(p , paiType , pai , isNotDoEffect)
	...
	if not isNotDoEffect then
		...
		ParticleManager.MJPGH(p , paiType)
	end
end
...
function C:create3DZhiGang(p , paiType , pai , isNotDoEffect)
	...
	--- 是否不做效果
	if not isNotDoEffect then
		ParticleManager.MJXY(self.model.GetSeatnoToPos(self.model.data.cur_chupai.p))
	...
	if not isNotDoEffect then
        ...
        if cur_p and mySeatno then
            if cur_p == mySeatno then
                ParticleManager.MJLJF()
            else
                ParticleManager.MJLJFOther(self.model.GetSeatnoToPos(cur_p))
            end
        end
...
function C:ChangeWG(p, pai)
    ...
    --  MjAnimation.AniItemMjPGH(p, self.model.PaiType.wg)
     ParticleManager.MJPGH(p,self.model.PaiType.wg)
end
