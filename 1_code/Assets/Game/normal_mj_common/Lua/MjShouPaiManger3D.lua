...
function C:Hupai(data)
	...
	ParticleManager.MJHuPaiAni(self.playerMgr.uiPos)
...
function C:AddHuPai(data)
	...
	if not self:checkIsXlModel() then
		self.hpImg = ParticleManager.MJHuPai(self.playerMgr.uiPos, huType, huIdx, huPai, self.transform)
	end
...
function C:ShowPoChan()
	...
	self.hpImg = ParticleManager.MJHuPai(self.playerMgr.uiPos, "pochan", 1, 11, self.transform)
end
