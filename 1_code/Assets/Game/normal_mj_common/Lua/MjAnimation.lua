			...
			imgAni:Play(function(pad)
				...
				if idx == spCnt then
					...
					Timer.New(function ()
						...
						ParticleManager.MJFanPai(spList[spCnt].transform.parent.transform.position)
					end, 0.15, 1):Start()
				end
...
function MjAnimation.DQIcon(dqIcon)
	...
	local tween2 =  dqIcon.transform:DOScale(Vector3.one * 1.2,0.5):From():OnComplete(function()
		ParticleManager.MJDQIcon(dqIcon.transform.position)
	end)
...
function MjAnimation.TJColor(tjImg)	
	local TJFX = ParticleManager.MJDQSwitchCreate(tjImg)	
