...
EliminateSHHeroBG = basefunc.class()
local M = EliminateSHHeroBG

math.randomseed(tostring(os.time()):reverse():sub(1, 6))
M.random = math.random(1,2)

M.name = "EliminateSHHeroBG"
...
function M:ctor(data)
    local parent = EliminateSHHeroManager.GetHeroBGContent()
    if not parent then return end
    self.data = data
    self.ui = {}
	self.ui.gameObject = GameObject.Instantiate(EliminateSHObjManager.item_obj.EliminateSHHeroBG,parent.transform)
