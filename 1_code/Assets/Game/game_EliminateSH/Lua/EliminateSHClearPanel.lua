-- 创建时间:2019-05-17
-- Panel:EliminateSHClearPanel
local basefunc = require "Game/Common/basefunc"
EliminateSHClearPanel = basefunc.class()
local C = EliminateSHClearPanel
C.name = "EliminateSHClearPanel"
...
function C:InitUI()
    ...
    self.childobjsgoldtext[2] = self.childobjs[2].gameObject.transform:Find("GoldText/Text"):GetComponent("Text")
    self.childobjsgoldtext[3] = self.childobjs[3].gameObject.transform:Find("GoldText/Text"):GetComponent("Text")
    self.childobjsgoldtext[4] = self.childobjs[4].gameObject.transform:Find("fenzhi/GoldText/wenzi/Text"):GetComponent("Text")
...
function C:view_lottery_end(data)
    local dangci = EliminateSHModel.GetAllResultLevel()
    local award = data.award_money
...
function C:ShowClear(data)
    if data[1] == 1 then
        ...
        self.soundlv1 =
            ...
            audio_config.shxxl.bgm_shxxl_jiangli2.audio_name,
            1,
            function()
                self.soundlv1 = nil
            end
    ...
    if data[1] == 2 then
        ExtendSoundManager.PlaySceneBGM(audio_config.shxxl.bgm_shxxl_beijing.audio_name)
        self.hidetime = 2
        ExtendSoundManager.PlaySound(audio_config.shxxl.bgm_shxxl_jiangli2.audio_name)
    end
    if data[1] == 3 then
        self.hidetime = 8
        self.soundlv3 = nil
        self.soundlv3 =
            ExtendSoundManager.PlaySound(
            audio_config.shxxl.bgm_shxxl_jiangli3.audio_name,
            1,
            function()
                self.soundlv3 = nil
            end
    ...
    if data[1] == 4 then
        ...
        self.soundlv4 =
            ExtendSoundManager.PlaySound(
            audio_config.shxxl.bgm_shxxl_jiangli4.audio_name,
            1,
            function()
                self.soundlv4 = nil
            end
    ...
    dump(auto, "<color=white>自动模式</color>")
    if auto or data[1] == 1 or data[1] == 2 then
        self.hide =
            Timer.New(
            function()
                for i = 1, #self.childobjs do
