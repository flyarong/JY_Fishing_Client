    ...
    if reConnectServerState == 2 then
        ...
        if data.result == 0 then
			...
			if gameRuntimePlatform == "Android" then
				...
				if first_login > 0 and gameMgr:getMarketChannel() == "pdd" then
					local lua_tbl = {}
					lua_tbl.msg = 0			--register_login
					lua_tbl.registerWay = 4	--other
...
function MainModel.IsShowAward(a_type)
    ...
    if UniWebViewMgr and UniWebViewMgr.IsInViewing() then
        return false
    end
    
    if "all_return_lb_overtime_award" == a_type
        or "gold_pig2_task_award" == a_type then
        return false
    end
    if string.sub(a_type,1,25) == "bullet_rank_award_settle_" then
        return false
    end

    if a_type == "box_exchange_active_award_60" then--屏蔽11.10双11
        return false
    end

    ------------------10.27 万圣幸运礼包----------------------
    for i=10331,10336 do
    ...
    if a_type == "task_award_1000116" then--火鸡掉落赢金
        return false
    end

    if string.sub(a_type, 1, 26) == "box_exchange_active_award_" then--兑换
        return true
    end
   ---------------------------------------
   ----------------一本万利-----------------
--[[    for i=10384,10391 do
        local str = "buy_gift_bag_"..i
        if a_type == str then
            return false
        end
    end--]]
    ...
    if string.sub(a_type, 1, 14) == "xycd_ljyj_task" then--幸运彩蛋赢金
        return false
    end
    if string.sub(a_type, 1, 23) == "task_p_026_lmqx_lmlh_yj" then
        return false
    end
...
function MainModel.OnNotifyAssetChangeMsg(proto_name,data)
    ...
    if data.change_asset then
        ...
        for k,v in pairs(data.change_asset) do
            ...
            -- 扩展
            if _G["GameItemModel"] then
                local tag = GameItemModel.GetItemTagToKey(v.asset_type)
                change_bag_tag = basefunc.bit_or(change_bag_tag, tag)
            end
        end
    end
    MainModel.ADD_JB_Log({no=data.no, jb=chg_jing_bi})

    -- 资产改变的背包Tag  =  change_bag_tag
...
function MainModel.IsHadBuyGiftByID(id)
    ...
    if data then
        ...
        if data.status == 0 and data.remain_time and data.remain_time > 0 then
            return false
        end
        return true
    end
...
function MainModel.FinishGiftShopByID(id)
    ...
    MainModel.UserInfo.GiftShopStatus[id].time = MainModel.GetCurTime()
...
-- 是否是提审状态
MainModel.is_trial = true
-- vivolow玩家
function MainModel.IsLowPlayer()
    local platform = gameMgr:getMarketPlatform()
    if MainModel.is_trial and platform == "xiaomi" then
        return true
    end

    local vip_l = MainModel.UserInfo.vip_level
     if platform == "vivo" or platform == "xiaomi" then
        -- key不准修改，修改的后果就是影响老玩家的状态
        local key1 = "Vivo_lx_state_" .. MainModel.UserInfo.user_id
        local key2 = "Vivo_lx_last_time_" .. MainModel.UserInfo.user_id
        local state = PlayerPrefs.GetInt(key1, 0)
        local old_t = PlayerPrefs.GetString(key2, "0")
...
local package_table = {
    normal = "com.changleyou.tthlby",
    cjj = "com.wuziqi.wzq",
    vivo = "com.changleyou.hlttby.vivo",
    xiaomi = "com.changleyou.tthlby.mi",
}
function MainModel.CleanWebViewAllCookies()
    if gameRuntimePlatform ~= "Android" then return end
    if PlayerPrefs.GetInt("Clean_WebView_Cookies_1", 0) == 0 then
        local platform = gameMgr:getMarketPlatform()
        ...
        PlayerPrefs.SetInt("Clean_WebView_Cookies_1", 1)
    end
end