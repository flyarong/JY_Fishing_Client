...
base_goto_ui = function (parm)
	...
	--[[if parm.requset then
		MainModel.Location = GameManager.MarkToSceneName(parm.requset)
	end--]]
    ...
    if gotoui then
		if gotoui == "game_Match" then
			local match_parm = GameManager.ConvertGameMatchParm(goto_scene_parm)
            GameManager.GotoSceneName("game_Match",match_parm,call,enter_scene_call,down_style)
		...
		elseif gotoui == "share_hall" then
        	runCall(call)
			-- return SharePanel.Create("hall", "hall_share",nil,goto_scene_parm)
            return MainModel.SendShareFinish("shared_hy") -- 临时关闭 直接完成
		elseif gotoui == "share_link" then
			runCall(call)
			ShareLogic.ShareGM(goto_scene_parm, function(str)
				...
				if infor_table.requset then
					Network.SendRequest(infor_table.requset, infor_table.parm or parm.p_requset, "", function (data)		
						if data.result == 0 then
							...
							parm.gotoui = sceneName
							--parm.requset = infor_table.requset
							base_goto_ui(parm)
						else
...
local location_mark = {
	fsg_3d_signup = "game_Fishing3D",
	xxl_enter_game = "game_Eliminate",
	xxl_shuihu_enter_game = "game_EliminateSH",
	xxl_caishen_enter_game = "game_EliminateCS",
	xxl_xiyou_enter_game = "game_EliminateXY",
	lxxxl_enter_game = "game_EliminateCJ",
	dmbj_enter_game = "game_DMBJ",
	fishing_dr_enter_room = "game_FishingDR",
	lwzb_enter_room = "game_LWZB",
	tantanle_enter_game = "game_TTL",
	guess_apple_enter_room = "game_ZPG",
	fg_signup = "game_Gobang",
	fsg_signup = "game_Fishing",
	xxl_sanguo_enter_game = "game_EliminateSG",
}

function GameManager.MarkToSceneName(mark)
	if mark and location_mark then
		return location_mark[mark]
	end
end