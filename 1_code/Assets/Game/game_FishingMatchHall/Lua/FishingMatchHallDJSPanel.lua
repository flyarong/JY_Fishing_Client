-- 创建时间:2020-04-24
-- Panel:FishingMatchHallDJSPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
FishingMatchHallDJSPanel = basefunc.class()
local C = FishingMatchHallDJSPanel
C.name = "FishingMatchHallDJSPanel"

FishingMatchHallDJSPanel.SignupStart = 
{
	SS_Null = "SS_Null", -- 空
...
function C:MakeLister()
    ...
    self.lister["model_fsmg_query_game_id_msg_finish"] = basefunc.handler(self, self.MyRefresh)
end
...
function C:ctor(parent_transform)
	...
	self.signup_start = FishingMatchHallDJSPanel.SignupStart.SS_Null
    self.award_top3 = {}
...
function C:UpdateQueryAward(num)
	num = num or 0
	self.award_pool_txt.text = 1000 * num .. "福利券"
end
...
function C:MyRefresh()
	...
	self.game_cfg = FishingManager.GetCurDjsToConfig()
	self.award_cfg = FishingManager.GetGameIDToAward( self.game_cfg.award_id )
    ...
    if now < self.game_cfg.start_time then
    	self.signup_start = FishingMatchHallDJSPanel.SignupStart.SS_Wait
    elseif now >= self.game_cfg.start_time and now <= self.game_cfg.over_time then
    	self.signup_start = FishingMatchHallDJSPanel.SignupStart.SS_BeginSignup
    else
    	dump(self.game_cfg, "<color=red>self.game_cfg 配置异常</color>")
	end
	
	-- 捕鱼比赛的第一场比赛
	if FishingManager.IsFirstMatchDJS() then
	...
	if self.signup_start == FishingMatchHallDJSPanel.SignupStart.SS_Null then
	...
	elseif self.signup_start == FishingMatchHallDJSPanel.SignupStart.SS_Wait or
			self.signup_start == FishingMatchHallDJSPanel.SignupStart.SS_BeginSignup then
		if self.signup_start == FishingMatchHallDJSPanel.SignupStart.SS_Wait then
			self.rect1.gameObject:SetActive(false)
			self.rect2.gameObject:SetActive(true)
		else
		...
		-- 特殊处理 todo
		local itemkey, item_count = FishingManager.GetMatchCanUseTool(self.game_cfg)
...
function C:on_fsmg_signup(_, data)
	...
	if data.result == 0 then
		GameManager.GotoSceneName("game_FishingMatch", {game_id = self.game_id})
	else
...
function C:OnOldMatchClick()
	FishingMatchOldRankPanel.Create({game_type="djs"})
end
