-- 创建时间:2020-04-24
-- Panel:FishingMatchHallQYSPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
FishingMatchHallQYSPanel = basefunc.class()
local C = FishingMatchHallQYSPanel
C.name = "FishingMatchHallQYSPanel"

FishingMatchHallQYSPanel.SignupStart = 
{
	SS_Null = "SS_Null", -- 空
...
function C:MakeLister()
    ...
    self.lister["fsqmg_signup_response"] = basefunc.handler(self, self.on_fsmg_signup)
    self.lister["fsqmg_req_specified_signup_num_response"] = basefunc.handler(self, self.on_fsmg_req_player_num)
    self.lister["model_fsqmg_query_game_id_msg_finish"] = basefunc.handler(self, self.MyRefresh)
end
...
function C:MyExit()
	...
	FishingMatchQYSAwardPanel.Close()
...
function C:ctor(parent_transform)
	...
	self.signup_start = FishingMatchHallQYSPanel.SignupStart.SS_Null
    self.award_top3 = {}
...
function C:UpdateQuerySignup()
	if self.game_id then
		Network.SendRequest("fsqmg_req_specified_signup_num", {id=self.game_id})
	end
end
...
function C:InitUI()
	FishingManager.QueryQYSData()
	self:MyRefresh()
end

function C:MyRefresh()
	self.signup_num = 0
	self.game_id = FishingManager.GetQYSRecentlyGameID()
	dump(self.game_id, "<color=red>self.game_id</color>")
	self.game_cfg = FishingManager.GetCurQysToConfig()
	self.award_cfg = FishingManager.GetQYSGameIDToAward(self.game_cfg.award_id)
	...
	if self.game_id then
	    ...
	    if now < self.game_cfg.start_time then
	    	self.signup_start = FishingMatchHallQYSPanel.SignupStart.SS_Wait
	    elseif now >= self.game_cfg.start_time and now <= self.game_cfg.over_time then
	    	self.signup_start = FishingMatchHallQYSPanel.SignupStart.SS_BeginSignup
	    else
	...
	-- 捕鱼比赛的第一场比赛
	if FishingManager.IsFirstMatchQYS() then
	...
	if self.signup_start == FishingMatchHallQYSPanel.SignupStart.SS_Null then
	...
	elseif self.signup_start == FishingMatchHallQYSPanel.SignupStart.SS_Wait or
			self.signup_start == FishingMatchHallQYSPanel.SignupStart.SS_BeginSignup then
		self.time_rect.gameObject:SetActive(true)
		self.award_btn.gameObject:SetActive(true)

		-- 特殊处理 todo
		local itemkey, item_count = FishingManager.GetMatchQYSCanUseTool(self.game_cfg)
		if not itemkey or itemkey == "jing_bi" then
			...
			if is_have_jb then
				...
				self.ticket_txt.text = string.format("持有金币：%s", StringHelper.ToCash(num))
			...
			elseif itemkey == "shop_gold_sum" then
				...
				self.ticket_txt.text = string.format("持有红包：%s", StringHelper.ToRedNum(num))
				self.hint_hf_txt.text = "10元"
				self.hint_hf_img.sprite = GetTexture("com_award_icon_money")
			end
		end
		-- 屏蔽显示
		self.signup_num_txt.transform.localScale = Vector3.zero
		self.signup_num_txt.text = "--"
		self.signup_btn.gameObject:SetActive(true)
		if self.signup_start == FishingMatchHallQYSPanel.SignupStart.SS_Wait then
...
function C:RefreshTop3()
	if self.award_cfg then
		...
		if self.award_cfg[4] then
			self.award4_txt.text = self.award_cfg[4].award or "-"
		end
		if self.award_cfg[5] then
			self.award5_txt.text = self.award_cfg[5].award or "-"
		end
	else
		for i = 1, 3 do
			self.award_top3[i].gameObject:SetActive(false)
		end
		self.award4_txt.text = ""
		self.award5_txt.text = ""
	end
end
local WeekToTable = {
    [0] = "天",
...
function C:OnAwardRankClick()
	...
	pp.signup_num_response = "fsqmg_req_specified_signup_num_response"
	pp.num = self.signup_num
	FishingMatchQYSAwardPanel.Create(pp)
end
...
function C:OnSignupClick()
	if self.game_id then
		...
		local signup = function ()
			Network.SendRequest("fsqmg_signup", {id=self.game_id}, "请求报名")		
		end
		...
		if itemkey then
			...
			-- 第一届比赛不需要金币报名
			if is_have_jb then
				HintPanel.Create(2, "报名参赛需要10万金币，是否立刻前往增加金币？", function ()
...
function C:on_fsmg_signup(_, data)
	...
	if data.result == 0 then
		GameManager.GotoSceneName("game_FishingMatchQYS", {game_id = self.game_id})
	else
...
function C:OnOldMatchClick()
	FishingMatchOldRankPanel.Create({game_type="qys"})
end
