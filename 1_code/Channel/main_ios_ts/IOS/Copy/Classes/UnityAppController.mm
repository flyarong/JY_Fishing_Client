...
#define WeiXinID @"wxe5218102416a3a8b"
#define UniversalLink @"https://game-support.jyhd919.cn/"
...
#pragma clang diagnostic pop
    ...
    int StartRecording(const char *fileName) {
        NSString *param = [[NSString alloc] initWithUTF8String:fileName];
        //return AppController_StartRecording(param);
        gAmrRecord = param;
        return [AudioUtil startAudioRecord];
    }

    void StopRecording(BOOL callback) {
        //AppController_StopRecording(callback);
        [AudioUtil stopAudioRecord];

		if(callback) {
			if([AudioUtil encodeAMR:gAmrRecord])
				UnitySendMessage(GameObjectName, RecordingName, [gAmrRecord UTF8String]);
		}
    }
    
    int PlayingRecord(const char *fileName) {
        NSString *param = [[NSString alloc] initWithUTF8String:fileName];
        return [AudioUtil playRecord:param];
    }
    void StopPlayingRecord() {
        [AudioUtil stopPlayRecord];
    }
...
- (void)cleanAudioFile {
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDir = FALSE, isDirExist = FALSE;

    if(IsValidString(gCafFile)) {
	isDir = FALSE;
	isDirExist = [fileManager fileExistsAtPath:gCafFile isDirectory:&isDir];
	if(isDirExist) {
		[fileManager removeItemAtPath:gCafFile error:nil];
	}
    }

    if(IsValidString(gMp3File)) {
	isDir = FALSE;
	isDirExist = [fileManager fileExistsAtPath:gMp3File isDirectory:&isDir];
	if(isDirExist) {
		[fileManager removeItemAtPath:gMp3File error:nil];
	}
    }
}
- (int)StartRecording:(NSString*)fileName
{
    gMp3Record = fileName;
    ...
    if(![self.audioRecorder isRecording]) {
        ...
        NSLog(@"Start recording: %@", gMp3Record);
...
- (void)StopRecording:(BOOL)callback
{
    if([self.audioRecorder isRecording]) {
    	...
    	NSLog(@"Stop no recording");
    	return;
    }
    
    if(callback) {
	    [ConvertAudioFile conventToMp3WithCafFilePath:gCafFile mp3FilePath:gMp3File sampleRate:ETRECORD_RATE callback:^(BOOL result) {
		NSLog(@"To mp3 finish: %d", result);
		
		if(result && IsValidString(gMp3Record)) {
			NSFileManager *fileManager = [NSFileManager defaultManager];
			if ([fileManager copyItemAtPath:gMp3File toPath:gMp3Record error:nil])
				UnitySendMessage(GameObjectName, RecordingName, [gMp3Record UTF8String]);
			else
				UnitySendMessage(GameObjectName, RecordingName, [@"" UTF8String]);
		} else
		    UnitySendMessage(GameObjectName, RecordingName, [@"" UTF8String]);
	    }];
    }
...
- (void)onResp:(BaseResp *)resp // ???????????sendReq???,??onResp????
{
	...
	if(temp.errCode != 0)
		...
		if(gIsRelogin)
			UnityFeedback("ReloginResult", dictData);
		else
			UnityFeedback("LoginResult", dictData);
    }
    }
...
int AppController_StartRecording(NSString *fileName)
{
    return [GetAppController() StartRecording:fileName];
}
void AppController_StopRecording(BOOL callback)
{
    [GetAppController() StopRecording:callback];
}
