...
public class YSDKController {
    ...
    public void onCreate(UnityPlayerActivity activity){
        ...
        YSDKApi.setMainActivity("com.tencent.tmgp.hlttby.UnityPlayerActivity");
        ...
        YSDKApi.setAntiAddictListener(new YSDKCallback(mainActivity));
        YSDKApi.setAntiRegisterWindowCloseListener(YSDKCallback.sRegisterWindowCloseListener);
        //游戏助手内截屏分享状态信息
        YSDKApi.setScreenCapturer(()->{
            return null;
        });
        //注册分享监听器接受分享状态信息
    ...
    public void letUserLogin(UserLoginRet ret) {
        try {
            ...
            if(platform == ePlatform.QQ)
                jsonResult.put("appid", "1111018546");
            else if(platform == ePlatform.WX)
                jsonResult.put("appid", "wx2ab55998a2d85119");
            else
    ...
    /*public void onLogin(String platform) {
        ...
        if(platform.equals("yyb_hlttby_qq")) {
            YSDKApi.login(ePlatform.QQ);
        } else if(platform.equals("yyb_hlttby_wechat")) {
            YSDKApi.login(ePlatform.WX);
        } else {
            YSDKApi.login(ePlatform.Guest);
        }
